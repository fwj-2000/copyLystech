...
import { kebabToCamelCase } from '../index';
import { getFirstNonNullOrUndefined } from '../is';
 ...
 * @param state
 */
export function usePriorityValues<T extends Record<string, any>, S extends Ref<Record<string, any>> = Readonly<Ref<NoInfer<T>, NoInfer<T>>>>(
  props: T,
  state: S | undefined,
) {
 ...
 * @param state
 */
export function useForwardPriorityValues<T extends Record<string, any>, S extends Ref<Record<string, any>> = Readonly<Ref<NoInfer<T>, NoInfer<T>>>>(
  props: T,
  state: S | undefined,
) {
  ...
  return computed(() => {
    ...
    if (unwrapResult.i18nConfig) {
      unwrapResult.schema = transformI18nBasicForm(unwrapResult.schema, unwrapResult.i18nConfig);
    }
