...
const timeZone = localeStore.getTimeZone;
...
// 处理GET请求
function handleGetRequest(config: any, params: any, menuId: string, joinTime: boolean) {
  if (isString(params)) {
    // 兼容restful风格
...
// 处理非GET请求
function handleNonGetRequest(config: any, params: any, data: string, joinParamsToUrl, formatDate) {
  if (isString(params)) {
    // 兼容restful风格
  ...
  const hasData = Reflect.has(config, 'data') && config.data && (Object.keys(config.data).length > 0 || config.data instanceof FormData);
  // 非GET请求如果没有提供data，则将params视为data
  config.data = hasData ? data : params;
  config.params = undefined;
...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (options.errorMessageMode === 'modal') {
      ...
      errorMsg = typeof errorMsg == 'string' ? errorMsg : t('sys.api.operationFailed');
  ...
  beforeRequestHook: (config, options) => {
    ...
    const menuId: string = (meta.modelId as string) || '';
    ...
    // }
    if (config.method?.toUpperCase() === RequestEnum.GET) {
      handleGetRequest(config, params, menuId, joinTime);
    } else {
      handleNonGetRequest(config, params, data, joinParamsToUrl, formatDate);
    }
    return config;
  },
  ...
  requestInterceptors: (config, options) => {
    // 请求之前处理config
    (config as Recordable).headers['lydc-origin'] = 'pc';
    (config as Recordable).headers['vue-version'] = '3';
    (config as Recordable).headers['Accept-Language'] = locale.replace('_', '-');
    (config as Recordable).headers['Lydc-SystemId'] = userStore.getUserInfo?.systemId;
    (config as Recordable).headers['Time-Zone'] = timeZone === 'system' ? Intl.DateTimeFormat().resolvedOptions().timeZone : timeZone;
