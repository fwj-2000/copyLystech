...
export function getAmountChinese(val) {
  ...
  if (integer) {
    ...
    for (let i = 0, len = integer.length; i < len; i++) {
      ...
      if (num === '0') {
        ...
        zeroCount = 0;
        res += NUMBER[Number.parseInt(num)] + N_UNIT1[m];
      }
  ...
  // 小数部分
  if (Number.parseInt(decimal)) {
    for (let i = 0; i < 4; i++) {
      const num = decimal.charAt(i);
      if (Number.parseInt(num)) res += NUMBER[Number.parseInt(num)] + D_UNIT[i];
    }
  } else {
    res += '整';
  }
...
export function getDateFormat(format) {
  ...
  const formatObj = {
    ...
    'YYYY-MM-DD HH:mm:ss': 'YYYY-MM-DD HH:mm:ss',
    'YYYY-WW': 'YYYY-WW',
    'YYYY-WW-DD': 'YYYY-WW-DD',
    'YYYY-WW-DD HH:mm': 'YYYY-WW-DD HH:mm',
    'YYYY-WW-DD HH:mm:ss': 'YYYY-WW-DD HH:mm:ss',
  };
  return formatObj[format] || 'YYYY-MM-DD HH:mm:ss';
}
export function getLydcAppId() {
  let appId = '';
  if (window.location.pathname?.startsWith('/LYDC_APP_')) {
...
export function formatDecimal(number, fixed = 0) {
  // 将数字转换为字符串
  ...
  // 如果没有小数点，或者小数位数小于等于7，直接返回原数
  if (decimalIndex === -1 || numberStr.length - decimalIndex - 1 <= fixed) {
    return number;
  }

  // 保留7位小数
  const truncatedStr = numberStr.slice(0, decimalIndex + fixed + 1); // 保留到小数点后7位

  return Number.parseFloat(truncatedStr); // 转换为数字并返回
}
