...
const whitePathList: PageEnum[] = [
  LOGIN_PATH,
  SSO_PATH,
  BASE_FORM_SHORT_LINK_PATH,
  '/dashboardv1/large/map',
  '/dashboardv1/large/rate',
  '/hrtrain/employeeSign',
  '/hrtrain/mentorAudit',
  '/screen/production/outputAndUtilizationRate',
];
...
export function createPermissionGuard(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (to.name === PAGE_NOT_FOUND_ROUTE.name) {
      ...
      const nextData = to.path === redirect ? { ...to, replace: true } : { path: redirect, query: omit(from.query, 'redirect') };
      next(nextData);
    }
  });
}
