...
export function useFrameKeepAlive() {
  ...
  const getFrameKey = (item: AppRouteRecordRaw) => {
    const name = item.name as string;
    const refreshKey = tabStore.getIframeRefreshKey(name);
    return `${name}-${refreshKey}`;
  };
  ...
  return { hasRenderFrame, getFramePages, showIframe, getAllFramePages, getFrameKey };
}
