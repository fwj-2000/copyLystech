<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" size="large" class="full-drawer message-drawer" :closeFunc="handleClose">
    <template #title>
...
<script lang="ts" setup>
  ...
  import { toDateValue } from '/@/utils/lydc';
      ...
      if (item.type == 4) {
        ...
        if (item.type == 1 || item.type == 3) {
          ...
          if (url.includes('/Download?')) {
            downloadByUrl({ url: url, target: '_blank', fileName: '数据备份' });
          } else if (bodyText.isExternalLink) {
