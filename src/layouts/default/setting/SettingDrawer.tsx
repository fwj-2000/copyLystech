...
export default defineComponent({
  ...
  setup(_, { attrs }) {
    ...
    const emitOpenChange = (visible: boolean) => {
      const handler = attrs?.onOpenChange;
      if (Array.isArray(handler)) {
        handler.forEach(fn => {
          if (typeof fn === 'function') fn(visible);
        });
        return;
      }
    ...
    const handleDrawerOpenChange = (visible: boolean) => {
      emitOpenChange(visible);
    };
    ...
    return () => (
      <BasicDrawer {...attrs} title={t('layout.setting.drawerTitle')} width={330} class='setting-drawer' onOpenChange={handleDrawerOpenChange}>
        <div class='p-20px'>
