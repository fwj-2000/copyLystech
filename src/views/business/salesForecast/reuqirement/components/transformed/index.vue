...
<script lang="ts" setup>
  ...
  import { getSalesForecast, exportSalesForecast, withdrawSalesForecast, getNodeList } from '/@/api/business/salesForecast';
  import { MODULE_TYPE_ENUM } from '/@/views/business/salesForecast/config';
  import { STATUS_ENUM } from '../submited/config';

  import { NodeModal } from '/@/components/CustomComponents';
  import DetailPopup from '/@/views/business/salesForecast/reuqirement/components/submited/detail.vue';
  ...
  const [VxeBasicTable, tableRef] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'business-salesForecast-reuqirement-transformed-list',
      mouseConfig: {
        area: false, // 是否开启区域选取
        extension: false,
      },
      ...
      height: 'auto',
      // @ts-ignore
      api: (params: any) => getSalesForecast(MODULE_TYPE_ENUM.客户需求, { ...params, status: STATUS_ENUM.已处理 }),
      scrollY: {
        enabled: true,
        mode: 'wheel',
      },
              ...
              events: {
                click: () => {
                  openExportModal(true, {
                    listQuery: {},
                    api: query => exportSalesForecast(MODULE_TYPE_ENUM.客户需求, { ...query, status: STATUS_ENUM.已处理 }),
  ...
  function showDetail(params: { batchCode?: string; mainProcess?: string | number }) {
    openDetailPopup(true, { ...params, isCanEdit: false });
  }
  ...
  function handleWithdraw() {
    ...
    createConfirm({
      ...
      onOk: () =>
        withdrawSalesForecast(checkedList.map(item => item.id))
          .then(res => {
