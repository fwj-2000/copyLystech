...
export const vxeTableColumns: VxeGridPropTypes.Columns = [
  ...
  {
    title: '状态',
    field: 'pmConfirmStatus',
    showOverflow: 'tooltip',
    width: '120',
    slots: {
      default: ({ row }) => {
        const { theme, fullName } = STATUS_OPTIONS.find(item => item.id === row.pmConfirmStatus) || {};
        return h(LydcTag, { theme: theme }, () => row.statusDesc || fullName || row.pmConfirmStatus);
      },
    },
  },
  // 当前节点
