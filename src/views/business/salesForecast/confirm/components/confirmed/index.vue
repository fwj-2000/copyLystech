...
<script lang="ts" setup>
  ...
  const [VxeBasicTable, tableRef] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'business-salesForecast-confirm-confirmed-list',
      mouseConfig: {
        area: false, // 是否开启区域选取
        extension: false,
      },
      ...
      // @ts-ignore
      api: (params: any) => getSalesForecast(MODULE_TYPE_ENUM.需求确认, { ...params, status: STATUS_ENUM.已确认 }),
      toolbarConfig: {
        export: false,
        print: false,
        buttons: [
              ...
              events: {
                click: () => {
                  openExportModal(true, {
                    listQuery: {},
                    api: query => exportSalesForecast(MODULE_TYPE_ENUM.需求确认, { ...query, status: STATUS_ENUM.已确认 }),
