...
<script lang="ts" setup>
  ...
  const [VxeBasicTable, tableRef] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'business-salesForecast-audit-pending-list',
      mouseConfig: {
        area: false, // 是否开启区域选取
        extension: false,
      },
      ...
      api: (params: any) => getSalesForecast(MODULE_TYPE_ENUM.需求审核, { status: '', ...params }),
      toolbarConfig: {
        export: false,
        print: false,
        buttons: [
          {
            content: t('common.audit'),
            visible: hasBtnP('btn_review'),
            buttonRender: {
              name: 'AButton',
              // @ts-ignore
              props: { type: 'primary' },
              ...
              events: {
                click: () => {
                  openExportModal(true, {
                    listQuery: {},
                    api: (query: any) => exportSalesForecast(MODULE_TYPE_ENUM.需求审核, { status: '', ...query }),
