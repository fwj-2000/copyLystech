...
<script lang="ts" setup>
  ...
  const [VxeBasicTable, tableRef] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'business-salesForecast-audit-processed-list',
      mouseConfig: {
        area: false, // 是否开启区域选取
        extension: false,
      },
      ...
      api: (params: any) => getSalesForecast(MODULE_TYPE_ENUM.需求审核, { ...params, status: STATUS_ENUM.同意 }),
      toolbarConfig: {
        export: false,
        print: false,
        buttons: [
              ...
              events: {
                click: () => {
                  openExportModal(true, {
                    listQuery: {},
                    api: query => exportSalesForecast(MODULE_TYPE_ENUM.需求审核, { ...query, status: STATUS_ENUM.同意 }),
