      ...
      <a-table :data-source="list" :columns="columns" size="small" :pagination="false" :scroll="{ x: 'max-content', y: '440px' }" class="import-preview-table">
        <template #bodyCell="{ column, text, record, index }">
          ...
          <template v-if="column.dataIndex === 'InsidePartNumber'">
            <a-tooltip :title="text">
              <span class="epic-tooltip">{{ text }}</span>
            </a-tooltip>
          </template>
          <template v-else-if="column.dataIndex === 'UrgencyLevel'">
            <div v-if="text == 1">一般</div>
            <div v-if="text == 2">紧急</div>
            <div v-if="text == 3">特急</div>
          </template>
...
<script lang="ts" setup>
  ...
  import { getQuotaTemplateDownload, postQuotationRequirements } from '/@/api/business/quota';
  ...
  const tableData = [
    ...
    { title: '产品内部料号', dataIndex: 'InsidePartNumber', width: 150 },
    { title: '项目总量(Kpcs)', dataIndex: 'TotalQty', width: 150 },
    { title: '单位用量', dataIndex: 'UnitQty', width: 120 },
    { title: '招标壳厂数(家)', dataIndex: 'TenderFactoryQty', width: 150 },
    { title: '量产周期(月)', dataIndex: 'ProductionCycle', width: 150 },
    { title: '需求时间', dataIndex: 'RequireDate', width: 150 },

    { title: '终端客户项目阶段', dataIndex: 'TerminalCustomerProjectStage', width: 150 },
    { title: '预计量产日期', dataIndex: 'ExpectedProductionDate', width: 150 },
    { title: '峰值数量(KPCS)/月', dataIndex: 'PeakQty', width: 150 },
    { title: '峰值开始月份', dataIndex: 'PeakMonthFrom', width: 150 },
    { title: '峰值结束月份', dataIndex: 'PeakMonthTo', width: 150 },
    { title: '整单备注', dataIndex: 'Remark', width: 150 },

    { title: '项目负责人', dataIndex: 'ProjectLeaderName', width: 150 },
    { title: '紧急程度', dataIndex: 'UrgencyLevel', width: 150 },
    { title: '行备注', dataIndex: 'LineRemark', width: 150 },
  ];
  ...
  const getAction = computed(() => globSetting.apiUrl + '/api/Information/quotationrequirements/import');
  ...
  function handleNext() {
    if (state.activeStep == 1) {
      ...
      postQuotationRequirements(state.BatchCode)
        .then(res => {
          const { data, msg } = res;
          state.result = data;
          state.btnLoading = false;
  ...
  function handleTemplateDownload() {
    getQuotaTemplateDownload().then(res => {
