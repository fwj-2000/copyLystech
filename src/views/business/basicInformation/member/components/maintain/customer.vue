      ...
      <div class="flex justify-between mb-2">
        <div class="flex">
          ...
          <!-- <ApiSelect
            ...
            :params="{ type: 2, mainProcess: memberStore.mainProcess, projectCode: state.innerCode }"
            :style="{ width: '180px' }"
            v-model:value="state.customerCode"
            placeholder="直接客户代码"
            result-field="data"
            value-field="code"
            label-field="name"
...
<script setup lang="ts">
  ...
  import { getSlectOps, getCustomerCodeList, savewaitmaintain } from '/@/api/business/member';
  ...
  const [registerTable, { getSelectRows, reload, clearSelectedRowKeys }] = useTable({
    api: getCustomerCodeList,
    beforeFetch: param => {
      param.insideProjectCode = state.innerCode;
      param.customerCode = state.customerCode;
