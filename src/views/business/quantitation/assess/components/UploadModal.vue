<template>
  <BasicModal
    ...
    <div class="modal-content">
      <Subtitle :title="t('dict.EngineeringDocApply.DocType.1')" />
      <a-upload v-feature class="btn-upload" :multiple="true" v-bind="customerUploadOption">
        <div class="upload-btn-wrapper">
          <a-button type="primary" ghost :disabled="customerState.uploadStatus === UploadStatus.Uploading">选择文件 </a-button>
...
<script setup lang="ts">
  ...
  import { uploadDrawings } from '/@/api/business/quantitation';
  ...
  const customerUpload = async info => {
    ...
    try {
      ...
      const { msg } = await uploadDrawings(state.Id, { file: info.file });
      message.success(msg);
      // 上传成功
      customerState.procedure = 100;
      customerState.uploadStatus = UploadStatus.Success;
    } catch (e) {
      console.error('🚀 ~ UploadModal.vue:163 ~ customerUpload ~ e:', e);
  ...
  const handleConfirm = () => {
    ...
    if (customerFileList.length > 0) {
      ...
      uploadDrawings(state.Id, formData)
        .then(({ code, msg }) => {
          if (code == 200) {
            // 上传成功
            state.customerFileList = [];
