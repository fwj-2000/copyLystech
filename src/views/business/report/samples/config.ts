...
const _vxeTableColumns: VxeGridPropTypes.Columns = [
  ...
  },
  {
    ...
    i18nField: 'drawingsReviewApplyDateTime',
    showOverflow: 'tooltip',
    width: '120',
    // formatter: ({ cellValue }) => (cellValue ? dayjs(cellValue).tz().format('YYYY-MM-DD HH:mm:ss') : ''),
    cellRender: {
  ...
  },
  {
    ...
    showOverflow: 'tooltip',
    width: '120',
    formatter: ({ cellValue, row }) => getDFMLabel(row.dFMEnum, cellValue),
  },
  {
    field: 'makeEngineeringInfo',
    showOverflow: 'tooltip',
    width: '120',
    formatter: ({ cellValue, row }) => getMakeEngineeringInfoLabel(row.makeEngineeringInfoEnum, cellValue),
  },
  {
