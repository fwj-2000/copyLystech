<template>
  <BasicPopup
    ...
    class="full-popup">
    <div class="box">
      <Subtitle :title="t('common.updateSelected')" />
      <Grid>
        <template #action="{ row }">
...
<script setup lang="ts">
  import { batchStategyMaterialColumns } from '../config';
  import { useMessage } from '/@/hooks/web/useMessage';
  import { ActionItem } from '/@/components/Table';
  import { getDetails, updateStrategy } from '/@/api/structure/materialCode';
  ...
  const [Grid, gridApi] = useVbenVxeGrid({
    gridOptions: {
      ...
      columns: batchStategyMaterialColumns,
      keyboardConfig: {
        isBack: false,
      },
      pagerConfig: {
  ...
  async function handleSubmit() {
    ...
    updateStrategy(tableData)
      .then(res => {
        if (res.code === 200) {
          createMessage.success(res.msg || t('common.submittedSuccessfully'));
          emits('reload');
          closePopup();
        }
      })
