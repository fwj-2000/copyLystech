<template>
  <div class="lydc-content-wrapper short-link-wrapper short-link-wrapper-list">
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="!(record.top || column.id?.includes('-'))">
                <template v-if="column.lydcKey === 'inputNumber'">
                  <lydc-input-number v-model:value="record[column.dataIndex]" :precision="column.precision" :thousands="column.thousands" disabled detailed />
                </template>
...
<script lang="ts" setup>
  ...
  import { getScriptFunc, thousandsFormat } from '/@/utils/lydc';
  ...
  function validFields(o) {
    if (!o.__config__ || !o.__config__.lydcKey) return true;
    const lydcKey = o.__config__.lydcKey;
    if (validFieldsList.includes(lydcKey) || (selectFieldsList.includes(lydcKey) && o.__config__.dataType === 'static')) return true;
    return false;
  }
