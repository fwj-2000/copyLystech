  ...
  function fillFormData(form, data) {
    const loop = (list, parent?) => {
      for (let i = 0; i < list.length; i++) {
        let item = list[i];
        if (item.__vModel__) {
          if (item.__config__.lydcKey === 'relationForm' || item.__config__.lydcKey === 'popupSelect') {
          ...
          if (['relationFormAttr', 'popupAttr'].includes(item.__config__.lydcKey)) {
            item.__config__.defaultValue = data[item.relationField.split('_lydcTable_')[0] + '_' + item.showField];
          }
        }
