<template>
  <a-col
    ...
    <template v-if="getConfig.layout === 'colFormItem'">
      <template v-if="getConfig.lydcKey === 'divider'">
        <lydc-divider :contentPosition="item.contentPosition" :content="item.content" />
      </template>
      <template v-else>
        <a-form-item :name="item.__vModel__" :labelCol="getLabelCol">
          ...
          <template v-if="getConfig.lydcKey === 'text'">
            <lydc-text :content="item.content" :textStyle="item.textStyle" />
          </template>
          <template v-else-if="getConfig.lydcKey === 'link'">
            <lydc-link :content="item.content" :href="item.href" :target="item.target" :textStyle="item.textStyle" />
          </template>
          <template v-else-if="getConfig.lydcKey === 'alert'">
            <lydc-alert
              :title="item.title"
              :type="item.type"
          ...
          <template v-else-if="getConfig.lydcKey === 'groupTitle'">
            <lydc-group-title :content="item.content" :contentPosition="item.contentPosition" :helpMessage="item.helpMessage" />
          </template>
          <template v-else-if="getConfig.lydcKey === 'button'">
            <lydc-button :align="item.align" :buttonText="item.buttonText" :type="item.type" :disabled="item.disabled" />
          </template>
          <template v-else-if="getConfig.lydcKey === 'uploadFile'">
            <lydc-upload-file v-model:value="getConfig.defaultValue" detailed disabled />
          </template>
          <template v-else-if="getConfig.lydcKey === 'uploadImg'">
            <lydc-upload-img v-model:value="getConfig.defaultValue" detailed disabled />
          </template>
          <template v-else-if="getConfig.lydcKey === 'colorPicker'">
            <lydc-color-picker v-model:value="getConfig.defaultValue" :showAlpha="item.showAlpha" :colorFormat="item.colorFormat" disabled />
          </template>
          <template v-else-if="getConfig.lydcKey === 'rate'">
            <lydc-rate v-model:value="getConfig.defaultValue" :count="item.count" :allowHalf="item.allowHalf" disabled />
          </template>
          <template v-else-if="getConfig.lydcKey === 'slider'">
            <lydc-slider v-model:value="getConfig.defaultValue" :min="item.min" :max="item.max" :step="item.step" disabled />
          </template>
          <template v-else-if="getConfig.lydcKey === 'editor'">
            <div v-html="getConfig.defaultValue"></div>
          </template>
          <template v-else-if="getConfig.lydcKey === 'relationForm'">
            <p class="link-text" @click="toDetail(item)">{{ item.name }}</p>
          </template>
          <template v-else-if="getConfig.lydcKey === 'popupSelect'">
            <p>{{ item.name }}</p>
          </template>
          <template v-else-if="getConfig.lydcKey === 'barcode'">
            <lydc-barcode
              :format="item.format"
          ...
          <template v-else-if="getConfig.lydcKey === 'qrcode'">
            <lydc-qrcode
              :format="item.format"
          ...
          <template v-else-if="getConfig.lydcKey === 'inputNumber'">
            <lydc-input-number
          ...
          <template v-else-if="getConfig.lydcKey === 'calculate'">
            <lydc-calculate
          ...
          <template v-else-if="getConfig.lydcKey === 'location'">
            <lydc-location v-model:value="getConfig.defaultValue" :enableLocationScope="item.enableLocationScope" detailed />
          </template>
          <template v-else-if="getConfig.lydcKey === 'sign'">
            <lydc-sign v-model:value="getConfig.defaultValue" detailed />
          </template>
          <template v-else-if="getConfig.lydcKey === 'signature'">
            <lydc-signature v-model:value="getConfig.defaultValue" detailed />
          </template>
          <template v-else-if="getConfig.lydcKey === 'iframe'">
            <lydc-iframe
              :href="item.href"
              :height="item.height"
          ...
          <template v-else-if="getConfig.lydcKey === 'input'">
            <lydc-input
              v-model:value="getConfig.defaultValue"
              :addonBefore="item.addonBefore"
              :addonAfter="item.addonAfter"
    ...
    <template v-else>
      <template v-if="getConfig.lydcKey === 'card'">
      ...
      <a-row v-if="getConfig.lydcKey === 'row'">
        <Item v-for="(childItem, childIndex) in getConfig.children" v-bind="getBindValue" :key="childIndex" :item="childItem" @toDetail="toDetail" />
      </a-row>
      <template v-if="getConfig.lydcKey === 'tab'">
      ...
      <template v-if="getConfig.lydcKey === 'collapse'">
      ...
      <template v-if="getConfig.lydcKey === 'table'">
        <a-form-item v-if="!getConfig.noShow">
          <LydcGroupTitle :content="getConfig.label" v-if="getConfig.showTitle && getConfig.label" :helpMessage="getConfig.tipLabel" :bordered="false" />
          <a-table
            ...
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-else-if="column.__config__?.lydcKey === 'uploadFile'">
                <lydc-upload-file v-model:value="record[column.dataIndex]" detailed disabled />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'uploadImg'">
                <lydc-upload-img v-model:value="record[column.dataIndex]" detailed disabled />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'colorPicker'">
                <lydc-color-picker v-model:value="record[column.dataIndex]" :showAlpha="column.showAlpha" :colorFormat="column.colorFormat" disabled />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'rate'">
                <lydc-rate v-model:value="record[column.dataIndex]" :count="column.count" :allowHalf="column.allowHalf" disabled />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'slider'">
                <lydc-slider v-model:value="record[column.dataIndex]" :min="column.min" :max="column.max" :step="column.step" disabled />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'relationForm'">
                <p class="link-text" @click="toTableDetail(column, record[column.dataIndex + '_id'])">{{ record[column.dataIndex] }}</p>
              </template>
              <template v-else-if="['relationFormAttr', 'popupAttr'].includes(column.__config__?.lydcKey)">
                <p v-if="!record[column.dataIndex]">{{ record[column.relationField.split('_lydcTable_')[0] + '_' + column.showField] }}</p>
                <p v-else>{{ record[column.dataIndex] }}</p>
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'inputNumber'">
                <lydc-input-number
              ...
              <template v-else-if="column.__config__?.lydcKey === 'calculate'">
                <lydc-calculate
                  :rowIndex="index"
              ...
              <template v-else-if="column.__config__?.lydcKey === 'location'">
                <lydc-location v-model:value="record[column.dataIndex]" :enableLocationScope="column.enableLocationScope" detailed />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'sign'">
                <lydc-sign v-model:value="record[column.dataIndex]" detailed />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'signature'">
                <lydc-signature v-model:value="record[column.dataIndex]" detailed />
              </template>
              <template v-else-if="column.__config__?.lydcKey === 'input'">
                <lydc-input
                  v-model:value="record[column.dataIndex]"
                  :addonBefore="column.addonBefore"
                  :addonAfter="column.addonAfter"
      ...
      <template v-if="getConfig.lydcKey === 'tableGrid'">
        <table
...
<script lang="ts" setup>
  ...
  const getColumns = computed(() => {
    if (unref(getConfig).lydcKey !== 'table') return [];
