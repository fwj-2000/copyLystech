<template>
  <div class="lydc-content-wrapper">
    <div class="lydc-content-wrapper-left" v-if="columnData.type === 2">
      <BasicLeftTree v-bind="getLeftTreeBindValue" ref="leftTreeRef" @reload="getTreeView()" @select="handleLeftTreeSelect" />
    </div>
    <div class="lydc-content-wrapper-center">
      <div class="lydc-content-wrapper-search-box" v-if="columnData.searchList?.length">
        <BasicForm
              ...
              <a-tab-pane :key="cIndex" :tab="child.label" :label="child.label" v-for="(child, cIndex) in childColumnList">
                <BasicTable @register="registerChildTable" :ellipsis="!!columnData.showOverflow" :data-source="record[child.prop]" :columns="child.children">
                  <template #bodyCell="{ column, record: childRecord }">
                    ...
                    <template v-if="column.lydcKey === 'calculate'">
                      <lydc-calculate
                        v-model:value="childRecord[column.dataIndex]"
                    ...
                    <template v-if="column.lydcKey === 'sign'">
                      <lydc-sign v-model:value="childRecord[column.dataIndex]" detailed />
                    </template>
                    <template v-if="column.lydcKey === 'signature'">
                      <lydc-signature v-model:value="childRecord[column.dataIndex]" detailed />
                    </template>
                    <template v-if="column.lydcKey === 'rate'">
                      <lydc-rate v-model:value="childRecord[column.dataIndex]" :count="column.count" :allowHalf="column.allowHalf" disabled />
                    </template>
                    <template v-if="column.lydcKey === 'slider'">
                      <lydc-slider v-model:value="childRecord[column.dataIndex]" :min="column.min" :max="column.max" :step="column.step" disabled />
                    </template>
                    <template v-if="column.lydcKey === 'uploadImg'">
                      <lydc-upload-img v-model:value="childRecord[column.dataIndex]" disabled detailed simple v-if="childRecord[column.dataIndex]?.length" />
                    </template>
                    <template v-if="column.lydcKey === 'uploadFile'">
                      <lydc-upload-file v-model:value="childRecord[column.dataIndex]" disabled detailed simple v-if="childRecord[column.dataIndex]?.length" />
                    </template>
                    <template v-if="column.lydcKey === 'input'">
                      <lydc-input
                        v-model:value="childRecord[column.dataIndex]"
          ...
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="columnData.type === 4">
              <template v-if="record.rowEdit">
                ...
                <template v-else-if="column.lydcKey === 'calculate'">
                  <lydc-calculate
                    v-model:value="record[column.prop]"
                    :isStorage="column.isStorage"
                    :precision="column.precision"
                ...
                <template v-else-if="column.lydcKey === 'rate'">
                  <lydc-rate v-model:value="record[column.prop]" :count="column.count" :allowHalf="column.allowHalf" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.lydcKey === 'slider'">
                  <lydc-slider v-model:value="record[column.prop]" :min="column.min" :max="column.max" :step="column.step" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.lydcKey === 'uploadImg'">
                  <lydc-upload-img
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.lydcKey === 'uploadFile'">
                  <lydc-upload-file
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.lydcKey === 'switch'">
                  <lydc-switch v-model:value="record[column.prop]" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.lydcKey === 'timePicker'">
                  <lydc-time-picker
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.lydcKey === 'datePicker'">
                  <lydc-date-picker
                    v-model:value="record[column.prop]"
                    :format="column.format"
                    :startTime="column.startTime"
                ...
                <template v-else-if="column.lydcKey === 'organizeSelect'">
                  <lydc-organize-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                ...
                <template v-else-if="column.lydcKey === 'depSelect'">
                  <lydc-dep-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'roleSelect'">
                  <lydc-role-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'groupSelect'">
                  <lydc-group-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'posSelect'">
                  <lydc-pos-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'userSelect'">
                  <lydc-user-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'usersSelect'">
                  <lydc-users-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'areaSelect'">
                  <lydc-area-select
                    v-model:value="record[column.prop]"
                    :level="column.level"
                    :placeholder="column.placeholder"
                ...
                <template v-else-if="['select', 'radio', 'checkbox'].includes(column.lydcKey)">
                  <lydc-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple || column.lydcKey === 'checkbox'"
                    :allowClear="column.clearable || ['radio', 'checkbox'].includes(column.lydcKey)"
                    :showSearch="column.filterable"
                    :disabled="column.disabled"
                    :options="column.options"
                    :fieldNames="column.props" />
                </template>
                <template v-else-if="column.lydcKey === 'cascader'">
                  <lydc-cascader
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'treeSelect'">
                  <lydc-tree-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'relationForm'">
                  <lydc-relation-form
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :allowClear="column.clearable"
                ...
                <template v-else-if="column.lydcKey === 'popupSelect' || column.lydcKey === 'popupTableSelect'">
                  <lydc-popup-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.lydcKey === 'autoComplete'">
                  <lydc-auto-complete
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :allowClear="column.clearable"
                ...
                <template v-else-if="['input', 'textarea'].includes(column.lydcKey)">
                  <lydc-input
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :allowClear="column.clearable"
                ...
                <template v-else-if="column.lydcKey === 'location'">
                  <lydc-location
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.lydcKey === 'sign'">
                  <lydc-sign v-model:value="record[column.prop]" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.lydcKey === 'signature'">
                  <lydc-signature v-model:value="record[column.prop]" :disabled="column.disabled" />
                </template>
                <template v-else-if="systemComponentsList.includes(column.lydcKey)">
              ...
              <template v-else>
                ...
                <template v-else-if="column.lydcKey === 'sign'">
                  <lydc-sign v-model:value="record[column.prop + '_name']" detailed />
                </template>
                <template v-else-if="column.lydcKey === 'signature'">
                  <lydc-signature v-model:value="record[column.prop + '_name']" detailed />
                </template>
                <template v-else-if="column.lydcKey === 'rate'">
                  <lydc-rate v-model:value="record[column.prop + '_name']" :count="column.count" :allowHalf="column.allowHalf" disabled />
                </template>
                <template v-else-if="column.lydcKey === 'slider'">
                  <lydc-slider v-model:value="record[column.prop + '_name']" :min="column.min" :max="column.max" :step="column.step" disabled />
                </template>
                <template v-else-if="column.lydcKey === 'uploadImg'">
                  <lydc-upload-img v-model:value="record[column.prop + '_name']" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-else-if="column.lydcKey === 'uploadFile'">
                  <lydc-upload-file v-model:value="record[column.prop + '_name']" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-else-if="column.lydcKey === 'input'">
                  <lydc-input
                    v-model:value="record[column.prop + '_name']"
                    :useMask="column.useMask"
                    :maskConfig="column.maskConfig"
            ...
            <template v-else>
              ...
              <template v-if="!(record.top || column.id?.includes('-'))">
                ...
                <template v-if="column.lydcKey === 'inputNumber'">
                  <lydc-input-number v-model:value="record[column.prop]" :precision="column.precision" :thousands="column.thousands" disabled detailed />
                </template>
                <template v-if="column.lydcKey === 'calculate'">
                  <lydc-calculate
                    v-model:value="record[column.prop]"
                ...
                <template v-if="column.lydcKey === 'sign'">
                  <lydc-sign v-model:value="record[column.prop]" detailed />
                </template>
                <template v-if="column.lydcKey === 'signature'">
                  <lydc-signature v-model:value="record[column.prop]" detailed />
                </template>
                <template v-if="column.lydcKey === 'rate'">
                  <lydc-rate v-model:value="record[column.prop]" :count="column.count" :allowHalf="column.allowHalf" disabled />
                </template>
                <template v-if="column.lydcKey === 'slider'">
                  <lydc-slider v-model:value="record[column.prop]" :min="column.min" :max="column.max" :step="column.step" disabled />
                </template>
                <template v-if="column.lydcKey === 'uploadImg'">
                  <lydc-upload-img v-model:value="record[column.prop]" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-if="column.lydcKey === 'uploadFile'">
                  <lydc-upload-file v-model:value="record[column.prop]" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-if="column.lydcKey === 'input'">
                  <lydc-input
                    v-model:value="record[column.prop]"
...
<script lang="ts" setup>
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, thousandsFormat, getParamList } from '/@/utils/lydc';
  ...
  import { LydcRelationForm } from '/@/components/Lydc';
  ...
  const getSummaryColumn = computed(() => {
    ...
    for (let i = 0; i < defaultColumns.length; i++) {
      ...
      if (e.lydcKey === 'table' || e.lydcKey === 'complexHeader') {
  ...
  function handleLeftTreeSelect(id, _node, nodePath) {
    ...
    if (state.treeRelationObj?.lydcKey && ['organizeSelect', 'cascader', 'areaSelect'].includes(state.treeRelationObj.lydcKey)) {
      let currValue = [];
      if (state.columnData.treeDataSource === 'formField' && state.treeRelationObj.lydcKey === 'organizeSelect') {
  ...
  function addHandleForRowEdit() {
    let item = { rowEdit: true, id: 'lydcAdd' };
    ...
    for (let i = 0; i < state.columnData.columnList.length; i++) {
      ...
      if (config.defaultCurrent) {
        if (config.lydcKey === 'datePicker') {
          config.defaultValue = dayjs(currDate).tz().startOf(getDateTimeUnit(e.format)).valueOf();
        }
        if (config.lydcKey === 'timePicker') {
          config.defaultValue = dayjs(currDate)
            .tz()
            .format(e.format || 'HH:mm:ss');
        }
        if (config.lydcKey === 'organizeSelect' && userInfo.organizeIdList?.length) {
          config.defaultValue = e.multiple ? [userInfo.organizeIdList] : userInfo.organizeIdList;
        }
        if (config.lydcKey === 'depSelect' && userInfo.departmentId) {
          config.defaultValue = e.multiple ? [userInfo.departmentId] : userInfo.departmentId;
        }
        if (config.lydcKey === 'userSelect' && userInfo.userId) {
          config.defaultValue = e.multiple ? [userInfo.userId] : userInfo.userId;
        }
        if (config.lydcKey === 'usersSelect' && userInfo.userId) {
          config.defaultValue = e.multiple ? [userInfo.userId + '--user'] : userInfo.userId + '--user';
        }
        if (config.lydcKey === 'posSelect' && userInfo.positionIds?.length) {
          config.defaultValue = e.multiple ? userInfo.positionIds.map(o => o.id) : userInfo.positionIds[0].id;
        }
        if (config.lydcKey === 'roleSelect' && userInfo.roleIds?.length) {
          config.defaultValue = e.multiple ? userInfo.roleIds : userInfo.roleIds[0];
        }
        if (config.lydcKey === 'groupSelect' && userInfo.groupIds?.length) {
          config.defaultValue = e.multiple ? userInfo.groupIds : userInfo.groupIds[0];
        }
        if (config.lydcKey === 'sign' && userInfo.signImg) {
  ...
  function editForRowEdit(record) {
    ...
    state.currFlow = list.length ? list[0] : state.flowList[0];
  ...
  function cancelRowEdit(record, index) {
    const id = !record.id || record.id === 'lydcAdd' ? '' : record.id;
    if (!id) return deleteTableDataRecord('lydcAdd');
  ...
  function submitForRowEdit(record) {
    if (props.isPreview) return createMessage.warning('功能预览不支持数据保存');
    record.id = !record.id || record.id === 'lydcAdd' ? '' : record.id;
  ...
  async function getTreeView() {
    ...
    if (state.columnData.treeDataSource === 'formField') {
      ...
      const lydcKey = treeRelationObj?.__config__?.lydcKey || '';
      if (state.columnData.treeRelation && ['organizeSelect', 'depSelect'].includes(lydcKey)) {
        ...
        if (treeRelationObj.selectType === 'custom' && treeRelationObj.ableIds?.length) {
          const departIds = lydcKey === 'organizeSelect' ? treeRelationObj.ableIds.map(o => o[o.length - 1]) : treeRelationObj.ableIds;
    ...
    if (state.columnData.treeRelation) {
      for (let i = 0; i < state.columnData.columnOptions.length; i++) {
        ...
        if (e.id === state.columnData.treeRelation) {
          state.treeRelationObj = { ...e, searchMultiple: false, lydcKey: e.__config__.lydcKey };
          break;
        }
      }
    }
  ...
  function getExportList(list) {
    let exportList: any[] = [];
    for (let i = 0; i < list.length; i++) {
      if (list[i].lydcKey === 'table') {
        if (state.columnData.type != 4) exportList.push(...list[i].children);
      } else if (list[i].lydcKey === 'complexHeader') {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (config.lydcKey === 'datePicker') {
          if (config.startTimeRule) {
            ...
            if (config.startTimeType == 4 || config.startTimeType == 5) {
              ...
              const startTime = dayjs().tz()[method](config.startTimeValue, type);
          ...
          if (config.endTimeRule) {
            ...
            if (config.endTimeType == 4 || config.endTimeType == 5) {
              ...
              const endTime = dayjs().tz()[method](config.endTimeValue, type);
        ...
        if (config.lydcKey === 'timePicker') {
          if (config.startTimeRule) {
            ...
            if (config.startTimeType == 3) cur.startTime = dayjs().tz().format(cur.format);
            if (config.startTimeType == 4 || config.startTimeType == 5) {
              const type = getTimeUnit(config.startTimeTarget + 3);
              const method = config.startTimeType == 4 ? 'subtract' : 'add';
              const startTime = dayjs().tz()[method](config.startTimeValue, type).format(cur.format);
              cur.startTime = startTime;
            }
          }
          if (config.endTimeRule) {
            if (config.endTimeType == 1) cur.endTime = config.endTimeValue || null;
            if (config.endTimeType == 3) cur.endTime = dayjs().tz().format(cur.format);
            if (config.endTimeType == 4 || config.endTimeType == 5) {
              const type = getTimeUnit(config.endTimeTarget + 3);
              const method = config.endTimeType == 4 ? 'subtract' : 'add';
              const endTime = dayjs().tz()[method](config.endTimeValue, type).format(cur.format);
              cur.endTime = endTime;
            }
          }
        }
