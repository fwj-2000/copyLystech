<template>
  <BasicPopup
    ...
    <template #insertToolbar>
      <a-button v-if="state.pollingParams.interval" @click="handleBack" type="primary" class="mr-15px">{{ t('common.clearData') }}</a-button>
    </template>
    <ScrollContainer>
      <uploadExcelData
        class="h-full"
...
<script lang="ts" setup>
  import { h, nextTick, onMounted, reactive, ref, toRefs } from 'vue';
  ...
  interface State {
    ...
    pollingParams: {
      ...
      getTaskDataList: (params: any) => Promise<any>;
    ...
    pollingInterval: Array<NodeJS.Timeout | null>;
    intervalTimer: any;
    multiplyKey: any;
    tplList: any[];
  ...
  const state = reactive<State>({
    ...
    pollingParams: {
      interval: 10000,
      getTaskStatus: () => {},
      getTaskDataList: () => Promise.resolve([]),
      cancelTask: () => {},
      taskRead: () => {},
    },
    multiplyList: [], // 多模板列表
