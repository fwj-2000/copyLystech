        ...
        <template #bodyCell="{ column, record, index }">
          <template v-for="item in tableData">
            <template v-if="column.key === item.key">
              <lydc-date-picker
                v-if="column.key == 'employmentDate'"
                class="!w-full"
...
<script lang="ts" setup>
  ...
  import { templateDownload, importPreview, importData } from '/@/api/hr/training/employee';
  ...
  const tableData = [
    ...
    { title: '办公级培训负责人', dataIndex: 'officeTrainingCoordinatorWorkNo', key: 'officeTrainingCoordinatorWorkNo', width: 150 },
    { title: '车间级培训负责人', dataIndex: 'workshopTrainingCoordinatorWorkNo', key: 'workshopTrainingCoordinatorWorkNo', width: 150 },
    { title: '班组级培训负责人', dataIndex: 'teamLevelTrainingCoordinatorWorkNo', key: 'teamLevelTrainingCoordinatorWorkNo', width: 150 },
  ];
  const columns: any[] = [
  ...
  const getAction = computed(() => globSetting.apiUrl + '/api/hrbis/employeetraining/Uploader');
  ...
  function handleNext() {
    if (state.activeStep == 0) {
      ...
      importPreview({ fileName: state.fileName })
        .then(res => {
          state.list = (res.data?.dataRow || []).map(item => {
            return { ...item, employmentDate: item.employmentDate ? dayjs(item.employmentDate).tz().valueOf() : '' };
          });
