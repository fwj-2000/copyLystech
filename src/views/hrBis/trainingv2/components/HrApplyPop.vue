  ...
  <BasicPopup
    ...
    <ScrollContainer style="background: #ebeef5">
      ...
      <div style="display: flex; gap: 12px; padding: 12px 0">
        <Card class="flex-1">
          <template #title> {{ t('dict.EmployeeTrain.EmployeeSubmitText') }} </template>
          <BasicForm @register="registerEmployeeSubmitInfoForm">
            <template #signature="{ model, field }">
              <div class="relative">
                <div class="h-10px rotate-90 absolute left-[23%]" style="translate: -110% -80%">
                  <a-image style="height: 160px" :src="employeeSignImgBse64" object-fit="cover" :preview="false" loading="lazy"> </a-image>
                </div>
              </div>
            </template>
          </BasicForm>
  ...
  async function getDetail(id?: number | string) {
    ...
    try {
      ...
      if (r.code == 200) {
        ...
        if (state.base.employeeSignInfo) {
