    ...
    <div class="lydc-content-wrapper-center">
      <div class="lydc-content-wrapper-content">
        <BasicTable @register="registerTable">
          <template #tableTitle>
            <a-button type="primary" preIcon="icon-ym icon-ym-btn-add" @click="handleAdd()">{{ t('common.addText') }}11</a-button>
          </template>
...
<script lang="ts" setup>
  ...
  function getGoodsColumnSum(data) {
    ...
    unref(goodsColumns).forEach((column, index) => {
      ...
      let sumVal = data.reduce((sum, d) => sum + Number.parseFloat(d[column.dataIndex as any]), 0);
