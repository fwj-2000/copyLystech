<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" title="项目任务" class="full-popup" :destroy-on-close="true">
    <BasicTable :columns="columns" @register="registerTable" class="lydc-task-table lydc-sub-table">
      <template #toolbar>
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'startTime' || column.key === 'endTime'">
          <span>{{ dayjs(record[column.key]).tz().format('YYYY-M-D') }}</span>
        </template>
...
<script lang="ts" setup>
  ...
  function getColumns() {
    let start_date_spilt = dayjs(state.startDate).tz().format('YYYY-M-D').split('-');
    let end_date_spilt = dayjs(state.endDate).tz().format('YYYY-M-D').split('-');
  ...
  function dayGanttType(row, date, unit: any = 'days') {
    ...
    let between = dayjs(date).tz().isBetween(start_date, end_date, unit);
    if (between) return 'item-on';
    let start = dayjs(start_date).tz().isSame(date, unit);
    let end = dayjs(end_date).tz().isSame(date, unit);
