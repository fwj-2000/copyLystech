          ...
          <a-row>
            <a-col :span="8">
              <a-form-item label="发货仓库" name="goodsWarehouse">
                <lydc-select v-model:value="dataForm.goodsWarehouse" placeholder="发货仓库" :options="storeOptions" />
              </a-form-item>
            </a-col>
            <a-col :span="8">
            ...
            <a-col :span="8">
              <a-form-item label="收款方式" name="gatheringType">
                <lydc-select v-model:value="dataForm.gatheringType" placeholder="选择收款方式" :options="options" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
        ...
        <a-col :span="6">
          <a-form :labelCol="{ style: { width: '120px' } }">
            <a-form-item label="订货类型">
              <lydc-select v-model:value="orderType" placeholder="选择订货类型" :options="orderTypeOptions" />
            </a-form-item>
          </a-form>
        </a-col>
      </a-row>
...
<script lang="ts" setup>
  ...
  watch(
    ...
    val => {
      ...
      for (let i = 0; i < val.length; i++) {
        money += Number.parseFloat(val[i].amount);
      }
      state.total = money;
    },
    {
  ...
  function handleProductSelect(item, index) {
    ...
    state.dataForm.productEntryList[index].amount = toDecimal(
      Number.parseFloat(state.dataForm.productEntryList[index].money) * Number.parseFloat(state.dataForm.productEntryList[index].qty),
    );
  }
