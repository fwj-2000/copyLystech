        ...
        <a-form :colon="false" :labelCol="{ style: { width: '110px' } }">
          <a-form-item label="用户邮箱">
            <a-col v-for="(item, i) in emailList" :key="i" class="align-center" :class="{ 'mt-10px': i != 0 }">
              <lydc-select v-model:value="item.email" :options="options" showSearch placeholder="选择邮箱" />
          ...
          <a-form-item label="产品评价">
            <lydc-color-picker v-model:value="color" />
          </a-form-item>
          <a-form-item label="产品评价">
            <lydc-slider v-model:value="range" />
          </a-form-item>
          ...
          <LydcGroupTitle content="订单详细" :bordered="false" />
