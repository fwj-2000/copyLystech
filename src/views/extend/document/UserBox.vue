...
<script lang="ts" setup>
  ...
  import { createShare, getShareUserList } from '/@/api/extend/document';
  ...
  function init(data) {
    ...
    state.nodeId = '0';
    state.activeKey = '1';
    getShareUserList(state.documentId).then(res => {
  ...
  function handleSubmit() {
    ...
    const userIds = state.selectedData.map(o => o.id).join(',');
    createShare(state.documentId, userIds)
      .then(res => {
