<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="表单外链配置" :width="800" @ok="handleSubmit" class="lydc-shortLink-modal">
    <a-form :colon="false" :model="dataForm" :rules="rules" :labelCol="{ style: { width: '100px' } }" ref="formElRef" hideRequiredMark class="h-500px">
      <a-form-item label="外链填单" name="formUse">
        <a-space :size="15">
          <LydcSwitch v-model:value="dataForm.formUse" />
        ...
        <a-row justify="start">
          <a-col>
            <a-form-item label="凭密码填写" name="formPassUse">
              <LydcSwitch v-model:value="dataForm.formPassUse" />
            </a-form-item>
          </a-col>
          <a-col class="ml-15px">
      ...
      <template v-if="webType != '1'">
        ...
        <a-form-item label="公开查询" name="columnUse">
          <a-space :size="15">
            <LydcSwitch v-model:value="dataForm.columnUse" />
          ...
          <a-row justify="start">
            <a-col>
              <a-form-item label="凭密码填写" name="columnPassUse">
                <LydcSwitch v-model:value="dataForm.columnPassUse" />
              </a-form-item>
            </a-col>
            <a-col class="ml-15px">
...
<script lang="ts" setup>
  ...
  function getOptions(id) {
    getInfo(id).then(res => {
      ...
      const searchOptions = list.filter(o => !noSearchList.includes(o.__config__.lydcKey));
