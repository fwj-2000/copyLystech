<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="日志详情" width="800px" :footer="null" class="lydc-log-detail-modal">
    <div class="log-detail-board">
      <div class="left-board">
        <div class="item-box" :class="{ active: activeItem.id == item.id }" v-for="(item, index) in list" @click="handleClick(item, index)">
          ...
          <div class="bottom">
            ...
            {{ dayjs(item.endTime).tz().format('YYYY-MM-DD HH:mm:ss') }}
          </div>
        </div>
      </div>
      <div class="center-board">
        ...
        <div class="time-box">
          <span>开始时间：{{ dayjs(activeItem.startTime).tz().format('YYYY-MM-DD HH:mm:ss') }}</span>
          <span>结束时间：{{ dayjs(activeItem.endTime).tz().format('YYYY-MM-DD HH:mm:ss') }}</span>
        </div>
