<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="getTitle" class="full-popup">
    <BasicTable :columns="columns" @register="registerTable" class="lydc-sub-table lydc-sub-table-full" v-show="!showFlow">
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'processId'">
          {{ record.processId }}
          <template v-if="record.isRetry">
            <a-tag color="blue" class="!mx-8px">重试</a-tag>
            <BasicHelp :text="[`原实例ID：${record.parentId}`, `原实例执行时间：${dayjs(record.parentTime).tz().format('YYYY-MM-DD HH:mm:ss')}`]">
