...
<script setup lang="ts">
  ...
  import { getNewMaterial, reviewMaterial, exportNewMaterialExcel, recallMaterial, rejectMaterial } from '/@/api/technologyCenter/newmaterialReview';
  ...
  import { getTechSearchFormSchema } from '/@/views/engineering/newMateria/review/config';
  ...
  defineOptions({ name: 'technologyCenter-newMaterialReview' });
  ...
  const [Grid_Pending, gridApi_Pending] = useVbenVxeGrid({
    formOptions: {
      ...
      schema: getTechSearchFormSchema(),
      i18nConfig: {
        moduleCode: 'MaterialDevelopApplyColumn',
        transferRange: ['placeholder'],
      },
    ...
    gridOptions: {
      id: 'technologyCenter-newMaterialReview-Pendinglist',
      columns: columns,
      // toolbarConfig: {
      //   superQuery: true,
      // },
  ...
  const [Grid_Processed, gridApi_Processed] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'technologyCenter-newMaterialReview-Processedlist',
      columns: columns,
      // toolbarConfig: {
      //   superQuery: true,
      // },
