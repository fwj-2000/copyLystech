<!--  -->
<template>
  <div class="oee-compo-container">
    <div class="oee-compo-header-container flex ct-between">
      <div class="title flex ct-start">
        <p class="factory-name">{{ props.info.OrgName }}</p>
      </div>
      <div class="flex">
        <a-popover placement="right" @mouseenter="getAllInfo">
          <template #content>
            <OeePopover :baseInfo="props.info" :info="state" @goDetail="goDetail"></OeePopover>
          </template>
          <p class="oee-value" :class="{ error: !props.info.OeeState }"> {{ t('views.dashboard.operate.oee.title') }}: {{ props.info.Oee || '0.00%' }}</p>
        </a-popover>
        <img :src="chartSvg" class="zoom-icon ml-8px" @click="goPage('/dashboard/operate/oee/detail')" />
      </div>
    </div>
    <div class="oee-compo-content-container">
      <div class="compo-content-wrapper flex column item-start ct-around">
        <div class="compo-head-content flex ct-between wrap">
          <a-popover placement="right" @mouseenter="getUtilizationInfo">
            <template #content>
              <TimeUtilizationRatePopover :info="state.utilizationInfo" :rate="props.info.Moqiejiadonglv" :state="props.info.MoqiejiadonglvState">
              </TimeUtilizationRatePopover>
            </template>
            <div
              class="head-info flex column ct-between"
          ...
          <a-popover placement="right" @mouseenter="getMquserateInfo">
            <template #content>
              <PerformanceUtilizationPopover :info="state.mquserateInfo" :rate="props.info.Xingnengliyonglv" :state="props.info.XingnengliyonglvState">
            ...
            <div
              ...
              <div class="flex ct-start">
                <p :class="['value', !props.info.XingnengliyonglvState ? 'error' : '']">{{ props.info.Xingnengliyonglv || '0%' }}</p>
                <StateSvg v-if="!props.info.XingnengliyonglvState" :value="props.info.Xingnengliyonglv" style="margin-left: 4px" />
              </div>
            </div>
          </a-popover>
          <a-popover placement="right" @mouseenter="getMqyieldInfo">
            <template #content>
              <DieCuttingYieldPopover :info="state.mqyieldInfo" :rate="props.info.Moqielianglv" :state="props.info.MoqielianglvState"> </DieCuttingYieldPopover>
            </template>
            <div class="head-info flex column ct-between" @click="goDetail('dieCuttingYieldAbnormal', props.info.MoqielianglvState, props.info.Moqielianglv)">
...
<script lang="ts" setup>
  ...
  import OeePopover from './OeePopover.vue';
  ...
  const goDetail = (name, state, rate) => {
    const { timeDimension, date, week } = getSearchFormValue() as SearchFormValueType;
    const query =
  ...
  // 打开新标签页
  const goPage = path => {
    const { date = dayjs().tz().subtract(1, 'day'), timeDimension = 'day' } = getSearchFormValue() as SearchFormValueType;
    const { OrgCode: orgCode } = props.info;
    go(`${path}?orgCode=${orgCode}&dimension=${timeDimension}&date=${date.format('YYYY-MM-DD')}`);
  };
</script>
