...
export const getAnalysisInfo = info => {
  ...
  res.push([
    {
      type: 'text',
      content: `${info[0].dimesionType.map(item => item.keys).join('+')}组合`,
    },
  ]);
  // 组合标题
  res.push([
  ...
  info.forEach((item, idx) => {
    ...
    res.push([
      {
        type: 'text',
        content: `${idx + 1}）${dimesionType.map(item => item.values).join('+')}：本周/月结单产值`,
      },
      {
        type: 'value',
        content: finishedProductOutputValue && finishedProductOutputValue.toFixed(2),
        style: {
