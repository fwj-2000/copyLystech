...
<script lang="ts" setup>
  ...
  import { getStateyieldYield, downloadStateyieldMqyield } from '/@/api/dashbord/report';

  defineOptions({ name: 'dashboard-operate-processYield-stateInfoDetail' });
  ...
  const showColumns: BasicColumn[] = [
    ...
    { dataIndex: 'ProdNo' },
    { dataIndex: 'Is985' },
    { dataIndex: 'Qty', title: '工单数量(PCS)' },
    { dataIndex: 'InstockQty' },
    { dataIndex: 'Yield', title: '结单良率(%)' },
  ];
  ...
  const { ORG_CODE_OPTIONS, exportLoading, exportToExcel, registerTable } = useInfoDetailPage({
    api: getStateyieldYield,
    downloadApi: downloadStateyieldMqyield,
    showColumns,
    searchFormValue,
  });
</script>
