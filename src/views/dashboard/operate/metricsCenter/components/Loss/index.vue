      ...
      <CompoHead>
        <template #left>
          <div class="flex">
            ...
            <div class="text-hover" @click="goPageMth('/dashboard/operate/loss')">
              <span> {{ t('views.dashboard.operate.loss.title') }}</span>
          ...
          <img
            ...
            @click="
              goPage('/dashboard/operate/ranking', {
                name: EModules.MATERIAL_LOSS,
                dimension: TimeDimension.WEEK,
                date: props.queryInfo.date.subtract(7, 'day').format('YYYY-MM-DD'),
              })
            " />
          <img :src="chartSvg" class="zoom-icon mr-6px" @click="goPageMth('/dashboard/operate/loss/detail')" />
