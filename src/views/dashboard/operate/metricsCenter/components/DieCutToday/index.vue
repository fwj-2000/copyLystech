...
<template>
  <div class="dashboard-metrics-center-compo">
    <div class="compo-header">
      ...
      <CompoHead>
        <template #left>
          <div class="text-hover" @click="goPage('/dashboard/operate/dieCutting')">
            <span> {{ t('views.dashboard.operate.dieCutToday.title') }}</span>
          </div>
        </template>
        <template #right>
          <img :src="rankSvg" class="zoom-icon mr-8px"
            @click="goPage('/dashboard/operate/ranking', { name: EModules.MQACHIEVE })" />
          <img :src="chartSvg" class="zoom-icon mr-6px" @click="goPage('/dashboard/operate/dieCutting/detail')">
...
import { getMqachievedata } from '/@/api/dashbord/operate';
...
const initData = async (data, {
  ...
  const { model } = data;
  isEmptyData.value = isEmpty(model);
  // 数据为空
