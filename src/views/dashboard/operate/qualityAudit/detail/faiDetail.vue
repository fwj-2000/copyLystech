...
<script lang="ts" setup>
  ...
  import { useInfoDetailPage2 } from '/@/views/dashboard/operate/hooks/useInfoDetailPage2';
  import { getQcauditIpqcDetail } from '/@/api/dashbord/operate';

  defineOptions({ name: 'dashboard-operate-qualityAudit-faiDetail' });
  ...
  const showColumns: BasicColumn[] = [
    ...
    { dataIndex: 'AuditorNum' },
    { dataIndex: 'AuditName' },
    { dataIndex: 'SendInspectTime' },
    { dataIndex: 'ReplyTime' },
    { dataIndex: 'CheckTime' },
    { dataIndex: 'Apperanceremark' },
    { dataIndex: 'Sizeremark' },
    { dataIndex: 'Pcs' },
  ...
  const { ORG_CODE_OPTIONS, exportLoading, exportToExcel, registerTable } = useInfoDetailPage2({
    api: getQcauditIpqcDetail,
    downloadApi: getQcauditIpqcDetail,
    showColumns,
    searchFormValue,
  });
</script>
