...
<script lang="ts" setup>
  ...
  import { getDiecutkpiMetricdetail } from '/@/api/dashbord/report';
  import { saveTableDatasToExcel } from '/@/utils/file/download';

  import { allColumnsOptions, monthColumnsOptions, weekColumnsOptions, EColumnsType, sortedList } from './config';

  import { SearchForm } from '/@/views/dashboard/operate/components/SearchForm/index';
  import { SpinContent } from '/@/views/dashboard/operate/components/styleItem/index';
  import { TimeDimension } from '../../types';
  import { cloneDeep, isArray, isEmpty, merge, uniq } from 'lodash-es';

  defineOptions({ name: 'dashboard-operate-mqkpi-detail' });
  ...
  const getTableData = (queryParams, searchFormValue) => {
    console.log('searchFormValue?.orgCode: ', searchFormValue?.orgCode);
    let orgCodes = searchFormValue?.orgCode;
    if (isArray(orgCodes)) {
      orgCodes = searchFormValue?.orgCode.join(';');
    }
    return new Promise((resolve, reject) => {
      if (isEmpty(orgCodes)) {
        return resolve({});
      }
      getDiecutkpiMetricdetail({
        orgCode: orgCodes,
        ...
        .then(res => {
          ...
          dataSource.value = dataList
            ...
            .sort((a, b) => {
              const indexA = sortedList.findIndex(item => item === a.factory);
              const indexB = sortedList.findIndex(item => item === b.factory);
  ...
  const getColumns = (data: any): BasicColumn[] => {
    ...
    return columns
      ...
      .reduce((pre, cur) => {
        const { type = EColumnsType.NORAML, isRowSpan = false, sortable = false, filterable = false, baseInfo, dataIndex } = cur;
  ...
  // 获取筛选选项信息
  function getFilterOptions(data: any, dataIndex: string) {
