...
<script lang="ts" setup>
  ...
  import { getFinanceeconomicOrdertype, getWorkorderloss38trend } from '/@/api/dashbord/operate';
  import { IS_INCLUDE_OPTIONS, DEFAULT_IS_INCLUDE } from '/@/views/dashboard/operate/metricsCenter/components/ThreePointEightLoss/config';
  ...
  defineOptions({ name: 'dashboard-operate-threePointEightLoss-detail' });
  ...
  const { searchFormValue, chartInfoList, loading, isRequestError, isEmptyData } = useTrendPage({
    ...
    pathInfo: {
      // path: '/dashboard/operate/threePointEightLoss/detail', // 趋势页路由
      // infoPath: '/dashboard/operate/ranking', // 详情页路由
    },
    chartQuotaInfoList,
    singleChartQuotaInfoList,
    requestMth: async params => {
      let res = await getWorkorderloss38trend(params);
      console.log(res, res.data);

      res.data.list = res.data.list.map(item => {
        item.lossRate = item.lossRate * 100;
        return item;
      });
      return res;
    },
