  ...
  <MetricItem>
    <template #title>
      <div class="whitespace-nowrap">
        <span class="font-bold text-hover" @click="goPath('/dashboard/customerService/project', { projectName: extraFormValue.projectName })">项目</span>
        <span class="ml-[8px]">
          <a-select v-model:value="extraFormValue.projectName" v-bind="selectAttrs" style="min-width: 164px">
            <a-select-option v-for="(option, oidx) in projectNameOptions" :key="oidx" :value="option.value" :title="option.text">
              {{ option.text }}
            </a-select-option>
          </a-select>
        </span>
...
<script lang="ts" setup>
  ...
  import { getfcprojectlist, getprojectbind } from '/@/api/dashbord/fc';
  ...
  const projectNameOptions = ref<any[]>([]);
  ...
  const [register, { goPath, fetchData, getCommonParams }] = useMetricContent({
    ...
    getParams: commonParams => {
      return {
        ...commonParams,
        // 其他参数
        projectName: (unref(extraFormValue).projectName ?? []).join(';'),
      };
    },
    beforeFetch: () => {
      // 请求前清除值
    ...
    afterFetch: resData => {
      ...
      charts.value = dataInfo.map(data => {
        ...
        const maxTextLen = 10;
        const categoryName = data.ProjectName.length < maxTextLen ? data.ProjectName : `${data.ProjectName.slice(0, maxTextLen)}...`;
        return {
          categoryName,
  ...
  async function fetchMth(...args) {
    ...
    console.log('getCommonParams: ', getCommonParams());
    if (!isEqual(params, lastFetchProductCategory)) {
      lastFetchProductCategory = cloneDeep(params);
      await getprojectbind(params).then(res => {
        const resData = res.data as any;
        projectNameOptions.value = resData.map(item => ({
          value: item.value,
          text: item.value,
        }));
        extraFormValue.projectName = resData.slice(0, 3).map(item => item.value);
        requestParams[0] = {
          ...requestParams[0],
          projectName: (unref(extraFormValue).projectName ?? []).join(';'),
        };
    ...
    return getfcprojectlist(...requestParams);
  }
