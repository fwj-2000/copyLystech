...
<template>
  <div class="relative w-[100%] h-[100%]">
    ...
    <div class="w-[100%] h-[calc(100%-32px)]">
      ...
      <template v-else>
        <div class="absolute top-0 left-[5%] w-[90%] h-[90%]" style="z-index: 0">
          <highcharts ref="highChart" :options="pieOptions" style="height: 207px; width: 100%" />
        </div>
        <div class="absolute bottom-0 z-2 w-[100%] flex flex-col text-[14px]">
          <div v-for="(item, idx) in props.data" :key="idx" class="w-[80%] flex justify-between mb-[12px] text-hover" @click="showDetailModal(item.text)">
            <div class="flex">
...
<script lang="ts" setup>
  ...
  const isNotEmpty = computed(() => {
    const res = props.data.some(item => item.value !== 0);
    return res;
  });

  const showDetailModal = text => {
    emits('click-legend', text);
  };
</script>
