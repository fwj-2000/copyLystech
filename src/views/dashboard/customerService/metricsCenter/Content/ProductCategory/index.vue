  ...
  <MetricItem>
    <template #title>
      <div class="whitespace-nowrap">
        <span class="font-bold text-hover" @click="goPath('/dashboard/customerService/productCategory', { productCategory: extraFormValue.productCategory })"
          >产品类别</span
        >
        <span class="ml-[8px]">
          <a-select v-model:value="extraFormValue.productCategory" v-bind="selectAttrs" style="min-width: 164px">
            <a-select-option v-for="(option, oidx) in productCategoryOptions" :key="oidx" :value="option.value" :title="option.text">
              {{ option.text }}
            </a-select-option>
          </a-select>
        </span>
            ...
            <div class="w-[100%] h-[100%] flex">
              <div class="w-[50%] h-[100%] p-[12px]">
                <div class="w-[100%] h-[34px] flex justify-between items-start">
                  <span>{{ item.categoryName }}-金额</span>
              ...
              <div class="w-[50%] h-[100%] p-[12px]">
                <div class="w-[100%] h-[34px] flex justify-between items-start">
                  <span>{{ item.categoryName }}-数量</span>
...
<script lang="ts" setup>
  ...
  import { getfcproductcategorylist, getproductcategorybind } from '/@/api/dashbord/fc';
  ...
  import { ETableStatus } from '/@/views/dashboard/customerService/metricsCenter/types';
  ...
  const [register, { goPath, fetchData, getCommonParams }] = useMetricContent({
    ...
    getParams: commonParams => {
      const { productCategory } = extraFormValue;
      return {
        ...commonParams,
        // 其他参数
        productCategory: (unref(productCategory) ?? []).join(';'),
      };
    },
    beforeFetch: () => {
      // 请求前清除值
    ...
    afterFetch: resData => {
      ...
      charts.value = dataInfo.map(data => {
        ...
        const maxTextLen = 10;
        const categoryName = data.CategoryName.length < maxTextLen ? data.CategoryName : `${data.CategoryName.slice(0, maxTextLen)}...`;
        return {
          categoryName,
  ...
  async function fetchMth(...args) {
    ...
    const params = pick(
      ...
      ['dimension', 'timeIndex', 'customerPerson', 'yearIndex', 'status', 'batchNumber'],
    );
    if (!isEqual(params, lastFetchProductCategory)) {
      extraFormValue.productCategory = [];
      lastFetchProductCategory = cloneDeep(params);
      await getproductcategorybind(params).then(res => {
        const resData = res.data as any;
        productCategoryOptions.value = resData.map(item => ({
          value: item.value,
          text: item.value,
        }));
        extraFormValue.productCategory = resData.slice(0, 3).map(item => item.value);
        requestParams[0] = {
          ...requestParams[0],
          productCategory: (unref(extraFormValue).productCategory ?? []).join(';'),
        };
      });
    }
    return getfcproductcategorylist(...requestParams);
  }
