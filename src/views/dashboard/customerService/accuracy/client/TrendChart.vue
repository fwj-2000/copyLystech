...
<script lang="ts" setup>
  ...
  import { getfcaccuracydetailclientgraphiclist } from '/@/api/dashbord/fc';
  import { removeWeekMonthYear, suffixMap } from '/@/views/dashboard/customerService/utils';
  ...
  defineOptions({ name: 'dashboard-customerService-accuracy-client' });
  ...
  const [register] = useMetricContent({
    ...
    fetchApi: getfcaccuracydetailclientgraphiclist,
    afterFetch: resData => {
      const dataInfo = removeWeekMonthYear(resData);
      const clientPerson = dataInfo?.ClientPerson ?? [];
      if (isEmpty(clientPerson)) {
        throw new Error('clientPerson is Empty');
      }
      const currentNum = dataInfo?.CurrentNum ?? 1;
      const lastNum = dataInfo?.LastNum ?? 0;
      const orderList = clientPerson.toSorted((a, b) => {
        const order = Number.parseFloat(b?.ChangeRate ?? 0) - Number.parseFloat(a?.ChangeRate ?? 0);
        return order;
      });
      ...
      const barData1 = orderList.map(item => {
        return Number.parseInt(item.CurrentClientQuantity, 10);
      });
      const barData2 = orderList.map(item => {
        return Number.parseInt(item.LastClientQuantity, 10);
      });
      ...
      options.value = merge(cloneDeep(trendChartOptions), {
        ...
        series: [
          ...
          },
          {
            type: 'bar',
            yAxisIndex: 0,
            name: `${lastNum}${suffixMap[dimension]}(预测)`,
            label: {
              show: true,
              position: 'top',
              distance: 2,
          ...
          },
          {
            type: 'bar',
            yAxisIndex: 0,
            name: `${currentNum}${suffixMap[dimension]}(当前)`,
            label: {
              show: true,
              position: 'top',
              distance: 2,
