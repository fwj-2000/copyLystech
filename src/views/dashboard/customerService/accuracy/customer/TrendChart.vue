...
<script lang="ts" setup>
  ...
  import { getfcaccuracydetailcustomerservicegraphiclist } from '/@/api/dashbord/fc';
  ...
  const [register] = useMetricContent({
    ...
    fetchApi: getfcaccuracydetailcustomerservicegraphiclist,
    afterFetch: resData => {
      const dataInfo = removeWeekMonthYear(resData);
      const customerServicePerson = dataInfo?.CustomerServicePerson ?? [];
      if (isEmpty(customerServicePerson)) {
        throw new Error('customerServicePerson is Empty');
      }
      const currentNum = dataInfo?.CurrentNum ?? 1;
      const orderList = customerServicePerson.toSorted((a, b) => {
        const order = Number.parseFloat(b?.ChangeRate ?? 0) - Number.parseFloat(a?.ChangeRate ?? 0);
        return order;
      });
      const xAxisData = orderList.map(item => {
        return item.CustomerServiceName;
      });
      const lineData = orderList.map(item => {
        return item.ChangeRate;
      });
      const barData1 = orderList.map(item => {
        return Number.parseInt(item.LastCustomerServiceQuantity, 10);
      });
      const barData2 = orderList.map(item => {
        return Number.parseInt(item.CurrentCustomerServiceQuantity, 10);
      });
      ...
      options.value = merge(cloneDeep(trendChartOptions), {
        ...
        series: [
          ...
          {
            type: 'bar',
            name: `${currentNum}${suffixMap[dimension]}(预测)`,
            yAxisIndex: 0,
            label: {
              show: true,
              position: 'top',
