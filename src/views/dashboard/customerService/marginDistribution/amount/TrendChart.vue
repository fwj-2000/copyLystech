...
<template>
  <div class="w-[100%] h-[100%] bg-[#fff] fc-shadow flex flex-col justify-start items-start">
    <MetricItem>
      <template #title>
        <span class="font-bold">厂区-金额</span>
      </template>
...
<script lang="ts" setup>
  ...
  const [register] = useMetricContent({
    ...
    afterFetch: resData => {
      ...
      const amountList = Object.values(dataInfo.FactoryPrice) as any;
      ...
      const lineData = amountList.map(item => {
        return Number.parseFloat(item.ChangeRateCount);
      });
      const barData1 = amountList.map(item => {
        return item.LastPriceCount;
      });
      const barData2 = amountList.map(item => {
        return item.CurrentPriceCount;
      });
