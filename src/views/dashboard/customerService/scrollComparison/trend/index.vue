    ...
    <div class="dashboard-fc-page-container">
      <div class="dashboard-fc-index-container">
        ...
        <div class="w-[100%] h-[100%] flex-shrink-1">
          <div class="w-[100%] h-[264px] mx-[6px] p-2">
...
<script lang="ts" setup>
  ...
  import { getfcrollvsdetailfourlist } from '/@/api/dashbord/fc';
  ...
  provide('getExportFilename', () => {
    ...
    const filename = `四周FC量-${metricName}`;
    return filename;
  });
  const [registerTable = register] = useBaseVxeTable({
    ...
    api: getfcrollvsdetailfourlist,
    afterQuery: (res: any) => {
      const { list } = (res.data as any) ?? {};
      const { CurrentWeek: currentWeek = 0 } = list?.[0] ?? [];
