      ...
      <div class="dashboard-fc-index-container">
        ...
        <SearchForm @register="register">
          <template #right>
            <div class="flex">
              <!-- 文本 -->
              <div class="flex pl-6px pr-9px" style="height: 32px; color: #1a1a1a; background-color: #e6e6e6">产品类别</div>
              <!-- 表单 -->
              <div class="ml-[-3px]">
                <a-select
                  v-model:value="searchFormValue.productCategory"
                  v-bind="{
                    ...commonMultiSelectAttrs,
                    dropdownMatchSelectWidth: true,
                  }">
                  <a-select-option v-for="(option, oidx) in productCategoryOptions" :key="oidx" :value="option.value" :title="option.label">
                    {{ option.label }}
                  </a-select-option>
...
<script lang="ts" setup>
  ...
  import { getfcproductcategorydetaillist, getproductcategorybind } from '/@/api/dashbord/fc';
  ...
  defineOptions({ name: 'dashboard-customerService-productCategory' });
  ...
  provide('getExportFilename', () => {
    ...
    const filename = `产品类别-${metricName}`;
    return filename;
  });
  ...
  const [registerTable = register] = useBaseVxeTable({
    ...
    getParams: () => {
      return {
        productCategory: searchFormValue.value.productCategory?.join(';'),
  ...
  async function getProjectOptions() {
    ...
    const { data = [] } = await getproductcategorybind(params);
    const filters = data.map(item => ({
      value: item.id,
      label: item.value,
    }));
    isFetchProjectOptions.value = false;
    productCategoryOptions.value = filters;
  }
</script>
