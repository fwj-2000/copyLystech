...
import { flattenColumns, formatDataList, getFormatMonthKeysValuesColumns, getFormatWeekKeysValuesColumns, getMergeRow } from './utils';
...
export function useBaseVxeTable(props: IProps): [(action: IAction) => void, IHookAction] {
  ...
  const getColumns = ({ data, columns }: { data: any; columns: BaseVxeTableTypes.Columns }) => {
    const allColumns: BaseVxeTableTypes.Columns = columns.reduce((pre, cur) => {
      ...
      if (columnType === EColumnType.DATA_LIST) {
        ...
        const columns = firstList.map((item, idx) => {
          return {
            ...cur,
            field: `${fieldStr}_${idx}_${item.keys}`,
            title: item.keys,
            columnType: EColumnType.VALUE,
          };
        });
  ...
  const register = (action: IAction) => {
    ...
    if (isFunction(getExportParams)) {
      state.getExportParams = getExportParams;
    }
  ...
  function setFooterData(res) {
    ...
    const flatColumns = flattenColumns(columns.value);
  ...
  return [
    register,
    {
      reloadData: params => instance?.value?.reloadData(params),
