          ...
          <div v-for="(item, idx) in allOptions" :key="idx" class="flex">
            ...
            <a-form-item name="isBian">
              <a-select
                ...
                :style="`${item.width ? `width: ${item.width}px;text-align: left` : ''}`"
...
<script lang="ts" setup>
  ...
  import { getFctrend } from '/@/api/dashbord/report';
  ...
  defineOptions({ name: 'dashboard-operation-salesRate-fcTrend' });
  ...
  // 使用维度搜索表格hooks
  const { loading, searchFormValue, getDateDimParams, registerTable, columns, dataSource } = useTableSearch({
    tableRef,
    columnsOptions,
  ...
  setAllOptions({ prodline: searchFormValue.prodline, period: searchFormValue.newOldProject });
  // 监听产品线--改变项目下拉项
