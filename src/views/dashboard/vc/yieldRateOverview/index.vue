            ...
            <div class="left-chart-box">
              <template v-for="(item, index) in singleOptions" :key="index">
                <div class="chart-box h-148px">
                  <Chart ref="leftChartRefs" :options="item" height="134px" style="height: 100%; width: 100%" />
                </div>
              </template>
            </div>
...
<script setup lang="ts">
  ...
  import { getCenterLineOptions, getLeftSingleChart, getRightBarChart } from '/@/views/dashboard/vc/yieldRateOverview/config';
  import { manmachineratioHeatdissitrend } from '/@/api/dashbord/operate';
  ...
  defineOptions({ name: 'dashboard-vc-yieldRateOverview' });
  ...
  function init() {
    manmachineratioHeatdissitrend(api.getFormParams()).then(async ({ code, data }) => {
      if (code === 200) {
        await nextTick();
  ...
  function initCenterLineChart(data) {
    return data.map(item => {
      ...
      item.tarGetChart.forEach(v => {
        ...
        values.push(v.rate === '0' ? v.rate : (v.rate * 100).toFixed(1)); // '0'为无良率
      });
      // 计算最小整十数
  ...
  function initRightBarChart(data) {
    // console.log('🚀 ~ initRightBarChart ~ data: ', data);
    return data.map(item => {
    ...
    // return merge({}, getRightBarChart(), {
    // 	xAxis: {
    // 		data: ['压伤', '划伤', '侧漏不良', '性能不良', '破真空', '变形', '其他'],
    // 	},
    // 	series: [
    // 		{
    // 			data: [380, 510, 680, 600, 500, 860, 850],
    // 		},
    // 		{
    // 			data: [30, 40, 50, 55, 40, 30, 20],
    // 		},
    // 	],
    // });
  }
