        ...
        <Grid>
          <template #toolbar-actions>
            <a-space>
              ...
              <a-button @click="handleExport">{{ t('common.downloadTemplate') }}</a-button>
...
<script lang="ts" setup>
  defineOptions({ name: 'dashboard-vc-basePage' });
  ...
  import { getBasePage, basedel, downloadTemplate, importBase } from '/@/api/dashbord/vc';
  ...
  const handleExport = async () => {
    const res = await downloadTemplate({ fileType: 'base' });
    if (res.code === 200) {
