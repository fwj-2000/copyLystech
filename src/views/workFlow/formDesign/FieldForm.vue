<template>
  <lydc-group-title content="字段设置" :bordered="false" />
  <a-table :data-source="drawingList" :columns="columns" size="small" :pagination="false" rowKey="id" class="search-table">
    <template #bodyCell="{ column, record, index }">
      ...
      <template v-if="column.key === 'lydcKey'">
        <lydc-select v-model:value="record.lydcKey" :options="componentList" :fieldNames="{ options: 'children' }" showSearch allowClear />
      </template>
...
<script lang="ts" setup>
  ...
  const columns = [
    ...
    { title: '控件类型', dataIndex: 'lydcKey', key: 'lydcKey', width: 150 },
  ...
  function getComponentList() {
    const realInputComponents = inputComponents
      .filter(o => !ignoreList.includes(o.__config__.lydcKey))
      .map(o => ({ id: o.__config__.lydcKey, fullName: o.__config__.label }));
    const realSelectComponents = selectComponents
      .filter(o => !ignoreList.includes(o.__config__.lydcKey))
      .map(o => ({ id: o.__config__.lydcKey, fullName: o.__config__.label }));
    const realSystemComponents = systemComponents.map(o => ({ id: o.__config__.lydcKey, fullName: o.__config__.label }));
    componentList.value = [
      {
        id: '1',
        fullName: '基础控件',
  ...
  function handleAdd() {
    let item = { filedName: '', filedId: '', lydcKey: '', required: false, multiple: false };
