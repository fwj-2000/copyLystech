  ...
  function fillFormData(form, data, isAdd) {
    ...
    const loop = (list, parent?) => {
      for (let i = 0; i < list.length; i++) {
        ...
        if (item.__vModel__) {
          ...
          if (isAdd || item.__config__.isSubTable) {
            if (item.__config__.defaultCurrent) {
              if (item.__config__.lydcKey === 'datePicker') {
                item.__config__.defaultValue = dayjs(currDate).tz().startOf(getDateTimeUnit(item.format)).valueOf();
              }
              if (item.__config__.lydcKey === 'timePicker') {
                item.__config__.defaultValue = dayjs(currDate)
                  .tz()
                  .format(item.format || 'HH:mm:ss');
              }
              if (item.__config__.lydcKey === 'organizeSelect' && userInfo.organizeIdList?.length) {
                item.__config__.defaultValue = item.multiple ? [userInfo.organizeIdList] : userInfo.organizeIdList;
              }
              if (item.__config__.lydcKey === 'depSelect' && userInfo.departmentId) {
                item.__config__.defaultValue = item.multiple ? [userInfo.departmentId] : userInfo.departmentId;
              }
              if (item.__config__.lydcKey === 'userSelect' && userInfo.userId) {
                item.__config__.defaultValue = item.multiple ? [userInfo.userId] : userInfo.userId;
              }
              if (item.__config__.lydcKey === 'usersSelect' && userInfo.userId) {
                item.__config__.defaultValue = item.multiple ? [userInfo.userId + '--user'] : userInfo.userId + '--user';
              }
              if (item.__config__.lydcKey === 'posSelect' && userInfo.positionIds?.length) {
                item.__config__.defaultValue = item.multiple ? userInfo.positionIds.map(o => o.id) : userInfo.positionIds[0].id;
              }
              if (item.__config__.lydcKey === 'roleSelect' && userInfo.roleIds?.length) {
                item.__config__.defaultValue = item.multiple ? userInfo.roleIds : userInfo.roleIds[0];
              }
              if (item.__config__.lydcKey === 'groupSelect' && userInfo.groupIds?.length) {
                item.__config__.defaultValue = item.multiple ? userInfo.groupIds : userInfo.groupIds[0];
              }
              if (item.__config__.lydcKey === 'sign' && userInfo.signImg) {
        ...
        if (['relationFormAttr', 'popupAttr'].includes(item.__config__.lydcKey) && !item.isStorage && state.config.disabled) {
          item.disabled = true;
        }
