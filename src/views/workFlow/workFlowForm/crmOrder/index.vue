      ...
      <a-row>
        ...
        <a-col :span="6" v-if="judgeShow('salesmanId')">
          <a-form-item label="业务人员" name="salesmanId">
            <lydc-user-select v-model:value="dataForm.salesmanId" placeholder="选择人员" @change="onSalesmanIdChange" :disabled="judgeWrite('salesmanId')" />
          </a-form-item>
        </a-col>
        <a-col :span="6" v-if="judgeShow('orderDate')">
          <a-form-item label="订单日期" name="orderDate">
            <lydc-date-picker v-model:value="dataForm.orderDate" format="YYYY-MM-DD HH:mm:ss" placeholder="选择日期" :disabled="judgeWrite('orderDate')" />
          </a-form-item>
        </a-col>
        ...
        <a-col :span="6" v-if="judgeShow('paymentMode')">
          <a-form-item label="付款方式" name="paymentMode">
            <lydc-select v-model:value="dataForm.paymentMode" :options="paymentOptions" placeholder="选择付款方式" :disabled="judgeWrite('paymentMode')" />
          </a-form-item>
        </a-col>
        ...
        <a-col :span="6" v-if="judgeShow('deliveryDate')">
          <a-form-item label="发货日期" name="deliveryDate">
            <lydc-date-picker v-model:value="dataForm.deliveryDate" placeholder="选择日期" :disabled="judgeWrite('deliveryDate')" />
          </a-form-item>
        </a-col>
        ...
        <a-col :span="24" v-if="judgeShow('fileJson')">
          <a-form-item label="相关附件" name="fileJson">
            <lydc-upload-file v-model:value="fileList" type="workFlow" :disabled="judgeWrite('fileJson')" />
          </a-form-item>
        </a-col>
        <a-col :span="24" v-if="judgeShow('description')">
          <a-form-item label="订单备注" name="description">
            <lydc-textarea v-model:value="dataForm.description" placeholder="订单备注" :disabled="judgeWrite('description')" />
          </a-form-item>
        </a-col>
      </a-row>
    </a-form>
...
<script lang="ts" setup>
  ...
  import { toDecimal } from '/@/utils/lydc';
  ...
  const getGoodsColumnSum = computed(() => {
    ...
    unref(getGoodsColumns).forEach((column, index) => {
      ...
      let sumVal = state.dataForm.goodsList.reduce((sum, d) => sum + Number.parseFloat(d[column.key]), 0);
  ...
  const getPlanColumnSum = computed(() => {
    ...
    unref(getPlanColumns).forEach((column, index) => {
      ...
      let sumVal = state.dataForm.collectionPlanList.reduce((sum, d) => sum + Number.parseFloat(d[column.key]), 0);
      sumVal = Number.isNaN(sumVal) ? '' : sumVal;
      sums[index] = sumVal && !Number.isInteger(sumVal) ? sumVal.toFixed(2) : sumVal;
    });
    return sums;
  ...
  watch(
    ...
    val => {
      ...
      for (let i = 0; i < val.length; i++) {
        const e = val[i];
        money += Number.parseFloat(e.actualAmount);
      }
  ...
  function count(record) {
    //金额 = 数量*单价
    record.amount = toDecimal(Number.parseFloat(record.price) * Number.parseFloat(record.qty));
    //折扣价 = (单价*折扣)
    ...
    //实际金额
    record.actualAmount = toDecimal(Number.parseFloat(record.actualPrice) * Number.parseFloat(record.qty));
  }
