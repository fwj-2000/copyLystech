      ...
      <a-row>
        ...
        <a-col :span="12" v-if="judgeShow('salesDate')">
          <a-form-item label="开单日期" name="salesDate">
            <lydc-date-picker v-model:value="dataForm.salesDate" format="YYYY-MM-DD HH:mm:ss" placeholder="选择日期" :disabled="judgeWrite('salesDate')" />
          </a-form-item>
        </a-col>
        ...
        <a-col :span="24" v-if="judgeShow('description')">
          <a-form-item label="开单备注" name="description">
            <lydc-textarea v-model:value="dataForm.description" placeholder="开单备注" :disabled="judgeWrite('description')" />
          </a-form-item>
        </a-col>
      </a-row>
    </a-form>
    <template v-if="judgeShow('entryList')">
      <LydcGroupTitle content="销售明细" :bordered="false" />
