    ...
    <a-form
      ...
      <template v-if="eventType === 'freeApprover'">
        <a-form-item label="加签人员" name="freeApproverUserId" :rules="[{ required: true, message: `必填`, trigger: 'change' }]">
          <lydc-user-select v-model:value="dataForm.freeApproverUserId" placeholder="请选择" allowClear />
        </a-form-item>
      ...
      <template v-if="eventType === 'audit' || (eventType === 'freeApprover' && dataForm.freeApproverType === 2)">
        <a-form-item
          ...
          <lydc-select v-model:value="dataForm.branchList" multiple placeholder="请选择" allowClear @change="onBranchChange" :options="branchList" showSearch />
        </a-form-item>
        <a-form-item
          ...
          <lydc-user-select v-model:value="item.value" multiple :placeholder="'请选择' + item.label" modalTitle="候选人员" v-else />
        </a-form-item>
      </template>
      ...
      <a-form-item label="抄送人员" name="copyIds" v-if="eventType !== 'freeApprover' && properties.isCustomCopy">
        <lydc-user-select v-model:value="dataForm.copyIds" multiple allowClear />
      </a-form-item>
      <template v-if="properties.hasOpinion">
        <a-form-item :label="`${label}意见`" name="handleOpinion">
          <lydc-textarea v-model:value="dataForm.handleOpinion" placeholder="请输入" />
          <CommonWordsPopover ref="commonWordsPopoverRef" @confirm="insertOpinion" />
        </a-form-item>
        <a-form-item :label="`${label}附件`" name="fileList">
          <lydc-upload-file v-model:value="dataForm.fileList" type="workFlow" :limit="3" />
        </a-form-item>
      </template>
      <a-form-item
        ...
        <lydc-sign v-model:value="dataForm.signImg" />
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
