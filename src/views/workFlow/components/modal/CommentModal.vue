  ...
  <BasicModal v-bind="$attrs" @register="registerModal" title="流程评论" @ok="handleSubmit" destroyOnClose>
    <a-form :colon="false" :model="dataForm" :rules="dataRule" ref="formElRef">
      <a-form-item name="text">
        <lydc-textarea v-model:value="dataForm.text" placeholder="请输入评论" />
      </a-form-item>
      <a-form-item>
        <a-upload
          v-feature
      ...
      <a-form-item>
        <lydc-upload-file ref="uploadFileRef" v-model:value="dataForm.file" type="workFlow" :limit="2" detailed class="comment-upload-file" />
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
