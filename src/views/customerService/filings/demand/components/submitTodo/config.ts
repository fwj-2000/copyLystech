...
import { statusOptions, STATUS_ENUM } from '../../config';

export { STATUS_ENUM } from '../../config';

const todoStatusEnumList = [STATUS_ENUM.待提交, STATUS_ENUM.撤回, STATUS_ENUM.终止];
const todoStatusOptions = statusOptions.filter(item => todoStatusEnumList.includes(item.id));
...
export function getFormConfig() {
  return [
    ...
    },
    {
      ...
      label: '',
      component: 'Select',
      componentProps: { placeholder: '状态', options: todoStatusOptions, fieldNames: { label: 'fullName', value: 'id' }, allowClear: true },
    },
  ];
}
