...
export const vxeTableColumns: VxeGridPropTypes.Columns = [
  ...
  // 审核结论
  {
    ...
    slots: {
      default: ({ row }) => {
        if (!row.auditResult) return '';
        const { theme, fullName } = STATUS_OPTIONS.find(item => item.id === row.auditResult) || {};
        return h(LydcTag, { theme: theme }, () => row.auditResultName || fullName || row.auditResult);
      },
    },
  },
  // 状态
