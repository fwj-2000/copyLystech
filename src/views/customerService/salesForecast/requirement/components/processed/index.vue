...
<script lang="ts" setup>
  ...
  import { getSalesForecast, withdrawSalesForecast, exportSalesForecast, getNodeList } from '/@/api/business/salesForecast';
  ...
  const [VxeBasicTable, tableRef] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'customerService-salesForecast-requirement-processed-list',
      mouseConfig: {
        area: false, // 是否开启区域选取
        extension: false,
      },
      ...
      // @ts-ignore
      api: (params: any) => getSalesForecast(MODULE_TYPE_ENUM.待产需求, { ...params, status: STATUS_ENUM.已处理 }),
      toolbarConfig: {
        export: false,
        print: false,
        buttons: [
              ...
              events: {
                click: () => {
                  openExportModal(true, {
                    listQuery: {},
                    api: (query: any) => exportSalesForecast(MODULE_TYPE_ENUM.待产需求, { ...query, status: STATUS_ENUM.已处理 }),
