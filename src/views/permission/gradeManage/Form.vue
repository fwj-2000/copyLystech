  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" :width="900" @ok="handleSubmit" class="grade-modal">
    <a-form :colon="false" :labelCol="{ style: { width: '100px' } }" :model="dataForm" :rules="dataRule" ref="formElRef">
      ...
      <a-form-item label="管理员" name="userId">
        <lydc-user-select v-model:value="dataForm.userId" placeholder="请选择" @change="onChange" :disabled="!!dataForm.id" v-if="getIsAdmin" />
    ...
    <a-table
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'thisLayer'">
          <LydcCheckboxSingle label="查看" checked disabled v-if="record.thisLayerSelect === 2" />
          <LydcCheckboxSingle label="查看" disabled v-if="record.thisLayerSelect === 3" />
          <template v-if="record.thisLayerSelect === 0 || record.thisLayerSelect === 1">
            <LydcCheckboxSingle label="查看" v-model:value="record.thisLayerSelect" @change="onThisLayerSelectChange($event, record)" />
          </template>
          <LydcCheckboxSingle label="添加" checked disabled v-if="record.thisLayerAdd === 2" />
          <LydcCheckboxSingle label="添加" disabled v-if="record.thisLayerAdd === 3" />
          <template v-if="record.thisLayerAdd === 0 || record.thisLayerAdd === 1">
            <LydcCheckboxSingle label="添加" v-model:value="record.thisLayerAdd" :disabled="!record.thisLayerSelect" />
          </template>
          <LydcCheckboxSingle label="编辑" checked disabled v-if="record.thisLayerEdit === 2" />
          <LydcCheckboxSingle label="编辑" disabled v-if="record.thisLayerEdit === 3" />
          <template v-if="record.thisLayerEdit === 0 || record.thisLayerEdit === 1">
            <LydcCheckboxSingle label="编辑" v-model:value="record.thisLayerEdit" :disabled="!record.thisLayerSelect" />
          </template>
          <LydcCheckboxSingle label="删除" checked disabled v-if="record.thisLayerDelete === 2" />
          <LydcCheckboxSingle label="删除" disabled v-if="record.thisLayerDelete === 3" />
          <template v-if="record.thisLayerDelete === 0 || record.thisLayerDelete === 1">
            <LydcCheckboxSingle label="删除" v-model:value="record.thisLayerDelete" :disabled="!record.thisLayerSelect" />
          </template>
        </template>
        <template v-if="column.key === 'subLayer'">
          <LydcCheckboxSingle label="查看" checked disabled v-if="record.subLayerSelect === 2" />
          <LydcCheckboxSingle label="查看" disabled v-if="record.subLayerSelect === 3" />
          <template v-if="record.subLayerSelect === 0 || record.subLayerSelect === 1">
            <LydcCheckboxSingle label="查看" v-model:value="record.subLayerSelect" @change="onSubLayerSelectChange($event, record)" />
          </template>
          <LydcCheckboxSingle label="添加" checked disabled v-if="record.subLayerAdd === 2" />
          <LydcCheckboxSingle label="添加" disabled v-if="record.subLayerAdd === 3" />
          <template v-if="record.subLayerAdd === 0 || record.subLayerAdd === 1">
            <LydcCheckboxSingle label="添加" v-model:value="record.subLayerAdd" :disabled="!record.subLayerSelect" />
          </template>
          <LydcCheckboxSingle label="编辑" checked disabled v-if="record.subLayerEdit === 2" />
          <LydcCheckboxSingle label="编辑" disabled v-if="record.subLayerEdit === 3" />
          <template v-if="record.subLayerEdit === 0 || record.subLayerEdit === 1">
            <LydcCheckboxSingle label="编辑" v-model:value="record.subLayerEdit" :disabled="!record.subLayerSelect" />
          </template>
          <LydcCheckboxSingle label="删除" checked disabled v-if="record.subLayerDelete === 2" />
          <LydcCheckboxSingle label="删除" disabled v-if="record.subLayerDelete === 3" />
          <template v-if="record.subLayerDelete === 0 || record.subLayerDelete === 1">
            <LydcCheckboxSingle label="删除" v-model:value="record.subLayerDelete" :disabled="!record.subLayerSelect" />
          </template>
        </template>
      </template>
    </a-table>
