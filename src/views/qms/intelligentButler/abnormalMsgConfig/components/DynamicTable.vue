...
<script setup lang="ts">
  ...
  function addColumn(line) {
    const templates = Array.from({ length: line }, () => cloneDeep(state.template));
  ...
  function initFn({ rowTemplate, title, columns, editRules, data = [], mode }: IinitState): void {
    state.title = title;
    ...
    state.template = rowTemplate || getRowTemplate(columns);
