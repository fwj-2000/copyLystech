...
<script setup lang="ts">
  ...
  import { getPurchaseReceiptOrder, getFactoryList } from '/@/api/qms/iqc';
  ...
  const [Grid, gridApi] = useVbenVxeGrid({
    gridOptions: {
      api: params => getPurchaseReceiptOrder({ ...params, ...getFieldsValue(), status: 2, factoryArea: state.factoryArea }),
      columns: SAPColumns,
      virtualXConfig: {
        enabled: false,
      },
