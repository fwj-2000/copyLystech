      ...
      <a-table :data-source="list" :columns="columns" size="small" :pagination="false" :scroll="{ x: 'max-content', y: '440px' }" class="import-preview-table">
        <template #bodyCell="{ column, text, record, index }">
          ...
          <template v-else-if="column.dataIndex === 'status'">
            <div v-if="text == 2">
              <Badge color="#f50" text="启用" />
            </div>
            <div v-if="text == 1">
              <Badge color="#87d068" text="停用" />
            </div>
          </template>
...
<script lang="ts" setup>
  ...
  import { downloadBaseDataLaborrateImportTemplate, saveBaseDataLaborrate } from '/@/api/purchase/rateOfWork';
  ...
  const getAction = computed(() => globSetting.apiUrl + '/api/basedata/laborrate/import');
  ...
  function handleNext() {
    if (state.activeStep == 1) {
      ...
      saveBaseDataLaborrate(state.BatchCode)
        .then(res => {
          const { data, msg } = res;
          state.result = data;
          state.btnLoading = false;
  ...
  function handleTemplateDownload() {
    downloadBaseDataLaborrateImportTemplate().then(res => {
