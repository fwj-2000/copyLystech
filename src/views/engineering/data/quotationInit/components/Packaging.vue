      ...
      <div class="lydc-content-wrapper-content bg-white">
        <div>
          <div style="display: flex; flex-direction: row; justify-content: space-between">
            <a-space>
              <Subtitle :title="props.title" id="packaging" />
              <div v-if="hasBtnP('price-packaging')" class="total"
...
<script lang="ts" setup>
  ...
  const calcValues = index => {
    nextTick(() => {
      ...
      const packQty = state.tableList[index].packQty;
      const price = state.tableList[index].price;
      const priceActual = state.tableList[index].priceActual;
      console.log(packQty, price, priceActual, 'packQty price');
      if (packQty && price) {
        console.log('calc start');
        const bigCost = divide(bignumber(price), bignumber(packQty));
        console.log(bigCost);
        const bigActualCost = divide(bignumber(priceActual || 0), bignumber(packQty));
        console.log(bigActualCost);
        state.tableList[index].cost = parseFloat(bigCost.toFixed(6));
        state.tableList[index].costActual = parseFloat(bigActualCost.toFixed(6));
      }
    });
  };
