      ...
      <div class="lydc-content-wrapper-content bg-white">
        <div>
          <div style="display: flex; flex-direction: row; justify-content: space-between">
            <a-space>
              <Subtitle :title="props.title" id="silicone" />
              <div v-if="hasBtnP('price-silicone')" class="total"
          ...
          <BasicTable @register="registerTable" :dataSource="state.tempList" :style="getTableStyle()">
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-if="column.compType == 'InputNumber'">
                ...
                <!--                  :disabled="column?.disabled" />-->
                <!--                <div v-else>{{ record[column.dataIndex] || '-' }}</div>-->
                <template v-if="column.dataIndex === 'defectRate'">
                  <LydcInputNumber
                    v-if="!props.detailed"
                    @change="calcValues(index)"
                    v-model:value="state.tableList[index][column.dataIndex]"
              ...
              <template v-if="column.compType == 'Select'">
                <template v-if="column.mode == 'searchInput'">
                  <ApiSelect
                    ...
                    :allowClear="column.dataIndex === 'insideCode' || column.dataIndex === 'materialTypeFromManufacturer'"
                    :disabled="
                      column.dataIndex === 'insideCode'
...
<script lang="ts" setup>
  ...
  import { add, bignumber, divide, evaluate, multiply, subtract } from 'mathjs';
  import { getLaborRateList, getMaterialPriceList } from '/@/api/engineering/quotatios';
  ...
  async function onChange(index, column, e, option) {
    ...
    if (column.compType == 'Input') {
      ...
      if (column.mode == 'searchInput') {
        ...
        if (column.dataIndex === 'insideCode' || column.dataIndex === 'materialTypeFromManufacturer') {
          ...
          const param = {
            insideCode: column.dataIndex === 'insideCode' ? e : '',
          ...
          console.log(props.fieldsValue.isBonded);
          console.log(props.fieldsValue.isBonded == 'true');
          if (code == '200' && data && e) {
            // 是否是保税价格 且报价是否大于0
            if (props.fieldsValue.isBonded == 'true' && data.bondedPricePurchasingUnit >= 0) {
              fieldsObj = {
                ...fieldsObj,
            ...
            createMessage.warning('没有查询到当前报价的成本价格');
          }
        }
  ...
  const calcValues = index => {
    nextTick(() => {
      ...
      // 价格 * 1000 * 材料用量 / ( 1 - 不良率 )
      const useQty = state.tableList[index].useQty;
      const wholePrice = state.tableList[index].wholePrice;
      const defectRate = state.tableList[index].defectRate;
      const priceActual = state.tableList[index].priceActual;
      console.log(useQty, wholePrice, defectRate, 'useQty wholePrice defectRate');
      if (useQty && wholePrice && defectRate) {
        const cost = divide(multiply(multiply(bignumber(wholePrice), 1000), bignumber(useQty)), subtract(1, divide(bignumber(defectRate), 100)));
        const costActual = divide(multiply(multiply(bignumber(priceActual || 0), 1000), bignumber(useQty)), subtract(1, divide(bignumber(defectRate), 100)));
