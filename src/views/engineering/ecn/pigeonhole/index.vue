<template>
  <div class="lydc-content-wrapper">
          ...
          <a-tab-pane key="1" :tab="t('common.todoText')">
            <Grid_Pending>
              <template #toolbar-actions>
                <a-button v-auth="'btn_edit'" :loading="executeLoading" @click="handleExecute" type="primary"
                  >{{ t('dict.ECNColumn.confirmExecution') }}
                </a-button>
    ...
    <NodeModal @register="registerNodeModal" @reload="reloadTable"></NodeModal>
    <DetailPopup @register="registerDetail" @close="waitReloadreloadTable" @reload="waitReloadreloadTable" />
    <TransferModal
...
<script lang="ts" setup>
  ...
  import { executeConfirm, exportEcn, getEcnPageList, getEcrPageList, getNodeDetail, withdrawEcn, turnEcn, turnEcnV2 } from '/@/api/engineering/ecn';
  ...
  defineOptions({ name: 'engineering-ecn-pigeonhole' });
  ...
  const [Grid_Pending, gridApi_Pending] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'engineering-ecn-pigeonhole-list',
      columns: waitGetColumns,
      // toolbarConfig: {
      //   superQuery: true,
      // },
  ...
  function handleExecute() {
    const rowKeys = waitGetSelectRowKeys();
    executeLoading.value = true;
    executeConfirm(rowKeys)
      .then(({ code, msg }) => {
        if (code == 200) {
          message.success(msg);
          reloadTable();
