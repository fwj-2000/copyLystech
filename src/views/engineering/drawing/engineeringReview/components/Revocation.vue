<template>
  <BasicModal v-bind="$attrs" @register="registerModal" canFullscreen draggable :title="t('common.revokeAllTip')" showOkBtn @ok="handleSubmit" destroy-on-close>
...
<script setup lang="ts">
  ...
  import { withdrawDrawingsReview } from '/@/api/engineering/reviewInquiry';
  ...
  async function handleSubmit() {
    ...
    if (isEmpty(state.list)) {
      ...
      params.idList = state.list.map(item => item.id);
    }
    withdrawDrawingsReview(params)
      .then(({ code, msg }) => {
        if (code === 200) {
