    ...
    <table border="1" cellspacing="0" cellpadding="8">
      ...
      <tbody>
        <template v-if="data.length > 0">
          <tr v-for="(item, index) in props.data" :key="`tr-${index}`" class="whole-node">
            <td v-for="(col, colIndex) in props.columns" :key="`td-${index}-${colIndex}`">
              ...
              <template v-else-if="col.type === 'editor' && col.field">
                <div v-html="getSafeContent(item[col.field])"></div>
              </template>
        ...
        <tr v-else class="whole-node">
          <td v-for="(col, colIndex) in props.columns" :key="`td-${col.field}-${colIndex}`"></td>
        </tr>
      </tbody>
      <tfoot>
        <tr class="whole-node">
...
<script lang="ts" setup>
  ...
  import { getSafeContent } from '/@/utils/editor';
