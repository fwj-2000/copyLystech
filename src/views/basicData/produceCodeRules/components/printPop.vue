<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" title="" destroyOnClose class="full-popup">
    <template #insertToolbar>
      <a-button @click="printFn" class="signs print mr-12px">{{ t('common.printText') }}</a-button>
    </template>
    <div class="print-content h-full">
      <div ref="printRefs" class="print-refs">
        <PrintComponent v-for="(item, index) in printComponentInfoList" :key="index" :printComponentInfo="item"></PrintComponent>
      </div>
    </div>
  </BasicPopup>
</template>
