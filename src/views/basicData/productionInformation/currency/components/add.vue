...
<script setup lang="ts">
  ...
  import { addCurrency, updateCurrency, getCurrencyById } from '/@/api/basicData/currency';
  ...
  function init(data) {
    ...
    if (state.dataForm.Id) {
      changeLoading(true);
      getCurrencyById(state.dataForm.Id).then(res => {
        const data = res.data;
        state.dataForm = data;
  ...
  async function handleSubmit() {
    ...
    const formMethod = state.dataForm.Id ? updateCurrency : addCurrency;
    formMethod(query)
      .then(res => {
