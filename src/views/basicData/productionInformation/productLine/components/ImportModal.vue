<template>
  <BasicModal
    ...
    <div class="import-main" v-show="activeStep === 1">
      <a-table
        :data-source="list"
        :columns="transformI18nBasicTable(columns, 'CustomerColumn')"
        size="small"
        :pagination="false"
...
<script lang="ts" setup>
  ...
  import { templateDownload, importPreview, importData } from '/@/api/basicData/productLine';
  ...
  const { t } = useI18n();
  const tableData = [
    { title: '错误信息', dataIndex: 'ImportError', key: 'ImportError', width: 70 },
    { title: '编码', dataIndex: 'Code', key: 'Code', width: 40 },
    { title: '名称', dataIndex: 'Name', key: 'Name', width: 50 },
    { title: '主要制程', dataIndex: 'MainProcess', key: 'MainProcess', width: 50 },
    { title: '状态', dataIndex: 'StatusName', key: 'StatusName', width: 50 },
  ];
  const columns: any[] = [
    { width: 50, title: '序号', i18nField: 'seq', align: 'center', customRender: ({ index }) => index + 1 },
    ...tableData,
    { title: '操作', dataIndex: 'action', key: 'action', width: 50, fixed: 'right' },
  ];
  const resultColumns: any[] = [{ width: 50, title: '序号', i18nField: 'seq', align: 'center', customRender: ({ index }) => index + 1 }, ...tableData];
