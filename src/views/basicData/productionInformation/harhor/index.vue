...
<script setup lang="ts">
  ...
  import { getHarhor, deleteHarhor, blukDeleteHarhor } from '/@/api/basicData/harhor';
  ...
  defineOptions({ name: 'basicData-productionInformation-harhor' });
  ...
  const columns = [
    ...
    { title: '口岸名称', field: 'Name', width: 150 },
  ...
  const [Grid, gridApi] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'productionInformation-harhor-list',
      columns: columns,
      api: getHarhor,
  ...
  function handleDelete(record: Recordable) {
    deleteHarhor(record.Id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    if (list?.length === 0) {
      ...
      createConfirm({
        ...
        onOk: async () => {
          try {
            const { code } = await blukDeleteHarhor(list);
            if (code == 200) {
              clearSelectedRowKeys();
