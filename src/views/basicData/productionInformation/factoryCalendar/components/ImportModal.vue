...
<script lang="ts" setup>
  ...
  import { templateDownload } from '/@/api/productionDeal/badCode';
  ...
  const tableData = [
    ...
    { title: '不良代码', dataIndex: 'Code', key: 'Code', width: 50 },
    { title: '不良代码描述', dataIndex: 'Description', key: 'Description', width: 50 },
  ...
  function handleNext() {
    if (state.activeStep == 0) {
      ...
      importBadCodePreview({ fileName: state.fileName })
        .then(res => {
    ...
    if (state.activeStep == 1) {
      ...
      state.btnLoading = true;
      importBadCodeData({ Data: state.list })
        .then(res => {
