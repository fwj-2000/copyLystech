...
<script lang="ts" setup>
  ...
  import { getLocalmaintpage, delLocalmaintpage, addorupdatelocalmaint } from '/@/api/dataAnalysis/warehouseKanban';
  ...
  const attrs = reactive<VxeGridProps<any>>({
    id: 'dataAnalysis-warehouseKanban-positionComparisonTable-list',
    proxyConfig: {
      response: {
  ...
  async function syncDataDele() {
    ...
    createConfirm({
      ...
      onOk: async () => {
        try {
          const { code } = await delLocalmaintpage(ids);
          if (code == 200) {
