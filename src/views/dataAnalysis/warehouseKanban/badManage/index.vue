...
<script lang="ts" setup>
  ...
  import { getBadManage, ExportBadManage } from '/@/api/dataAnalysis/warehouseKanban';
  import { useTableLayout } from '/@/views/dataAnalysis/components/TableLayout/useTableLayout';
  import { useModal } from '/@/components/Modal';
  import ExportModal from '/@/components/ExportModal/index.vue';
  import { getAllColumns, formOptions, filterOptions } from './config';
  defineOptions({ name: 'warehouseKanban-badManage' });
  const columns = ref(getAllColumns());
  const attrs = reactive<VxeGridProps<any>>({
    id: 'warehouseKanban-badManage-list',
    proxyConfig: {
      response: {
