...
<script lang="ts" setup>
  ...
  import { getAboutToExpire, ExportAboutToExpire } from '/@/api/dataAnalysis/warehouseKanban';
  import { useTableLayout } from '/@/views/dataAnalysis/components/TableLayout/useTableLayout';
  import { useModal } from '/@/components/Modal';
  import ExportModal from '/@/components/ExportModal/index.vue';
  import { getAllColumns, formOptions, filterOptions } from './config';
  defineOptions({ name: 'warehouseKanban-aboutToExpire' });
  const columns = ref(getAllColumns());
  const attrs = reactive<VxeGridProps<any>>({
    id: 'warehouseKanban-aboutToExpire-list',
    proxyConfig: {
      response: {
  ...
  const [TableLayout, api] = useTableLayout({
    ...
    tableConfig: {
      columns,
      attrs,
      api: getAboutToExpire,
    },
    toolbarConfig: {
      baseExportMethod: () => {
        const { getFormParams } = api.searchFormApi;
        openExportModal(true, {
          api: ExportAboutToExpire,
