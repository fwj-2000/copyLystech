...
<script lang="ts" setup>
  ...
  import { getIncomeZfi021Na, ExportIncomeZfi021Na } from '/@/api/dataAnalysis/revenue';
  ...
  defineOptions({ name: 'dataAnalysis-financial-revenue-incomeVimFc' });
  const columns = ref(getAllColumns());
  const attrs = reactive<VxeGridProps<any>>({
    id: 'dataAnalysis-financial-revenue-incomeVimFc-list',
    filterConfig: {
      remote: true,
    },
    proxyConfig: {
  ...
  const [TableLayout, api] = useTableLayout({
    ...
    tableConfig: {
      columns,
      attrs,
      api: getIncomeZfi021Na,
    },
    toolbarConfig: {
      baseExportMethod: () => {
        const { getFormParams } = api.searchFormApi;
        openExportModal(true, {
          api: ExportIncomeZfi021Na,
