...
<script lang="ts" setup>
  ...
  import { getIncomeZmm010i, ExportIncomeZmm010i } from '/@/api/dataAnalysis/revenue';
  ...
  defineOptions({ name: 'dataAnalysis-financial-revenue-inSalesIncome' });
  const columns = ref(getAllColumns());
  const attrs = reactive<VxeGridProps<any>>({
    id: 'dataAnalysis-financial-revenue-inSalesIncome-list',
    filterConfig: {
      remote: true,
    },
    proxyConfig: {
  ...
  const [TableLayout, api] = useTableLayout({
    ...
    tableConfig: {
      columns,
      attrs,
      api: getIncomeZmm010i,
    },
    toolbarConfig: {
      baseExportMethod: () => {
        const { getFormParams } = api.searchFormApi;
        openExportModal(true, {
          api: ExportIncomeZmm010i,
