import { flattenDeep, keyBy, isFunction, omit, orderBy, mapKeys } from 'lodash-es';
...
export const getMergeRow = ({ columns, data }) => {
  ...
  mergeColumns.forEach((column, index) => {
    spans.push(...getSingleColMergeRow({ column, data, columnIdx: index, mergeColumns }));
  });
  return spans.sort((a, b) => a.row - b.row);
};

// 获取单列合并行数据
function getSingleColMergeRow({ column, data, columnIdx, mergeColumns }) {
  const spans: any = [];
  let currentIdx = 0;
  let startIdx = 0;
  // const { field } = column;
  ...
  while (currentIdx < data.length) {
    ...
    if (nextIdx < data.length) {
      // 不仅要比较当前列的字段值，还要比较前面所有需要合并的列的字段值
      for (let i = 0; i <= columnIdx; i++) {
        const prevColumn = mergeColumns[i];
        const prevMergeField = prevColumn.mergeConfig?.mergeRowField ?? prevColumn.field;
        if (data[currentIdx][prevMergeField] !== data[nextIdx][prevMergeField]) {
          shouldMerge = false;
          break;
        }
      }
    ...
    if (nextIdx === data.length || !shouldMerge) {
  ...
  const res = data.map(item => {
    const columnData = columns.reduce((pre, cur) => {
      ...
      if (columnType === EColumnType.DATA_LIST) {
        ...
        return {
          ...pre,
          ...getFormatDimKeysValuesInfo(item[field]),
        };
...
// 格式化 WEEK_KEYS_VALUES 类型的数据
function getFormatDimKeysValuesInfo(values: any[]) {
  const info = values.reduce((pre, cur) => {
    return {
      ...pre,
      ...getSingleInfo(cur),
  ...
  function getSingleInfo(info: Record<string, any>) {
    return info.Values.reduce((pre, cur) => {
      return {
        ...pre,
        [`${info.DimKey}_${cur.keys ?? cur.Keys}`]: cur.values ?? cur.Values,
      };
    }, {});
  }
  return info;
...
// 获取 Dim_KEYS_VALUES 类型的表头配置
export function getFormatDimKeysValuesColumns(firstValues: any[], columnOption: IColumn): BaseVxeTableTypes.Columns {
  if (!firstValues) {
    return [];
  }
  const { parentOption = {}, commonOption = {}, childOption = {}, getChildConfig } = columnOption;
  const columns = firstValues.map(item => {
    return {
      ...
      children: item.Values.map(value => {
        const lowerCaseValue = mapKeys(value, (_, key) => key.toLowerCase()); //将value中键转小写
        return {
          ...commonOption,
          ...childOption,
          ...(typeof getChildConfig === 'function' ? getChildConfig(lowerCaseValue) : {}),
          field: `${item.DimKey}_${lowerCaseValue.keys}`,
          title: lowerCaseValue.keys,
        };
      }),
    };
  });
...
export function getFormatWeekKeysValuesColumns(firstValues: any[], columnOption: IColumn): BaseVxeTableTypes.Columns {
  ...
  console.log(firstValues, '11firstValues');
...
// 格式化表尾数据
export const formatFooterData = (columns: BaseVxeTableTypes.Columns, data: any[]) => {
