<template>
  <BasicModal v-bind="$attrs" @register="registerModal" defaultFullscreen :footer="null" :closable="false" :keyboard="false" class="lydc-full-modal full-modal">
    ...
    <BasicColumnDesign ref="columnDesignRef" :columnData="columnData" :appColumnData="appColumnData" :formInfo="dataForm" v-if="activeStep == 2" />
...
<script lang="ts" setup>
  ...
  const { activeStep, loading, btnLoading, tables, mainTableFields, dbType, formData, columnData, appColumnData, dataForm } = toRefs(state);
  const columns = [
  ...
  function init(data) {
    ...
    if (state.dataForm.id) {
      ...
      state.dataForm.webType = data.webType || 1;
      state.loading = false;
      changeLoading(false);
    }
  }
