  ...
  function initData() {
    getSysMonitor()
      .then(res => {
        ...
        state.cpuInfo.series[0].data.push(Number.parseFloat(state.data.memory.used));
        state.memoryInfo.series[0].data.push(Number.parseFloat(state.data.memory.usageRate));
        state.loading = false;
      })
      .catch(() => {
        state.loading = false;
