<template>
  <BasicDrawer v-bind="$attrs" width="500px" @register="registerDrawer" :title="t('common.detailText')" class="lydc-log-detail" destroyOnClose>
    <a-form class="!mx-20px !mt-20px" :colon="false" :model="dataForm" ref="formElRef" :labelCol="{ style: { width: '80px' } }">
      ...
      <a-collapse v-model:activeKey="activeName" ghost expandIconPosition="right">
        <a-collapse-panel key="2" header="请求地址">
          <div class="lydc-code-box" v-if="dataForm.requestUrl">{{ dataForm.requestUrl }}</div>
        </a-collapse-panel>
        <a-collapse-panel key="3" header="请求方法" v-if="activeKey != '4'">
          <div class="lydc-code-box" v-if="dataForm.requestTarget">{{ dataForm.requestTarget }}</div>
        </a-collapse-panel>
        <a-collapse-panel key="4" header="请求参数" v-if="activeKey != '4'">
          <div class="lydc-code-box" v-if="dataForm.requestParam">{{ dataForm.requestParam }}</div>
        </a-collapse-panel>
        <a-collapse-panel key="5" :header="activeKey != '4' ? '返回结果' : '异常描述'">
          <div class="lydc-code-box" v-if="dataForm.jsons">{{ dataForm.jsons }}</div>
        </a-collapse-panel>
      </a-collapse>
    </a-form>
  </BasicDrawer>
