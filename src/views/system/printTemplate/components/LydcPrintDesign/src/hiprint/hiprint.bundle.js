...
'use strict';
function mmToPt(mmInput) {
  // 解析输入值（支持 "99mm"、"99 mm"、纯数字等格式）
  const mmValue = typeof mmInput === 'string' ? parseFloat(mmInput.replace('mm', '').trim()) : mmInput;

  // 验证输入有效性
  if (isNaN(mmValue) || mmValue <= 0) {
    throw new Error('请输入有效的毫米值，例如: "99mm" 或 99');
  }

  // 换算公式: pt = mm × (72 / 25.4)
  const ptValue = mmValue * (72 / 25.4);

  // 返回精确到2位小数的结果
  return Number(ptValue.toFixed(2));
}
...
import './plugins/canvas2image.js';
// 数字转中文,大写,金额
import * as Nzh from './plugins/nzh.min.js';
// 解析svg 到 canvas, 二维码条形码需要
import './plugins/r.js';
import * as Canvg from './plugins/canvg.min.js';
// 默认自定义拖拽列表
import defaultTypeProvider from './etypes/default-etyps-provider';
import cn from './i18n/cn.json';
import cn_tw from './i18n/cn_tw.json';
...
var languages = { zh_CN: cn, zh_TW: cn_tw, en_US: en, vi_VN: yue };
var i18n = {
          ...
          (BasePrintElement.prototype.updatePanelHeight = function (h, p) {
            if ('none' == this.panel.panelPageRule) {
              try {
                ...
                console.log('updatePanelHeight error', error);
              }
            }
          }),
            ...
            if ((t && (e = t.getFields()), e)) {
              ...
              if (this) {
                this.target.find('button').click(function () {
                  if (TemporaryHiprint.onClickField) {
                    TemporaryHiprint.onClickField();
                  }
                });
              }
            }
      ...
      TablePrintElement = (function (_super) {
        ...
        return (
          ...
          (TablePrintElement.prototype.getTableHtml = function (t, e) {
            ...
            return (
              this.isNotDesign && ['first', 'none'].includes(this.options.tableHeaderRepeat) ? o.append(headerList) : o.append(headerList[0]),
            ...
            n(i.options.stage)
              .on('mousemove', function (e) {
                if (d) {
                  ...
                  console.log(E, 'E');
                  // console.log('向下移动', e, e.target.style.height, t, i, c, this, d);
                  // console.log($('div.hiprint-printPaper.hidroppable.design')[0].style.height, `hiprint-printPaper`)
                  console.log(t[0].style.height, 't');
                  if (t[0].className.indexOf('hiprint-printElement-table') !== -1) {
                    let divH = TemporaryHiprint.printPanels[0].paperFooter - TemporaryHiprint.printPanels[0].paperHeader;
                    let tH = Number(t[0].style.height.replace('pt', ''));
                    if (tH + 5 >= divH && E > 0) {
                      console.log('这个dom含有hiprint-printElement-table', divH, tH);
                      if (tH > divH) {
                        t.css({
                          height: divH - 2 + 'pt',
                        });
                    ...
                    console.log('这个dom不含有hiprint-printElement-table');
                  }
                  t.css({
                    height: '100%',
                  }),
    ...
    var _r,
      ...
      v = (function (t) {
        ...
        return (
          ...
          (e.prototype.updateTargetImage = function (t, e, n) {
            ...
            if (n.length) i.find('img').css('cssText', `width:100%;height:100%;content:url("${n}")!important;object-fit: contain;`);
      ...
      ut = (function () {
        ...
        return (
          ...
          (t.prototype.buildSetting = function (t) {
            ...
            var r;
            if (tabs.length) {
              e.printTemplate.onclick && e.printTemplate.onclick(t.printElement.printElementType.title);
          ...
          (t.prototype.buildSettingByCustomOptions = function (t, e) {
            ...
            if (TemporaryHiprint.onclick !== undefined) {
              TemporaryHiprint.onclick('空白');
            }
            t.title &&
              r.append(
                '<div class="hiprint-option-item hiprint-option-item-row">\n            <div class="hiprint-option-item-label hiprint-option-title">\n              ' +
      ...
      ct = (function () {
        function t(t) {
          ...
          this.historyPos = 0;
          this.onClickField = n.onClickField;
            ...
            o.a.event.on('hiprintTemplateDataChanged_' + this.id, function (type) {
              if (t.history) {
                ...
                if (t.historyList.length > 50) {
                  t.historyList = t.historyList.slice(1, 51);
                } else {
                  t.historyPos += 1;
                }
