    ...
    <BasicForm @register="registerForm">
      <template #icon="{ model, field }">
        <a-row type="flex">
          ...
          <a-form-item-rest>
            <lydc-color-picker v-model:value="dataForm.propertyJson.iconBackgroundColor" size="small" :predefine="predefineList" name="iconBackground" />
          </a-form-item-rest>
        </a-row>
      </template>
      ...
      <template #moduleId="{ model, field }">
        <LydcTreeSelect v-model:value="model[field]" lastLevel :options="tempData" placeholder="请选择关联" @change="onModuleIdChange" :key="moduleIdKey" />
      </template>
    </BasicForm>
  </BasicModal>
</template>
