<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="title" class="full-popup">
    <div class="lydc-content-wrapper-search-box ml-10px mt-10px">
      <BasicForm class="search-form" @register="registerSearchForm" @submit="handleSubmit" @reset="handleReset" />
    </div>
    <a-tabs v-model:activeKey="listQuery.category" type="card" class="lydc-content-table-tabs" destroyInactiveTabPane>
        ...
        <BasicTable @register="registerWebTable" :columns="menuTableColumns" :searchInfo="getSearchInfo">
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'icon'">
              <SvgIcon class="svg-icon" size="16" :name="record.icon" v-if="record.icon && record.icon[0] === '#'" />
              <i :class="record.icon + ' table-icon'" v-else />
            </template>
