<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" showOkBtn @ok="handleSubmit" class="lydc-condition-modal" destroy-on-close>
    <a-form :colon="false" :labelCol="{ style: { width: '0' } }" :model="dataForm" :rules="dataRule" ref="formElRef" class="condition-main">
      ...
      <div class="mb-10px" v-if="condition.length">
        ...
        <lydc-select v-model:value="dataForm.matchLogic" placeholder="请选择" :options="logicOptions" class="!w-68px" />
      </div>
      <a-form-item v-for="(item, index) in condition" :key="index" class="!mb-10px">
        <div>
          <a-row :gutter="8">
            <a-col :span="6">
              <lydc-select v-model:value="item.logic" placeholder="请选择" :options="logicOptions" />
            </a-col>
            <a-col :span="5">
          ...
          <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" class="mt-10px">
            <a-col :span="6">
              <lydc-select v-model:value="child.id" placeholder="请选择字段" allowClear :options="fieldOptions" @change="onFieldChange($event, child)" />
            </a-col>
            <a-col :span="5">
              <lydc-select v-model:value="child.op" placeholder="选择符号" allowClear :options="child.opOptions" @change="onSymbolChange($event, child)" />
            </a-col>
            <a-col :span="12">
              <template v-if="child.conditionText === 'inputNumber'">
                <lydc-number-range v-model:value="child.value" :disabled="child.disabled" v-if="child.op == 'between'" />
                <lydc-input-number v-model:value="child.value" :disabled="child.disabled" placeholder="请输入" v-else />
              </template>
...
<script lang="ts" setup>
  ...
  const systemFieldOptions = [
    ...
    { id: '@userAndInfiniteSubordinates', fullName: '当前用户及无限层级下属' },
    { id: '@groupCodes', fullName: '当前用户分组(编码)' },
    ...
    getDataAuthorizeList({ moduleId: state.dataForm.moduleId })
      .then(res => {
        ...
        state.fieldOptions.map(o => {
          o.enCode = o.enCode.replace('_lydc_', '.');
          o.enCode = o.enCode.replace('lydc_', '');
        });
  ...
  function onSymbolChange(val, item) {
    const systemFieldOptions = [
      '@userId',
      '@userAndSubordinates',
      '@userAndInfiniteSubordinates',
      '@groupCodes',
      '@organizeId',
      '@organizationAndSuborganization',
      '@branchManageOrganize',
    ];
