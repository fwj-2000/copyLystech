...
function getDateRangeWithWeekdays(dateRange) {
  ...
  while (currentDate.isBefore(endDate) || currentDate.isSame(endDate)) {
    ...
    const days = currentDate.format('DD');
...
export function useDate(dateRange: string[]) {
  const dateArray = getDateRangeWithWeekdays(dateRange);
  return { dateArray };
}
