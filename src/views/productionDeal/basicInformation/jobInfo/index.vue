...
<script setup lang="ts">
  ...
  import { getJobInfo, deleteJobInfo, blukDeleteJobInfo, exportJobInfoExcel } from '/@/api/productionDeal/jobInfo';
  ...
  const searchFormSchema = [
    {
      ...
      defaultValue: '',
      componentProps: {
        placeholder: t(['common.inputPlaceholder', 'dict.JobInfoColumn.Code']), //'请输入岗位编码',
      },
      colProps: { span: 4 },
    },
    {
      ...
      defaultValue: '',
      componentProps: {
        placeholder: t(['common.inputPlaceholder', 'dict.JobInfoColumn.Name']), //'请输入岗位名称',
      },
      colProps: { span: 4 },
    },
    {
  ...
  function handleDelete(record: Recordable) {
    deleteJobInfo(record.Id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    if (list?.length === 0) {
      ...
      createConfirm({
        ...
        onOk: async () => {
          try {
            const { code } = await blukDeleteJobInfo(list);
            if (code == 200) {
              clearSelectedRowKeys();
