...
<script setup lang="ts">
  ...
  import { addGroupInfo, updateGroupInfo, getGroupInfoById } from '/@/api/productionDeal/groupInfo';
  ...
  function init(data) {
    ...
    if (state.dataForm.Id) {
      changeLoading(true);
      getGroupInfoById(state.dataForm.Id).then(res => {
        const data = res.data;
        state.dataForm = data;
        setFieldsValue({ ...data, GroupMembersIds: data.GroupMembersIds && data.GroupMembersIds.split(',') }); //设置表单值
  ...
  transformSchemas(schemas, 'GroupInfoColumn', { OrgId: 'OrgName' });
  ...
  async function handleSubmit() {
    ...
    const query = {
      ...
      GroupMembersIds: values.GroupMembersIds && values.GroupMembersIds.join(','),
    };
    const formMethod = state.dataForm.Id ? updateGroupInfo : addGroupInfo;
    formMethod(query)
      .then(res => {
