...
<script setup lang="ts">
  ...
  import { getProductConfigParam, deleteProductConfigParam, blukProductConfigParam } from '/@/api/productionDeal/productConfigParam';
  import { useModal } from '/@/components/Modal';
  import { useMessage } from '/@/hooks/web/useMessage';
  import { message } from 'ant-design-vue';
  import add from './components/add.vue';
  defineOptions({ name: 'productionDeal-basicInformation-productConfigParam' });
  const { t } = useI18n();
  ...
  const [Grid, { reload, getSelectRowKeys }] = useVbenVxeGrid({
    formOptions: {
      ...
      i18nConfig: {
        moduleCode: 'ProductionConfigParamColumn',
        transferRange: ['placeholder'],
      },
    },
    gridOptions: {
      id: 'productionDeal-basicInformation-productConfigParam-list',
      columns: column as any,
      api: getProductConfigParam,
  ...
  function handleDelete(record: Recordable) {
    deleteProductConfigParam(record.id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    createConfirm({
      ...
      onOk: async () => {
        const { code } = await blukProductConfigParam(list);
        if (code == 200) {
