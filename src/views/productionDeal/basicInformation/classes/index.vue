...
<script setup lang="ts">
  ...
  import { getClasses, deleteClasses, blukDeleteClasses, exportClassesExcel } from '/@/api/productionDeal/classes';
  ...
  const columns = [
    ...
    { title: '班次编码', field: 'Code' },
    { title: '班次名称', field: 'Name' },
    { title: '所属组织', field: 'OrgName' },
    { title: '班次时间段', field: 'ClassesTimeSlot' },
    { title: '上班时间1', field: 'OnWorkTimeOne' },
    { title: '下班时间1', field: 'OffWorkTimeOne' },
    { title: '上班时间2', field: 'OnWorkTimeTwo' },
    { title: '下班时间2', field: 'OffWorkTimeTwo' },
    { title: '加班上班时间', field: 'OverTimeOnWorkTime' },
    { title: '加班下班时间', field: 'OverTimeOffWorkTime' },
    {
      title: '是否启用',
      field: 'Status',
      cellRender: {
  ...
  const searchFormSchema = [
    {
      ...
      defaultValue: '',
      componentProps: {
        placeholder: t(['common.inputPlaceholder', 'dict.ClassesColumn.Code']), //'请输入班次编码',
        allowClear: true,
      },
    },
    {
      ...
      component: 'Select',
      componentProps: {
        placeholder: t(['common.selectPlaceholder', 'dict.ClassesColumn.Name']), //'请选择班次',
        allowClear: true,
      },
    },
    {
  ...
  const [
    ...
    gridOptions: {
      id: 'productionDeal-basicInformation-classes',
  ...
  function handleDelete(record: Recordable) {
    deleteClasses(record.Id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    if (list?.length === 0) {
      ...
      createConfirm({
        ...
        onOk: async () => {
          try {
            const { code } = await blukDeleteClasses(list);
            if (code == 200) {
              clearSelectedRowKeys();
