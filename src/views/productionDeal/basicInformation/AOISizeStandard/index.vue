...
<script setup lang="ts">
  ...
  import { getAOIaizestandard, deleteAOIaizestandard, blukAOIaizestandard } from '/@/api/productionDeal/AOISizeStandard';
  import { useModal } from '/@/components/Modal';
  import { useMessage } from '/@/hooks/web/useMessage';
  import { message } from 'ant-design-vue';
  import add from './components/add.vue';
  defineOptions({ name: 'productionDeal-basicInformation-AOISizeStandard' });
  const { t } = useI18n();
  ...
  const [Grid, { reload, getSelectRowKeys }] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'productionDeal-basicInformation-AOISizeStandard-list',
      columns: column as any,
      api: getAOIaizestandard,
  ...
  function handleDelete(record: Recordable) {
    deleteAOIaizestandard(record.id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    createConfirm({
      ...
      onOk: async () => {
        const { code } = await blukAOIaizestandard(list);
        if (code == 200) {
