...
<script setup lang="ts">
  ...
  import { getLineType, deleteLineType, blukLineType } from '/@/api/productionDeal/lineTypeConfig';
  import { useModal } from '/@/components/Modal';
  import { useMessage } from '/@/hooks/web/useMessage';
  import { message } from 'ant-design-vue';
  import add from './components/add.vue';
  defineOptions({ name: 'productionDeal-basicInformation-lineTypeConfig' });
  const { t } = useI18n();
  ...
  const [Grid, { reload, getSelectRowKeys }] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'productionDeal-basicInformation-lineTypeConfig-list',
  ...
  function handleDelete(record: Recordable) {
    deleteLineType(record.id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    createConfirm({
      ...
      onOk: async () => {
        const { code } = await blukLineType(list);
        if (code == 200) {
