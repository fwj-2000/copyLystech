...
<script lang="ts" setup>
  ...
  import { templateDownload, importSamplingLevelPreview, importSamplingLevelData } from '/@/api/productionDeal/samplingLevel';
  ...
  const tableData = [
    ...
    { title: '批量上限', dataIndex: 'BatchLimit', key: 'BatchLimit', width: 50 },
    { title: '批量下限', dataIndex: 'BatchLowerLimit', key: 'BatchLowerLimit', width: 50 },
    { title: 'AQL值', dataIndex: 'AQLValue', key: 'AQLValue', width: 50 },
    { title: '样本大小', dataIndex: 'SampleSize', key: 'SampleSize', width: 50 },
    { title: '检验水平', dataIndex: 'TestingLevel', key: 'TestingLevel', width: 50 },
    { title: '允许数量', dataIndex: 'AllowQuantity', key: 'AllowQuantity', width: 50 },
    { title: '拒收数量', dataIndex: 'RejectQuantity', key: 'RejectQuantity', width: 50 },
  ...
  function handleNext() {
    if (state.activeStep == 0) {
      ...
      importSamplingLevelPreview({ fileName: state.fileName })
        .then(res => {
    ...
    if (state.activeStep == 1) {
      ...
      state.btnLoading = true;
      importSamplingLevelData({ Data: state.list })
        .then(res => {
