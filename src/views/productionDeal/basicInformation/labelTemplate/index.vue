      ...
      <div class="lydc-content-wrapper-content">
        <BasicTable @register="registerTable">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'IsLotNoIncrease'">
              <a-tag :color="record.IsLotNoIncrease === 1 ? 'success' : 'error'">{{ record.IsLotNoIncrease === 1 ? t('common.yes') : t('common.no') }}</a-tag>
            </template>
            <template v-if="column.dataIndex === 'IsUseSeparator'">
              <a-tag :color="record.IsUseSeparator === 1 ? 'success' : 'error'">{{ record.IsUseSeparator === 1 ? t('common.yes') : t('common.no') }}</a-tag>
            </template>
...
<script setup lang="ts">
  ...
  import { getLabelTemplate, deleteLabelTemplate, blukDeleteLabelTemplate, exportLabelTemplateExcel } from '/@/api/productionDeal/labelTemplate';
  ...
  const columns: BasicColumn[] = [
    ...
    { title: '过滤Id', dataIndex: 'FilterId' },
    { title: '包含标签数', dataIndex: 'ContainLableNumber' },
    { title: '是否Lot递增', dataIndex: 'IsLotNoIncrease' },
    { title: '是否千分符', dataIndex: 'IsUseSeparator' },
  ...
  const exprotColumns: BasicColumn[] = [{ title: '模板内容', dataIndex: 'LabelTemplate' }, ...columns];
  ...
  const searchFormSchema: FormSchema[] = [
    {
      ...
      defaultValue: '',
      componentProps: {
        placeholder: t(['common.inputPlaceholder', 'dict.LabelTemplateColumn.LabelName']), //'请输入标签名称',
      },
      colProps: { span: 4 },
    },
    {
      ...
      defaultValue: '',
      componentProps: {
        placeholder: t(['common.inputPlaceholder', 'dict.LabelTemplateColumn.FilterId']), //'请输入过滤Id',
      },
      colProps: { span: 4 },
    },
    {
  ...
  const [registerTable, { getRowSelection, reload, getFetchParams, getForm }] = useTable({
    ...
    i18nConfig: {
      moduleCode: `LabelTemplateColumn`,
    },
  });
  ...
  function handleDelete(record: Recordable) {
    deleteLabelTemplate(record.Id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    if (list?.length === 0) {
      ...
      createConfirm({
        ...
        onOk: async () => {
          try {
            const { code } = await blukDeleteLabelTemplate(list);
            if (code == 200) {
              getRowSelection().selectedRowKeys = [];
