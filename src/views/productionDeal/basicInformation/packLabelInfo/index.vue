...
<script setup lang="ts">
  ...
  import { getPackLabelInfo, deletePackLabelInfo, blukDeletePackLabelInfo, exportPackLabelInfoExcel } from '/@/api/productionDeal/packLabelInfo';
  ...
  const [registerForm, { getFieldsValue }] = useForm({
    ...
    i18nConfig: {
      moduleCode: 'PackLabelInfoColumn',
  ...
  const [Grid, { reload, getFetchParams, getSelectRowKeys }] = useVbenVxeGrid({
    gridOptions: {
      id: 'productionDeal-PackLabelInfo-list',
      columns: getColumn(),
      api: getPackLabelInfo,
  ...
  function handleDelete(record: Recordable) {
    deletePackLabelInfo(record.Id).then(res => {
  ...
  async function handleDeleteList() {
    ...
    if (list?.length === 0) {
      ...
      createConfirm({
        ...
        onOk: async () => {
          try {
            const { code } = await blukDeletePackLabelInfo(list);
            if (code == 200) {
