        ...
        <BasicTable @register="registerTable">
          ...
          <template #bodyCell="{ column, record, text }">
            ...
            <template v-if="column.dataIndex == 'Yield'">
              ...
              <span v-else-if="text <= 0 || text == null"></span>
              <span v-else>{{ text }}%</span>
            </template>
            <!-- 待完成数量 -->
            ...
            <!--下达日期 -->
            <template v-if="column.dataIndex === 'WorkOrderDate'">
              {{ dayjs(text).format('YYYY-MM-DD') }}
            </template>
            <!--达成率 -->
            <template v-if="column.dataIndex === 'AchievementRate'">
              <a-progress :percent="text" />
              <!-- <a-progress :percent=text v-if="text >=100"/>
                    <a-progress :percent=text v-else-if="text >= 50 && text <100" status="active"/>
                    <a-progress :percent=text v-else/> -->
            </template>
          </template>
        </BasicTable>
      </div>
...
<script lang="ts" setup>
  ...
  import { page, exportSamplingLevelExcel } from '/@/api/productionDeal/planScheduleReport';
  ...
  var isNullList = ['WorkOrderQuantity', 'PlansQuantity', 'GoodProductsQuantity', 'BadPproductsQuantity'];

  const columns: BasicColumn[] = [
    { title: '工单编号', dataIndex: 'WorkOrderNo', align: 'center' },
    { title: '工单数量(K)', dataIndex: 'WorkOrderQuantity', align: 'center' },
    { title: '工单类型', dataIndex: 'WorkOrderTypeStr', align: 'center' },
    { title: '工单下达日期', dataIndex: 'WorkOrderDate', align: 'center', width: 160, format: 'date|YYYY/MM/DD' },
    { title: '产品编号', dataIndex: 'ProductCode', align: 'center', width: 180 },
    { title: '计划单号', dataIndex: 'PlanNumber', align: 'center' },
    { title: '计划数量(K)', dataIndex: 'PlansQuantity', align: 'center' },
    { title: '标准良率', dataIndex: 'StandardYieldRate', align: 'center' },
    { title: '良品数(K)', dataIndex: 'GoodProductsQuantity', align: 'center' },
    { title: '不良品数(K)', dataIndex: 'BadPproductsQuantity', align: 'center' },
    { title: '良率', dataIndex: 'Yield', align: 'center' },
    { title: '达成率', dataIndex: 'AchievementRate', align: 'center' },
    { title: '待完成数量(K)', dataIndex: 'BacklogQuantity', align: 'center' },
    { title: '生产日期', dataIndex: 'ProduceDate', align: 'center', format: 'date|YYYY/MM/DD' },
    { title: '操作人', dataIndex: 'OperatorName', align: 'center', width: 240 },
    { title: '厂区', dataIndex: 'FactoryStr', align: 'center' },
    { title: '入库数', dataIndex: 'InWarehouseQuantity', align: 'center' },

    // { title: '机台号', dataIndex: 'MachineNo',align: 'center',width: 100},
    // { title: '班次', dataIndex: 'ClassesStr',align: 'center',width: 60},
  ];
  ...
  const [registerTable, { getFetchParams, getForm }] = useTable({
    ...
    formConfig: {
      ...
      i18nConfig: {
        moduleCode: 'PlanScheduleReportColumn',
  ...
  function handleExport() {
    ...
    openExportModal(true, {
      ...
      idProps: 'dataIndex',
      i18nConfig: {
        moduleCode: 'PlanScheduleReportColumn', // 字典分类EnCode
      },
    });
  }
