...
<script setup lang="ts">
  ...
  import { addWorkOrder, updateWorkOrder, getInnermaterialnumberlist, getWorkorderbymouldno, getWorkshoplist } from '/@/api/productionDeal/workOrder';
  ...
  const schemas: FormSchema[] = [
    ...
    },
    {
      ...
      label: '备注',
      component: 'Textarea',
      componentProps: { row: 1, autoSize: { minRows: 1, maxRows: 6 } },
      colProps: { span: 12 },
    },
    // {
    //   field: 'deliveryTime',
  ...
  const getTitle = computed(() => (state.dataForm.id ? t('common.editOrder') : t('common.createOrder')));
  ...
  async function init(data) {
    ...
    if (!data.workOrderTypeList) {
      ...
      data.workOrderTypeList = workOrderTypeList;
    }
    // const _arr: any = [];
    // data.workOrderTypeList.forEach(i => {
    //   if (!['3', '4'].includes(i.id)) {
    //     _arr.push({
    //       id: i.id,
    //       fullName: i.fullName,
    //     });
    ...
    // updateSchema({ field: 'workOrderType', componentProps: { options: _arr } });
