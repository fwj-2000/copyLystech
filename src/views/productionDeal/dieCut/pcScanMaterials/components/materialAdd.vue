...
<script setup lang="ts">
  ...
  import {
    ...
    checksubsncode,
  } from '/@/api/productionDeal/pcScanMaterials';
  ...
  const column = [
    { title: 'SN', field: 'snCode', width: 90 },
  ...
  const [Grid, { getFetchParams, remove, insertAt, reloadData, getDataSource }] = useVbenVxeGrid({
    gridOptions: {
      id: 'pcScanMaterials-materialAdd-list',
      columns: column,
      toolbarConfig: {
        enabled: false,
      },
  ...
  const schemas: FormSchema[] = [
    ...
    },
    {
      ...
      ifShow: opt => !opt.values.documentNumber,
      rules: [{ required: false, trigger: 'blur', message: t('component.form.enterOrScan') }],
    },
    {
      field: 'workOrderNo',
  ...
  const snCodeKeydown = async e => {
    ...
    if (e?.target?.value) {
      if (e.keyCode == 13) {
        data = e.target.value;
        if (getFieldsValue().documentNumber) {
  ...
  const subsnCodeKeydown = async e => {
    if (!e) return;
    if (e?.target?.value) {
      if (e.keyCode == 13) {
        data = e.target.value;
        const { code, msg } = await checksubsncode({ subSnCode: data, routeId: state.routeId, routeNodeId: state.routeNodeId });
        if (code === 200) {
          scanData.value = {
            snCode: data,
          };
          insertAt(unref(scanData), -1);
          setFieldsValue({ subsnCode: '' });
        } else {
          createMessage.error(msg);
        }
      }
  ...
  async function getSnCodeInfo(snCode) {
    const { code, data, msg } = await getcurrentdetailbysn({ snCode });
    if (code === 200) {
      const {
        snCode,
        documentNumber,
