    ...
    <ScrollContainer class="bg-[#ebeef5]">
      ...
      <div class="process-block bg-[#fff] p-20px" v-for="(item, index) in processList" :key="item.uuid" @click="handleClickBlock(index)">
        <div class="opera-block">
          ...
          <div class="flex">
            <!-- 添加行 -->
            <AddCustomRows v-if="index === 0 && !state.id" lable="添加数量" @submit="handleAddColumn" />
...
<script lang="ts" setup>
  ...
  //     console.log(val, 'val');
  //     if (!val) return;
  //     getPccversionlistFn(val);
  //   },
  // );

  // watch([() => state.templistParams.process, () => state.wordOrderNo], val => {
  //   if (val.includes('')) return;
  //   // getPccversionlistFn(val);
  //   getRoutenodedetailFn(state.wordOrderNo, state.templistParams.process);
  // });
  ...
  async function handleSubmit() {
    ...
    for (const item of tempFormRef.value) {
      list.push(await item.validateFormFn());
    }
    ...
    const param = processList.value.map((obj, idx) => ({
      factoryArea: state.templistParams.orgId, // 厂区
      ...obj,
...
<style lang="less" scoped>
  ...
  .pcc-preview {
    ...
    &-content {
      ...
      border-radius: 4px;
      height: 600px;
      // background: url('/@/assets/images/process/nodata-bg.png') center/100% 100% no-repeat;
      background: #fff;
      display: flex;
      align-items: top;
