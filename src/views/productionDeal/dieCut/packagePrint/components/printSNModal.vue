...
<script setup lang="ts">
  ...
  const [Grid, { getDataSource, remove, loadData }] = useVbenVxeGrid({
    gridOptions: {
      id: 'productionDeal-dieCut-packagePrint-snList',
      columns: column,
      position: 'modal',
      toolbarConfig: {
        enabled: false,
  ...
  async function getLocalStorageData(isRescan) {
    ...
    const packageLocalStorageData = localStorage.getItem('packageCache');
    const packageReScanLocalStorageData = localStorage.getItem('packageReScanCache');
  ...
  function setLocalstoragePackage(innerMaterialNumber) {
    ...
    if (inCode.value) {
      ...
      localStorage.setItem('packageReScanCache', JSON.stringify(packageLocalStorage));
    } else {
      // 单品打印
      localStorage.setItem('packageCache', JSON.stringify(packageLocalStorage));
    }
  }
  ...
  const handlerInputChangeFn = async (val: string) => {
    ...
    // 根据料号前缀判断料号是否一致
    if (tableList.length && data.innerMaterialNumber !== tableList[0].innerMaterialNumber) return message.error(t('dict.CommonCol.scanConsistentTip'));
  ...
  function removeStorage() {
    if (inCode.value) {
      // 补扫
      localStorage.removeItem('packageReScanCache');
    } else {
      // 单品打印
