...
<script lang="ts" setup>
  ...
  import { getPage, deleteMItem, blukDeleteMItems } from '/@/api/equipment/maintenanceItems';
  ...
  const columns: BasicColumn[] = [
    { title: '保养项编码', dataIndex: 'code' },
    { title: '保养项名称', dataIndex: 'name' },
    { title: '保养项标准', dataIndex: 'standard' },
    { title: '保养方法', dataIndex: 'method' },
  ...
  const [registerTable, { getRowSelection, reload, getForm }] = useTable({
    ...
    formConfig: {
      ...
      i18nConfig: {
        moduleCode: 'MaintenanceItemsColumn',
  ...
  function handleDelete(record) {
    deleteMItem(record.id).then(res => {
  ...
  function handleDeleteList() {
    ...
    if (list?.length === 0) {
      ...
      createConfirm({
        ...
        onOk: async () => {
          try {
            const { code } = await blukDeleteMItems(list);
            if (code == 200) {
              getRowSelection().selectedRowKeys = [];
