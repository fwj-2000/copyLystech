      ...
      <div class="lydc-content-wrapper-content bg-white">
        <Grid>
          <template #toolbar-actions>
            <a-space>
              <!-- <a-button v-auth="'btn_recall'" type="primary" ghost @click="handleRecall">
                {{ t('common.revoke') }}
              </a-button> -->
              <ModelConfirmButton
                v-auth="'btn_recall'"
...
<script lang="ts" setup>
  ...
  import { getNodeList, exportExcel, recallPro, handlePageList } from '/@/api/engineering/sample';
  ...
  const [Grid, gridApi] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'purchase-sampleDemand-processed-list',
      columns: getColumn(),
      api: params => handlePageList({ ...params, isHandle: true }),
      pagerConfig: {
        autoHidden: false,
      },
      beforeFetch: params => {
