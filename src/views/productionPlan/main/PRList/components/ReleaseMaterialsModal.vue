...
<script setup lang="ts">
  ...
  import { getDetailPR, updateonlymaterialPR } from '/@/api/mps/productionPlan/MPS';
  ...
  const [Grid, { reload, validate, getDataSource, reloadData }] = useVbenVxeGrid({
    gridOptions: {
      id: 'productionPlan-main-PRList-list-ReleaseMaterialsModal',
  ...
  async function init({ ids }) {
    changeLoading(true);
    idList.value = ids;
    const { code, data } = await getDetailPR(ids).finally(() => {
      changeLoading(false);
    });
    if (code == 200) {
      reloadData(data);
  ...
  async function handleSubmit() {
    ...
    const res = await updateonlymaterialPR({ ids: idList.value }).finally(() => {
