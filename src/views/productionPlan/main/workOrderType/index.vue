...
<script lang="ts" setup>
  ...
  import { getOrderType, blukDeleteOrderType } from '/@/api/mps/productionPlan/orderType';
  ...
  defineOptions({ name: 'productionPlan-main-workOrderType' });
  ...
  const [Grid, gridApi] = useVbenVxeGrid({
    ...
    gridOptions: {
      id: 'productionPlan-main-workOrderType-list',
      api: getOrderType,
      columns,
      i18nConfig: {
        moduleCode: 'WorkOrderShopColumn',
  ...
  async function handleDelete(row) {
    return blukDeleteOrderType({ ids: [row.id] })
      .then(res => {
