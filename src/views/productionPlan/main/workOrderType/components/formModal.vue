...
<script setup lang="ts">
  ...
  import { addOrderType, updateOrderType, getOrderTypeById } from '/@/api/mps/productionPlan/orderType';
  ...
  async function init({ id }) {
    ...
    if (id) {
      ...
      changeLoading(true);
      const { data, code } = await getOrderTypeById(id).finally(() => {
        changeLoading(false);
      });
      if (code == 200) {
        setFieldsValue(data);
      ...
      // updateSchema({ field: 'type', componentProps: { disabled: false } });
  ...
  async function handleSubmit() {
    ...
    const api = query.id ? updateOrderType : addOrderType;
