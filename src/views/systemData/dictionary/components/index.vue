<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" title="字典分类管理" width="900px" class="full-drawer" destroy-on-close>
    <BasicTable @register="registerTable">
      <template #tableTitle>
        <a-button type="primary" preIcon="icon-ym icon-ym-btn-add" @click="addOrUpdateHandle()">{{ t('common.addText') }}</a-button>
        <lydc-upload-btn url="/api/system/DictionaryData/Actions/Import" accept=".bdd" @on-success="reload"></lydc-upload-btn>
      </template>
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'isTree'">
          <a-tag :color="record.isTree == 1 ? 'success' : 'error'">{{ record.isTree == 1 ? '是' : '否' }}</a-tag>
        </template>
        <template v-if="column.key === 'isLang'">
          <a-tag :color="record.isLang == 1 ? 'success' : 'error'">{{ record.isLang == 1 ? '是' : '否' }}</a-tag>
        </template>
