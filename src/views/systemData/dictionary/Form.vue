...
<script lang="ts" setup>
  ...
  const [registerForm, { setFieldsValue, resetFields, validate, updateSchema, getFieldsValue }] = useForm({
    schemas: [
      ...
      },
      {
        ...
        label: '字典编码',
        component: 'Input',
        componentProps: { placeholder: '采用简写', maxlength: 50 },
        rules: [
          { required: true, trigger: 'blur', message: '必填' },
          { pattern: /^[a-zA-Z0-9_]{1,18}$/, message: '只能输入字母、数字、下划线且小于18个字符' },
        ],
      },
      {
        field: 'sortCode',
    ...
    getDictionaryDataTypeSelector(data.typeId, data.isTree, data.id).then(res => {
      ...
      if (id.value) {
        getInfo(id.value).then(res => {
          ...
          originCode.value = res.data.enCode;
  ...
  async function handleSubmit() {
    ...
    const currentCode = values.enCode;
    if (!(currentCode !== '' && originCode.value == currentCode)) {
