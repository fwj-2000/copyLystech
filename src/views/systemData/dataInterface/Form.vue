  ...
  <BasicPopup
      ...
      <a-col :span="14" :offset="5">
        <BasicForm @register="registerForm">
          <template #hasPage="{ model, field }">
            <LydcSwitch v-model:value="model[field]" :checkedValue="1" :unCheckedValue="0" @change="onHasPageChange" :disabled="hasPageDisabled" />
    ...
    <div class="config h-full" v-if="getShowApiBox()">
      <div class="middle-pane">
          ...
          <a-form-item label="" name="url" :rules="pathRules" class="!mt-20px !mx-20px">
            <a-input v-model:value="apiData.url" placeholder="请输入接口路径">
              <template #addonBefore>
                <div class="!w-80px">
                  <lydc-select v-model:value="apiData.method" :options="methodOptions" />
                </div>
              </template>
            </a-input>
          </a-form-item>
        ...
        <lydc-radio v-if="activeKey === 2" v-model:value="apiData.bodyType" class="!mb-20px !ml-20px" :options="bodyTypeList" @change="onBodyTypeChange" />
