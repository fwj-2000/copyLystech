...
export function getUserInfo() {
  const systemCode = getLydcAppId() ? getLydcAppId().replace('LYDC_APP_', '') : '';
  return defHttp.get({ url: Api.GetUserInfo, data: { systemCode } }, { errorMessageMode: 'none' });
}
// 退出登陆
...
// 获取转办用户列表
export function getTransferUserList() {
  return defHttp.get({ url: '/api/permission/users/all' });
}
