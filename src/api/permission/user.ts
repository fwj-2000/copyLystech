...
// 获取用户下拉框列表
export function getImUserSelector(organizeId = 0, data = {}) {
  return defHttp.post({ url: Api.Prefix + `/ImUser/Selector/${organizeId}`, data });
}
// 获取用户下拉框列表(包括已离职人员)
export function getAllUserSelector(_, data = {}) {
  return defHttp.get({ url: Api.Prefix + `/GetUserAllList`, data });
}
// 获取用户基本信息
...
export function getUserIdByPhone(data) {
  return defHttp.post({ url: Api.Prefix + `/getidbyaccount`, data });
}
// 获取用户下拉框列表
interface UserSelectorListQuery {
  keyword?: string;
  ids?: string[];
  accounts?: [];
  // 以上参数应只填其中一个，否则为"与"的关系
  containsDisabled?: boolean; // 是否返回已离职人员
  withOrganize?: boolean; // 是否返回组织信息
}
export function getUserSelectorList(data: UserSelectorListQuery) {
  return defHttp.post({ url: Api.Prefix + `/getuserlistselector`, data });
}
