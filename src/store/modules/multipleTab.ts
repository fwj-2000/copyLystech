...
export const useMultipleTabStore = defineStore({
  ...
  state: (): MultipleTabState => ({
    ...
    lastDragEndIndex: 0,
    // Track iframe reload counts by route name
    iframeRefreshMap: {},
  }),
  getters: {
    ...
    getIframeRefreshKey() {
      return (name: string) => this.iframeRefreshMap[name] || 0;
    },
  },
  actions: {
    /**
    ...
    async refreshPage(router: Router) {
      ...
      const routeName = typeof name === 'string' ? name : name ? String(name) : '';
      const isFrameRoute = !!route.meta?.frameSrc;

      if (isFrameRoute && routeName) {
        this.iframeRefreshMap[routeName] = (this.iframeRefreshMap[routeName] || 0) + 1;
      }

      const findTab = this.getCachedTabList.find(item => item === routeName);
      if (findTab) {
