    ...
    <ScrollContainer class="config-content condition-content">
      <div class="condition-main">
        <div class="mb-10px" v-if="formConf.conditions?.length">
          ...
          <lydc-select v-model:value="formConf.matchLogic" placeholder="请选择" :options="logicOptions" class="!w-68px" />
        </div>
        <div class="condition-list" v-for="(item, index) in formConf.conditions" :key="index">
          ...
          <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" class="mt-10px">
            <a-col :span="9" class="!flex items-center">
              <lydc-select v-model:value="child.fieldType" :options="conditionTypeOptions" @change="onFieldTypeChange(child)" class="!w-85px" />
              <lydc-select
            ...
            <a-col :span="9" class="!flex items-center">
              <lydc-select v-model:value="child.fieldValueType" :options="conditionTypeOptions1" @change="onFieldValueTypeChange(child)" class="!w-85px" />
              <lydc-select
              ...
              <div class="flex-1 w-150px" v-if="child.fieldValueType === 2">
                ...
                <template v-else-if="child.lydcKey === 'switch'">
                  <lydc-switch v-model:value="child.fieldValue" />
                </template>
                <template v-else-if="child.lydcKey === 'timePicker'">
                  <lydc-time-picker v-model:value="child.fieldValue" :format="child.format || 'HH:mm:ss'" allowClear />
                </template>
                <template v-else-if="['datePicker', 'createTime', 'modifyTime'].includes(child.lydcKey)">
                  <lydc-date-picker v-model:value="child.fieldValue" :format="child.format" allowClear @change="onConditionDateChange($event, child)" />
                </template>
                <template v-else-if="['organizeSelect', 'currOrganize'].includes(child.lydcKey)">
                  <lydc-organize-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionListChange(child, val, data)" />
                </template>
                <template v-else-if="['depSelect'].includes(child.lydcKey)">
                  <lydc-dep-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="child.lydcKey === 'roleSelect'">
                  <lydc-role-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="child.lydcKey === 'groupSelect'">
                  <lydc-group-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="['posSelect', 'currPosition'].includes(child.lydcKey)">
                  <lydc-pos-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="['userSelect', 'createUser', 'modifyUser'].includes(child.lydcKey)">
                  <lydc-user-select v-model:value="child.fieldValue" hasSys allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
                <template v-else-if="['usersSelect'].includes(child.lydcKey)">
                  <lydc-users-select v-model:value="child.fieldValue" allowClear @change="(val, data) => onConditionObjChange(child, val, data)" />
                </template>
      ...
      <a-form :colon="false" layout="vertical" :model="formConf" class="config-form !m-10px" v-if="formConf.conditionType === 1">
        <a-form-item label="转向节点">
          <lydc-select v-model:value="formConf.swerveNode" placeholder="请选择" :options="beforeNodeOptions" showSearch @change="onSwerveNodeChange" />
        </a-form-item>
      </a-form>
    </ScrollContainer>
...
<script lang="ts" setup>
  ...
  function onFieldValueChange(item, val, data) {
    ...
    item.fieldValueLydcKey = val ? data.__config__.lydcKey : '';
  }
