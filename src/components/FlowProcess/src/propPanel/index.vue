<template>
  <BasicDrawer
    ...
    @open-change="onDrawerVisibleChange"
    showFooter
    :maskClosable="false"
    @ok="handleOk">
    <template #title>
...
<script lang="ts" setup>
  ...
  // 所有表单字段
  const formFieldsOptions = computed(() => state.formFieldList.filter(o => o.__config__.lydcKey !== 'table').map(o => ({ ...o, disabled: false })));
  // 不包含子表的表单字段
  ...
  const requiredDisabled = lydcKey => {
    return ['billRule', 'createUser', 'createTime', 'modifyTime', 'modifyUser', 'currPosition', 'currOrganize', 'table'].includes(lydcKey);
  };
  const getDataType = data => {
    if (!data.__config__ || !data.__config__.lydcKey) return '';
    const lydcKey = data.__config__.lydcKey;
    if (['inputNumber', 'datePicker', 'rate', 'slider'].includes(lydcKey)) {
      return 'number';
    } else if (['checkbox', 'uploadFile', 'uploadImg', 'cascader', 'organizeSelect', 'areaSelect'].includes(lydcKey)) {
      return 'array';
    } else if (
    ...
    if (!formOperates.length || isUpdate) {
      for (let i = 0; i < state.formFieldList.length; i++) {
        ...
        res.push({
          ...
          requiredDisabled: requiredDisabled(data.__config__.lydcKey) || data.__config__.required,
          lydcKey: data.__config__.lydcKey,
  ...
  function getConditionNodeFieldList() {
    ...
    if (!state.prevNodeList.length) {
      ...
      state.formFieldList = formFieldList.filter(o => o.__config__.lydcKey !== 'table');
    }
  }
  // 获取上一节数据
