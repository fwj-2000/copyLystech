  ...
  function transformFieldList(formFieldList) {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && data.__config__.lydcKey !== 'table') {
        ...
        if (dyOptionsList.includes(config.lydcKey) && config.dataType !== 'static') delete item.options;
        list.push(item);
      }
    ...
    outer: for (let i = 0; i < list.length; i++) {
      ...
      for (let j = 0; j < e.groups.length; j++) {
        ...
        if (
          ...
          (!['inputNumber', 'calculate'].includes(child.lydcKey) || (['inputNumber', 'calculate'].includes(child.lydcKey) && child.fieldValue !== 0))
        ) {
