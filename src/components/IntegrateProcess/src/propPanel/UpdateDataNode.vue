    ...
    <ScrollContainer class="config-content">
      <a-form :colon="false" layout="vertical" :model="formConf" class="config-form">
        <a-form-item label="目标表单">
          ...
          <lydc-select v-model:value="formConf.flowId" :options="flowOptions" placeholder="请选择流程" showSearch class="!mt-10px" v-if="formConf.enableFlow" />
        </a-form-item>
        <a-form-item label="更新字段" class="!flex-nowrap">
          <div class="condition-main">
            ...
            <a-row :gutter="8" v-for="(item, index) in formConf.transferList" :key="index" class="mt-10px">
              ...
              <a-col :span="5">
                <lydc-select v-model:value="item.sourceType" :options="sourceTypeOptions" @change="item.sourceValue = ''" />
              </a-col>
              <a-col :span="8">
                <lydc-select
        ...
        <a-form-item class="!flex-nowrap" v-if="integrateType != 1">
          ...
          <div class="condition-main">
            <div class="mb-10px" v-if="formConf.ruleList?.length">
              ...
              <lydc-select v-model:value="formConf.ruleMatchLogic" placeholder="请选择" :options="logicOptions" class="!w-68px" />
            </div>
            <div class="condition-list" v-for="(item, index) in formConf.ruleList" :key="index">
              ...
              <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" class="mt-10px">
                ...
                <a-col :span="5">
                  <lydc-select class="w-full" v-model:value="child.symbol" placeholder="运算符号" :options="symbolOptions" disabled />
                </a-col>
                <a-col :span="10">
                  <lydc-select
        ...
        <a-form-item>
          ...
          <lydc-radio v-model:value="formConf.unFoundRule" :options="unFoundRuleOptions" />
        </a-form-item>
      </a-form>
    </ScrollContainer>
  </section>
...
<script lang="ts" setup>
  ...
  const emptyChildItem = { field: '', symbol: '==', fieldValueType: 1, fieldValue: undefined, lydcKey: '', fieldValueLydcKey: '', cellKey: +new Date() };
  const emptyItem = { logic: 'and', groups: [emptyChildItem] };

  const getRuleOptions = computed(() => props.formConf.formFieldList.filter(o => o.id.indexOf('-') < 0 && !notSupportList.includes(o.__config__.lydcKey)));
