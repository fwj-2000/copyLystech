      ...
      <a-form :colon="false" layout="vertical" :model="formConf" class="config-form" v-show="activeKey === '1'">
        <template v-if="integrateType == 1">
          ...
          <a-form-item label="触发事件">
            <lydc-radio v-model:value="formConf.triggerEvent" :options="triggerEventOptions" />
          </a-form-item>
          <a-form-item>
        ...
        <template v-if="integrateType == 2">
          <a-form-item label="触发开始时间">
            <lydc-date-picker v-model:value="formConf.startTime" format="YYYY-MM-DD HH:mm:ss" placeholder="选择开始时间" />
          </a-form-item>
          <a-form-item label="触发规则">
            <a-form-item>
              <div class="ant-form-item-label"><label class="ant-form-item-no-colon">Cron表达式</label></div>
              <lydc-cron v-model:value="formConf.cron" />
            </a-form-item>
          </a-form-item>
          <a-form-item label="触发结束时间">
            <lydc-select v-model:value="formConf.endTimeType" :options="endTimeTypeOptions" />
            <a-input-number
        ...
        <template v-if="integrateType == 3">
          <a-form-item label="webhook URL">
            ...
            <a-input :value="getSysConfig.lydcDomain + formConf.webhookUrl" readonly>
              <template #addonAfter>
                <span class="cursor-pointer" @click="handleCopy(getSysConfig.lydcDomain + formConf.webhookUrl)">复制链接</span>
              </template>
            </a-input>
          </a-form-item>
      ...
      <div v-show="activeKey === '2'">
        <a-form :colon="false" :model="formConf" layout="vertical" class="config-form">
          <a-form-item label="通知人">
            <lydc-select v-model:value="formConf.msgUserType" :options="msgUserOptions" multiple />
          </a-form-item>
          <a-form-item v-if="formConf.msgUserType.includes(3)">
            ...
            <lydc-users-select v-model:value="formConf.msgUserIds" multiple />
          </a-form-item>
          <a-form-item>
            <template #label>执行失败<BasicHelp text="执行失败时发送通知！" /></template>
            <lydc-select v-model:value="formConf.failMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
          ...
          <a-form-item>
            ...
            <lydc-select v-model:value="formConf.startMsgConfig.on" :options="noticeOptions" />
          </a-form-item>
