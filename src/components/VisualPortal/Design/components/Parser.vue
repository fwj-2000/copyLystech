<template>
  <div class="h-full" @click.stop="handleClick(item)">
    <component :is="layouts" v-bind="getBindValue" v-if="item.lydcKey !== 'card' && item.lydcKey !== 'tab'" />
    <template v-if="item.lydcKey === 'card'">
    ...
    <template v-if="item.lydcKey === 'tab'">
      <a-tabs
...
<script lang="ts" setup>
  ...
  if (props.item.lydcKey !== 'card') layouts.value = markRaw(defineAsyncComponent(() => getComponents(props.item.lydcKey)));
  ...
  const getBindValue = computed(() => ({
    ...
    key: useNeedKeyList.includes(props.item.lydcKey) ? props.item.renderKey : '',
  }));
