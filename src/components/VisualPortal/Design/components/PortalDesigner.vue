        ...
        <div v-if="showType == 'pc'">
          <grid-layout :layout.sync="layout" :row-height="40" v-if="layout.length">
            <grid-item
              ...
              <div class="mask" v-if="!noNeedMaskList.includes(item.lydcKey)"></div>
                ...
                <draggable v-if="layout.length" class="drawing-board" :list="layout" :animation="340" group="componentsGroup" item-key="i">
                  <template #item="{ element }">
                    <div class="item-box m-10px portal-tabs-box" @click="handleClick(element)" :class="{ 'active-item': element.i === activeId }">
                      ...
                      <div class="mask" v-if="!noNeedMaskList.includes(element.lydcKey)"></div>
...
<script lang="ts" setup>
  ...
  function handleAddComponent(item, type?, currentItem?) {
    ...
    let row = { ...clone, i, x, y };
    if (needDefaultList.includes(row.lydcKey) && type != 'copy') row.option.defaultValue = getDefaultValue(row);
    if (currentItem) {
      if (currentItem.lydcKey == 'card') currentItem.children = [row];
      if (currentItem.lydcKey == 'tab') {
  ...
  function getDefaultValue(row) {
    const lydcKey = row.lydcKey;
    if (lydcKey == 'text') return 'DC Manage System';
    if (lydcKey == 'image') return 'https://app.cdn.lydcsoft.com/image/3.2/banner1.png';
    if (lydcKey == 'video') return 'https://cdn.lydcsoft.com/2022/video/index_video.mp4';
    if (lydcKey == 'barChart' || lydcKey == 'lineChart' || lydcKey == 'pieChart') return chartData.baseBarData;
    if (lydcKey == 'radarChart') return chartData.radarData;
    if (lydcKey == 'mapChart') return mapChartData;
    if (lydcKey == 'rankList') return rankList;
    if (lydcKey == 'timeAxis') return timeAxisList;
    if (lydcKey == 'tableList') return tableList;
  }
    ...
    const loop = data => {
      if (data.children && item.children.length) {
        data.children.map(ele => {
          if (ele.lydcKey) emitter.emit('eChart' + ele.i);
    ...
    return new Promise((resolve, reject) => {
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if ((e.lydcKey == 'video' || e.lydcKey == 'image') && option.styleType == 2) {
            const val = e.lydcKey == 'video' ? '视频' : '图片';
