<template>
  <BasicModal
    ...
    :keyboard="false"
    destroyOnClose
    class="lydc-full-modal full-modal lydc-preview-portal-modal">
    <template #title>
  ...
  function filterList(layout) {
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        const item = list[i];
        if (!(Array.isArray(item.visibility) && item.visibility.includes('pc')) && item.lydcKey) {
          list.splice(i, 1);
          i--;
        }
