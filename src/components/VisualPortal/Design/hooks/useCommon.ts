...
export function useCommon(activeData) {
  ...
  const getLydcKey = computed(() => activeData.lydcKey);

  watch(
    () => unref(getOption).styleType,
    val => {
      if (unref(getLydcKey) == 'rankList' && (val == 3 || val == 4)) handleRankList();
    },
  );
  watch(
    () => activeData.dataType,
    () => {
      if (arrayList.includes(unref(getLydcKey))) {
  ...
  watch(
    ...
    val => {
      ...
      if (unref(getLydcKey) == 'rankList' && (getOption.value.styleType == 3 || getOption.value.styleType == 4)) handleRankList();
      if (unref(getLydcKey) == 'timeAxis') handleTimeAxis();
      if (unref(getLydcKey) == 'video' && getOption.value.styleType == 1 && val.url) state.value = globSetting.apiUrl + val.url;
    },
    ...
    val => {
      ...
      getDataInterfaceRes(val, query).then(res => {
        if (arrayList.includes(unref(getLydcKey))) {
          ...
          if (unref(getLydcKey) == 'rankList' && (unref(getOption).styleType == 3 || unref(getOption).styleType == 4)) handleRankList();
          if (unref(getLydcKey) == 'timeAxis') handleTimeAxis();
        } else {
  ...
  function handleData(val) {
    ...
    if (unref(getLydcKey) == 'rankList' && (option.styleType == 3 || option.styleType == 4)) handleRankList();
    if (unref(getLydcKey) == 'timeAxis') handleTimeAxis();
    if (unref(getLydcKey) == 'video' && option.styleType == 1 && val.url) state.value = globSetting.apiUrl + val.url;
  }
