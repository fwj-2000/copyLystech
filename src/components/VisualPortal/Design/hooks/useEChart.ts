...
export function useEChart(activeData, chartRef) {
  ...
  function initChartData() {
    const lydcKey = activeData.lydcKey;
    ...
    typeArr.forEach((title, index) => {
      ...
      let obj = {
        name: title,
        type: type,
        stack: styleType == 5 || styleType == 6 || styleType == 7 || (styleType != 4 && lydcKey == 'lineChart') ? title : 'total',
      };
      ...
      series: (() => {
        const list: any[] = (seriesData || []).map((ele: any, index) => {
          let label: any = {
            ...
            position: lydcKey == 'pieChart' ? option.seriesLabelPosition : 'top',
          ...
          if (lydcKey == 'barChart' || lydcKey == 'lineChart' || lydcKey == 'radarChart') itemStyle.color = getColor(index);
          if (lydcKey == 'lineChart') {
    ...
    if (lydcKey == 'barChart' || lydcKey == 'lineChart') {
      if (lydcKey == 'lineChart' && option.areaStyle) xAxis.boundaryGap = false;
