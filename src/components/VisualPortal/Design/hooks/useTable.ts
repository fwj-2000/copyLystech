...
export function useTable(activeData) {
  ...
  function initData() {
    ...
    if (activeData.lydcKey === 'tableList') {
      ...
      getNoticeList({ typeList }).then(res => {
        ...
        list.value.map(o => {
          o.creatorTime = dayjs(o.creatorTime).tz().format('YYYY-MM-DD HH:mm:ss');
          o.releaseTime = dayjs(o.releaseTime).tz().format('YYYY-MM-DD HH:mm:ss');
        });
        handleData(2);
      });
    }
