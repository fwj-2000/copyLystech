<template>
  <BasicModal v-bind="$attrs" width="1000px" class="lydc-modal-portal" @register="registerModal" title="选项设置" @ok="handleSubmit" destroyOnClose>
    <a-table :data-source="state.list" :columns="getColumns" size="small" :pagination="false" rowKey="id" :scroll="{ x: 1200, y: 478 }" class="carousel-table">
      ...
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.key === 'icon'">
          <LydcIconPicker v-model:value="record.icon" />
        </template>
        <template v-if="column.key === 'iconColor'">
          <lydc-color-picker v-model:value="record.iconColor" size="small" color-format="rgb" />
        </template>
        <template v-if="column.key === 'linkType'">
          <div class="flex items-center">
            <lydc-select class="!w-160px !mr-10px" v-model:value="record[column.key]" :options="linkTypeList" placeholder="请选择" showSearch allowClear />
            <div class="flex-1">
              <lydc-tree-select
              ...
              <a-input v-if="record.linkType == 2" v-model:value="record.urlAddress" placeholder="请输入">
                <template #addonAfter v-if="state.showType == 'pc'">
                  <lydc-select v-model:value="record.linkTarget" :options="typeList" placeholder="请选择" class="!w-90px" />
                </template>
              </a-input>
            </div>
          </div>
