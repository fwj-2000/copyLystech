<template>
  <a-collapse-panel>
    ...
    <a-form-item label="数据类型">
      <lydc-radio v-model:value="activeData.dataType" :options="dataTypeOptions" optionType="button" buttonStyle="solid" @change="onDataTypeChange" />
    </a-form-item>
    ...
    <div v-if="activeData.option.styleType == 7 && activeData.lydcKey == 'barChart'" class="my-20px">
      <a-table :data-source="activeData.option.barType" :columns="columns" size="small" :pagination="false">
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'type'">
            <lydc-select v-model:value="record[column.key]" placeholder="请选择" :options="barTypeList" showSearch />
          </template>
        </template>
      </a-table>
    </div>
