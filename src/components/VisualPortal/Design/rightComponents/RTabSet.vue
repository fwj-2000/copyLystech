<template>
  <a-collapse-panel>
    ...
    <a-form-item label="风格类型" v-if="showType === 'pc'">
      <lydc-radio v-model:value="activeData.type" :options="typeOptions" optionType="button" button-style="solid" class="right-radio" />
    </a-form-item>
    <a-form-item label="选项卡位置" v-if="showType === 'pc'">
      <lydc-radio v-model:value="activeData.tabPosition" :options="positionOptions" optionType="button" button-style="solid" class="right-radio" />
    </a-form-item>
      ...
      <draggable v-model="activeData.children" :animation="300" group="selectItem" handle=".option-drag" itemKey="name">
        <template #item="{ element, index }">
          <div class="select-item">
            ...
            <LydcIconPicker class="!w-130px" v-model:value="element.icon" v-show="showType === 'pc'" />
