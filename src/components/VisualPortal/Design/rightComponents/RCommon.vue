<template>
  <a-collapse-panel key="1">
    ...
    <a-form-item v-if="!noStyleTypeList.includes(activeData.lydcKey)">
      <template #label>风格类型<BasicHelp v-if="!!getTipLabel()" :text="getTipLabel()" /></template>
      <lydc-select v-model:value="activeData.option.styleType" :options="getStyleTypeOptions" placeholder="请选择" showSearch @change="onStyleTypeChange" />
    </a-form-item>
    <template v-if="activeData.lydcKey == 'lineChart'">
    ...
    <template v-if="activeData.lydcKey == 'pieChart'">
    ...
    <template v-if="activeData.lydcKey == 'radarChart'">
    ...
    <template v-if="activeData.lydcKey == 'timeAxis'">
      <a-form-item>
        <template #label>标签布局<BasicHelp text="APP不支持所有交错类型，默认显示轴右侧" /></template>
        <lydc-select v-model:value="activeData.option.layout" :options="layoutStyleList" placeholder="请选择" showSearch />
      </a-form-item>
      <a-form-item label="排序">
        <lydc-radio v-model:value="activeData.option.sortable" :options="sortList" optionType="button" buttonStyle="solid" />
      </a-form-item>
    ...
    <template v-if="activeData.lydcKey == 'text'">
      <a-form-item label="文本类型">
        <lydc-radio v-model:value="activeData.option.styleType" :options="textStyleList" optionType="button" buttonStyle="solid" @change="textChange" />
      </a-form-item>
      <a-form-item label="数据类型">
        <lydc-radio v-model:value="activeData.dataType" :options="dataTypeList" optionType="button" buttonStyle="solid" @change="dataTypeChange" />
      </a-form-item>
      ...
      <template v-if="activeData.option.styleType != 2">
        ...
        <a-form-item label="文本颜色">
          <lydc-color-picker v-model:value="activeData.option.textFontColor" size="small" />
        </a-form-item>
        <a-form-item label="文本位置">
          <lydc-radio v-model:value="activeData.option.textLeft" :options="alignList" optionType="button" buttonStyle="solid" class="right-radio" />
        </a-form-item>
        <a-form-item label="文本下划线">
          <lydc-radio v-model:value="activeData.option.textUnderLine" :options="underLineList" optionType="button" buttonStyle="solid" />
        </a-form-item>
      </template>
      ...
      <a-form-item label="背景色">
        <lydc-color-picker v-model:value="activeData.option.textBgColor" size="small" />
      </a-form-item>
      <Link :activeData="activeData" :menuList="menuList" :appMenuList="appMenuList" :showType="showType" />
    </template>
    <template v-if="activeData.lydcKey == 'image'">
      ...
      <a-form-item label="上传图片" v-if="activeData.option.styleType == 1">
        <LydcUploadImgSingle v-model:value="activeData.option.defaultValue" />
      </a-form-item>
      ...
      <a-form-item label="填充方式">
        <lydc-select v-model:value="activeData.option.imageFillStyle" :options="fillStyleList" placeholder="请选择" showSearch />
      </a-form-item>
    ...
    <template v-if="activeData.lydcKey == 'carousel'">
      ...
      <template v-if="showType == 'pc'">
        <a-form-item label="箭头显示" v-if="activeData.option.carouselDirection == 'horizontal'">
          <lydc-radio v-model:value="activeData.option.carouselArrow" :options="arrowList" optionType="button" buttonStyle="solid" />
        </a-form-item>
      </template>
    ...
    <template v-if="activeData.lydcKey == 'video'">
      ...
      <a-form-item label="上传视频" v-if="activeData.option.styleType == '1'">
        <LydcUploadFile v-model:value="fileList" :limit="1" buttonText="上传视频" accept="video/*" fileSize="100" @change="onUploadFileChange" />
      </a-form-item>
      <a-form-item v-else-if="activeData.option.styleType == 2">
      ...
      <a-form-item label="播放频率">
        <lydc-radio v-model:value="activeData.option.playNumber" :options="playNumberList" optionType="button" buttonStyle="solid" class="right-radio-more" />
      </a-form-item>
      <a-form-item label="静音播放">
        <a-switch v-model:checked="activeData.option.mutePlay" />
      </a-form-item>
    </template>
    <template v-if="activeData.lydcKey == 'iframe'">
      <a-form-item>
        <template #label>链接地址<BasicHelp text="地址以http://或https://为开头" /></template>
        <a-input v-model:value="activeData.option.defaultValue" placeholder="请输入" />
      </a-form-item>
    </template>
    <template v-if="activeData.lydcKey == 'todo'">
      <template v-if="showType == 'app'">
        <a-form-item label="风格类型">
          <lydc-select v-model:value="activeData.option.styleType" :options="appTodoStyleList" placeholder="请选择" showSearch />
        </a-form-item>
        <template v-if="activeData.option.styleType == 2">
      ...
      <a-form-item label="名称颜色">
        <lydc-color-picker v-model:value="activeData.option.labelFontColor" size="small" />
      </a-form-item>
      ...
      <a-form-item label="数值颜色">
        <lydc-color-picker v-model:value="activeData.option.valueFontColor" size="small" />
      </a-form-item>
      <a-form-item label="数值背景色" v-if="showType == 'app'">
        <lydc-color-picker v-model:value="activeData.option.valueFontBgColor" size="small" />
      </a-form-item>
    </template>
    <template v-if="activeData.lydcKey == 'commonFunc'">
      <template v-if="showType == 'app'">
        <a-form-item label="风格类型">
          <lydc-select v-model:value="activeData.option.appStyleType" :options="appTodoStyleList" placeholder="请选择" showSearch />
        </a-form-item>
      ...
      <a-form-item label="图标位置" v-if="showType == 'pc'">
        <lydc-radio v-model:value="activeData.option.styleType" :options="commonFuncStyleList" optionType="button" buttonStyle="solid" />
      </a-form-item>
      <a-form-item label="名称大小">
        <a-input-number v-model:value="activeData.option.labelFontSize" placeholder="请输入" :min="12" :max="25" />
      </a-form-item>
    ...
    <template v-if="activeData.lydcKey == 'dataBoard'">
      <template v-if="showType == 'app'">
        <a-form-item label="风格类型">
          <lydc-select v-model:value="activeData.option.appStyleType" :options="appTodoStyleList" placeholder="请选择" showSearch />
        </a-form-item>
      ...
      <a-form-item label="数据类型">
        <lydc-radio v-model:value="activeData.dataType" :options="dataTypeList" optionType="button" button-style="solid" @change="dataBoardDataTypeChange" />
      </a-form-item>
      ...
      <a-form-item label="单位颜色">
        <lydc-color-picker v-model:value="activeData.option.unitFontColor" size="small" />
      </a-form-item>
    </template>
...
<script lang="ts" setup>
  ...
  const getStyleTypeOptions = computed(() => {
    const lydcKey = props.activeData.lydcKey;
    if (lydcKey == 'barChart') return JSON.parse(JSON.stringify(barStyleList));
    if (lydcKey == 'lineChart') return lineStyleList;
    if (lydcKey == 'pieChart') return pieStyleList;
    if (lydcKey == 'radarChart') return radarStyleList;
    if (lydcKey == 'rankList') return rankStyleList;
    if (lydcKey == 'timeAxis') return timeStyleList;
    if (lydcKey == 'image') return imageStyleList;
    if (lydcKey == 'video') return videoStyleList;
    if (lydcKey == 'mapChart') return mapStyleList;
  });
  ...
  function getTitle() {
    const lydcKey = props.activeData.lydcKey;
    if (lydcKey == 'barChart') return '柱状图设置';
    if (lydcKey == 'lineChart') return '折线图设置';
    if (lydcKey == 'pieChart') return '饼图设置';
    if (lydcKey == 'radarChart') return '雷达图设置';
    if (lydcKey == 'rankList') return '排行榜设置';
    if (lydcKey == 'timeAxis') return '时间轴设置';
    if (lydcKey == 'text') return '文本设置';
    if (lydcKey == 'image') return '图片设置';
    if (lydcKey == 'carousel') return '轮播图设置';
    if (lydcKey == 'video') return '视频设置';
    if (lydcKey == 'iframe') return 'Iframe设置';
    if (lydcKey == 'mapChart') return '地图设置';
    if (lydcKey == 'todo') return '待办设置';
    if (lydcKey == 'commonFunc') return '常用功能设置';
    if (lydcKey == 'dataBoard') return '数据面板设置';
  }
  function getTipLabel() {
    const lydcKey = props.activeData.lydcKey;
    if (lydcKey == 'barChart') return 'APP不支持背景类型，默认显示基础类型';
    if (lydcKey == 'lineChart') return 'APP不支持堆叠类型，默认显示基础类型';
    if (lydcKey == 'mapChart') return 'APP不支持柱形和柱形排名类型，默认显示散点类型';
    return '';
  }
  function onStyleTypeChange(val) {
    const lydcKey = props.activeData.lydcKey;
    if (lydcKey == 'image' || lydcKey == 'video') {
  ...
  function initData() {
    state.fileList = [];
    if (props.activeData.lydcKey == 'video' && props.activeData.option.defaultValue && props.activeData.option.styleType == 1) {
  ...
  function textChange() {
    props.activeData.option.defaultValue = '';
    if (props.activeData.dataType == 'static') props.activeData.option.defaultValue = 'DC Manage System';
  }
