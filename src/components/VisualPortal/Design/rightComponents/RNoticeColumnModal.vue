    ...
    <a-table :data-source="state.list" :columns="state.columns" size="small" :pagination="false" rowKey="id" class="carousel-table">
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'fixed'">
          <lydc-select v-model:value="record.fixed" :options="fixedOptions" placeholder="请选择" showSearch v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
        <template v-if="column.key === 'align'">
          <lydc-select v-model:value="record.align" :options="alignOptions" placeholder="请选择" showSearch v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
        ...
        <template v-if="column.key === 'fontColor'">
          <lydc-color-picker v-model:value="record.fontColor" size="small" v-if="record.filedName !== 'classify'" />
          <span v-else></span>
        </template>
      </template>
    </a-table>
