<template>
  <a-collapse-panel>
    ...
    <template v-if="activeData.lydcKey != 'pieChart' && activeData.lydcKey != 'radarChart' && showType == 'pc'">
      <a-form-item label="文字颜色">
        <lydc-color-picker v-model:value="activeData.option.AxisTextStyleColor" size="small" />
      </a-form-item>
      <a-form-item label="轴线颜色">
        <lydc-color-picker v-model:value="activeData.option.AxisLineStyleColor" size="small" />
      </a-form-item>
    </template>
    <a-table :data-source="activeData.option.colorList" :columns="getColumns" :pagination="false" size="small">
      <template #bodyCell="{ column, record, index }">
        <template v-if="column.key === 'color1' || column.key === 'color2'">
          <lydc-color-picker v-model:value="record[column.key]" size="small" />
        </template>
