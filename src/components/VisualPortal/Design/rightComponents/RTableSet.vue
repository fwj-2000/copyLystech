  ...
  <a-collapse-panel>
    ...
    <template v-if="showType == 'pc'">
      <a-form-item label="风格类型">
        <lydc-select v-model:value="activeData.option.styleType" :options="styleTypeOptions" placeholder="请选择" showSearch @change="renderKeyChange" />
      </a-form-item>
      <template v-if="activeData.option.styleType == 1">
        ...
        <template v-if="activeData.option.showHeader">
          ...
          <a-form-item label="字体颜色">
            <lydc-color-picker v-model:value="activeData.option.headerFontColor" size="small" />
          </a-form-item>
          <a-form-item label="字体位置">
            <lydc-radio v-model:value="activeData.option.headerLeft" :options="alignList" optionType="button" button-style="solid" class="right-radio" />
          </a-form-item>
          <a-form-item label="背景色">
            <lydc-color-picker v-model:value="activeData.option.headerBgColor" size="small" />
          </a-form-item>
        </template>
        ...
        <a-form-item label="字体颜色">
          <lydc-color-picker v-model:value="activeData.option.tableFontColor" size="small" />
        </a-form-item>
      </template>
      ...
      <a-form-item label="背景色">
        <lydc-color-picker v-model:value="activeData.option.tableBgColor" size="small" />
      </a-form-item>
      <a-form-item label="奇行颜色">
        <lydc-color-picker v-model:value="activeData.option.tableOddLineColor" size="small" />
      </a-form-item>
      <a-form-item label="偶行颜色">
        <lydc-color-picker v-model:value="activeData.option.tableEvenLineColor" size="small" />
      </a-form-item>
      <a-divider v-if="activeData.option.styleType == 1">列数据设置</a-divider>
      <a-form-item label="数据类型">
        <lydc-radio v-model:value="activeData.dataType" :options="dataTypeList" optionType="button" button-style="solid" @change="dataTypeChange" />
      </a-form-item>
    ...
    <template v-else>
      ...
      <template v-if="activeData.option.showName">
        ...
        <a-form-item label="名称颜色">
          <lydc-color-picker v-model:value="activeData.option.nameFontColor" size="small" />
        </a-form-item>
      </template>
      ...
      <a-form-item label="数据颜色">
        <lydc-color-picker v-model:value="activeData.option.dataFontColor" size="small" />
      </a-form-item>
    </template>
