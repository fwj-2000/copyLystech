  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="t('common.detailText')">
    <a-form class="!mx-20px" :colon="false" :model="dataForm" ref="formElRef" :labelCol="{ style: { width: '80px' } }">
      ...
      <a-form-item label="附件">
        <LydcUploadFile v-model:value="dataForm.files" disabled detailed />
      </a-form-item>
      <a-form-item label="开始时间">
        <p>{{ dayjs(dataForm.startDay).tz().format('YYYY-MM-DD HH:mm:ss') }} </p>
      </a-form-item>
      <a-form-item label="结束时间">
        <p>{{ dayjs(dataForm.endDay).tz().format('YYYY-MM-DD HH:mm:ss') }} </p>
      </a-form-item>
