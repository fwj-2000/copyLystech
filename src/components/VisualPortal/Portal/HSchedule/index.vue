...
<script lang="ts" setup>
  ...
  function handleDateClick(data) {
    let startTime = dayjs(data.date).tz().format('YYYY-MM-DD HH:00');
    ...
    if (clickTime == currTime) {
      ...
      startTime = dayjs(thisDate).tz().format('YYYY-MM-DD HH:00');
    }
  ...
  function datesRender(calendar) {
    ...
    state.startTime = dayjs(view.activeStart).tz().format('YYYY-MM-DD HH:mm');
    state.endTime = dayjs(view.activeEnd).tz().format('YYYY-MM-DD HH:mm');
    initData();
  }
  function initData() {
    let query = {
