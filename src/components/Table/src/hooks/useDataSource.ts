...
export function useDataSource(
  ...
  async function fetch(opt?: FetchParams) {
    ...
    const { api, searchInfo, defSort, fetchSetting, beforeFetch, afterFetch, useSearchForm, pagination, tableSetting, deleteMark = 0 } = unref(propsRef);
    ...
    try {
      ...
      let params: Recordable = merge(
        ...
        opt?.sortInfo ?? {},
        opt?.filterInfo ?? {},
        tableSetting?.delStatus ? { deleteMark } : {},
      );
      if (beforeFetch && isFunction(beforeFetch)) {
        params = (await beforeFetch(params)) || params;
      }
      // 当不需要获取删除数据时，不传值
      if (params.deleteMark == 0) {
        delete params.deleteMark;
      }
