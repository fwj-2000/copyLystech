...
import { SCROLL_WITH_MOUSE } from '../const';
...
export function useTableScroll(
  ...
  function handleMouseMove(e) {
    const { scrollWithMouse = SCROLL_WITH_MOUSE } = unref(propsRef);
    if (!scrollWithMouse) return;
    ...
    if (mouseFlag) {
      // 设置水平方向的元素的位置
      bodyEl.scrollLeft -= -mouseOffset + (mouseOffset = e.clientX);
      // 同理可设置垂直方向的元素的位置
      // bodyEl.scrollTop -= (- mouseOffset + (mouseOffset = e.clientY));
    }
  }

  return { getScrollRef, redoHeight, handleMouseDown, handleMouseUp, handleMouseMove };
}
