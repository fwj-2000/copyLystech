...
export interface BasicTableProps<T = any> {
  ...
  deleteMark?: string | number | boolean;
  // 国际化配置
  i18nConfig: {
    moduleCode: string;
  };
...
export interface BasicColumn extends ColumnProps<Recordable> {
   ...
   * 格式化
   * 1.表格tag组件
   *   eg: STATUS_OPTIONS = [id: 1, fullName: '待处理', theme:'green', color: '#52C41A', rowKey: 'StatusDesc']
   *       format: 'tag|'+JSON.stringify(STATUS_OPTIONS)
   *   id<number>: 该字段的值，eg status的值、enalbel的值
   *   theme<'green'|'blue'|'yellow'|'red'|'purple'|'gray'>: 内置样式
   *   color<hex>: 颜色值，支持自定义内置颜色以外的色值，该色值为字体颜色，背景色会基于字体颜色淡化。
   *   fullName<string>: 自定义该表格要显示的值
   *   rowKey：显示其他字段的值
   * 2. 日期格式化  eg. 'date| YYYY/MM/DD'
   * 3. CallbackFunction
   */
  format?: CellFormat;
  ...
  editRender?: (opt: {
    ...
    column: BasicColumn;
    index: number;
  }) => Promise<VNodeChild | JSX.Element>;
  // 动态 Disabled
  editDynamicDisabled?: boolean | ((record: Recordable) => boolean);
  i18nField?: string; // 国际化配置字段
}
