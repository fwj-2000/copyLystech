<template>
  <div class="column-design-container">
    <div class="main-board">
      <lydc-group-title content="查询字段" :bordered="false" />
      <a-table :data-source="columnData.searchList" :columns="searchColumns" size="small" :pagination="false" rowKey="id" class="search-table">
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-if="column.key === 'lydcKey'">
            <lydc-select
              v-model:value="record.lydcKey"
              :options="viewLydcKeyOptions"
              :disabled="record.__config__.isFromParam"
              class="!w-160px"
              @change="onLydcKeyChange($event, record, index)" />
            <template v-if="canSetAttrs.includes(record.lydcKey) && !record.__config__.isFromParam">
          ...
          <template v-if="column.key === 'searchType'">
            <lydc-select
              ...
              :disabled="!['input', 'textarea'].includes(record.lydcKey)"
              v-if="!record.__config__.isFromParam" />
            <span v-else></span>
          </template>
          <template v-if="column.key === 'value'">
            <template v-if="showInputList.includes(record.lydcKey)">
              <a-input v-model:value="record.value" placeholder="请输入" allowClear />
            </template>
            <template v-if="record.lydcKey === 'inputNumber'">
              <lydc-number-range v-model:value="record.value" :precision="record.precision" :disabled="record.disabled" />
            </template>
            <template v-if="['rate', 'slider'].includes(record.lydcKey)">
              <lydc-number-range v-model:value="record.value" :precision="record.allowHalf ? 1 : 0" :disabled="record.disabled" />
            </template>
            <template v-if="showSelectList.includes(record.lydcKey)">
              <lydc-select
            ...
            <template v-if="record.lydcKey === 'datePicker'">
              <lydc-date-range v-model:value="record.value" :format="record.format" allowClear />
            </template>
            <template v-if="record.lydcKey === 'timePicker'">
              <lydc-time-range v-model:value="record.value" :format="record.format" allowClear />
            </template>
            <template v-if="record.lydcKey === 'roleSelect'">
              <lydc-role-select
                v-model:value="record.value"
            ...
            <template v-if="record.lydcKey === 'groupSelect'">
              <lydc-group-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.lydcKey === 'areaSelect'">
              <lydc-area-select v-model:value="record.value" :level="record.level" :multiple="record.searchMultiple" :key="record.cellKey" allowClear />
            </template>
            <template v-if="record.lydcKey === 'organizeSelect'">
            ...
            <template v-if="record.lydcKey === 'cascader'">
              <lydc-cascader
                v-model:value="record.value"
                placeholder="请选择"
                :options="record.options"
            ...
            <template v-if="record.lydcKey === 'depSelect'">
              <lydc-dep-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.lydcKey === 'factoryAreaSelect'">
              <lydc-factory-area-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
                :multiple="record.searchMultiple"
            ...
            <template v-if="record.lydcKey === 'posSelect'">
              <lydc-pos-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.lydcKey === 'userSelect'">
              <lydc-user-select
                v-model:value="record.value"
            ...
            <template v-if="record.lydcKey === 'usersSelect'">
              <lydc-users-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.lydcKey === 'autoComplete'">
              <lydc-auto-complete
                v-model:value="record.value"
          ...
          <template v-if="column.key === 'isKeyword'">
            <a-checkbox v-model:checked="record.isKeyword" :disabled="!canSetKeyword.includes(record.lydcKey) || (getIsKeywordDisabled && !record.isKeyword)" />
          </template>
          <template v-if="column.key === 'searchMultiple'">
            <a-checkbox
              v-model:checked="record.searchMultiple"
              :disabled="!multipleList.includes(record.lydcKey)"
              @change="onSearchMultipleChange(record, index)" />
          </template>
        </template>
      </a-table>
      <lydc-group-title content="列表字段" :bordered="false" class="mt-20px" />
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'sortable'">
            <a-checkbox
              ...
              :disabled="
                record.id.indexOf('_lydc_') > 0 || (record.__config__ && record.__config__.isSubTable) || noGroupList.includes(record.__config__.lydcKey)
              " />
          </template>
          ...
          <template v-if="column.key === 'align'">
            <lydc-select v-model:value="record.align" :options="alignOptions" />
          </template>
        ...
        <ScrollContainer v-show="activeKey === 'column'">
          <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" class="right-board-form">
            ...
            <div v-if="columnData.type == 2">
              ...
              <div v-if="columnData.treeDataSource === 'dictionary'">
                <a-form-item label="数据字典">
                  <lydc-tree-select v-model:value="columnData.treeDictionary" :options="dicOptions" lastLevel allowClear placeholder="请选择" />
                </a-form-item>
              ...
              <div v-if="columnData.treeDataSource === 'api'">
                <a-form-item label="数据加载">
                  <lydc-radio v-model:value="columnData.treeSyncType" :options="treeSyncTypeOptions" />
                </a-form-item>
                <a-form-item v-if="columnData.treeSyncType == 1">
                  <template #label
                    >数据接口
                    <BasicHelp text="提供异步调用的数据接口" />
                  </template>
                  <interface-modal
            ...
            <a-form-item>
              <template #label
                >溢出省略
                <BasicHelp :text="['启用：溢出省略号显示；', '禁用：显示全部信息，溢出换行显示']" />
              </template>
            ...
            <div v-if="columnData.type !== 3 && columnData.type !== 5">
              ...
              <a-form-item label="分页条数" v-if="columnData.hasPage">
                <lydc-radio v-model:value="columnData.pageSize" :options="pageSizeOptions" optionType="button" button-style="solid" class="right-radio" />
              </a-form-item>
            ...
            <a-form-item label="子表样式" v-if="(columnData.type == 1 || columnData.type == 2) && webType != 4">
              <lydc-select v-model:value="columnData.childTableStyle" :options="childTableStyleOptions" placeholder="请选择" />
            </a-form-item>
...
<script lang="ts" setup>
  ...
  const columnBtnsOption = ref([
    ...
    { value: 'detail', icon: 'icon-ym icon-ym-generator-menu', label: t('common.detailText') },
  ]);
  ...
  const searchColumns = computed(() => {
    let list = [
      ...
      { title: '列名', dataIndex: 'label', key: 'label', width: 200 },
      { title: '字段', dataIndex: 'prop', key: 'prop' },
      { title: '输入类型', dataIndex: 'lydcKey', key: 'lydcKey', width: 200 },
    ...
    if (unref(webType) == 4) {
      ...
      list = list.filter(o => o.dataIndex != 'lydcKey');
    }
    return list;
  });
  const multipleList = ['select', 'depSelect', 'roleSelect', 'userSelect', 'usersSelect', 'organizeSelect', 'posSelect', 'groupSelect', 'factoryAreaSelect'];
  ...
  const viewLydcKeyOptions = [
    ...
    { id: 'factoryAreaSelect', fullName: '厂区选择' },
  ];
  const alignOptions = [
  ...
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__config__ && data.__config__.lydcKey) {
  ...
  const viewFieldOptions = computed(() => {
    ...
    return props.viewFields.map(o => ({ id: o.defaultValue, fullName: o.field, __vModel__: o.defaultValue, __config__: { lydcKey: 'input' } }));
  });
  const selectOptions = computed(() => {
    let options = unref(formFieldsOptions).map(o => ({ ...o, disabled: false }));
    if (state.columnData.treeDataSource === 'formField')
      options = options.filter(o => ['organizeSelect', 'depSelect', 'factoryAreaSelect'].includes(o.__config__.lydcKey));
    return options;
  });
  const summaryFieldOptions = computed(() =>
    state.groupFieldOptions.filter(o => ['input', 'inputNumber', 'calculate'].includes(o.__config__.lydcKey) && o.__vModel__ && !o.useMask),
  );
  ...
  function editDefaultSortConfig() {
    openDefaultSortConfigModal(true, {
      ...
      columnOptions: state.groupFieldOptions.filter(o => o.id.indexOf('_lydc_') < 0),
    });
  }
  ...
  function onLydcKeyChange(val, record, i) {
    record.__config__.lydcKey = val;
    let defaultItem: any = {
      id: record.id,
      fullName: record.fullName,
      label: record.label,
      prop: record.prop,
      lydcKey: record.lydcKey,
    ...
    if (['organizeSelect', 'depSelect', 'userSelect', 'factoryAreaSelect'].includes(val)) {
    ...
    outer: for (let i = 0; i < data.length; i++) {
      inter: for (let ii = 0; ii < defaultData.length; ii++) {
        if (data[i].prop === defaultData[ii].prop) {
          ...
          if (type === 'search') {
            if (data[i].lydcKey === defaultData[ii].lydcKey) {
  ...
  function setDefaultUpLoadData() {
    ...
    for (let i = 0; i < unref(formFieldsOptions).length; i++) {
      ...
      const lydcKey = e.__config__.lydcKey;
      if (!noUploadList.includes(lydcKey) && (required || systemComponentsList.includes(lydcKey)) && !newList.includes(e.id)) {
        newList.push(e.id);
      }
    }
  ...
  function init(list) {
    const columnOptions = list.filter(o => !noColumnShowList.includes(o.__config__.lydcKey) || o.isStorage);
    let searchOptions = list.filter(o => !noSearchList.includes(o.__config__.lydcKey));
    if (unref(webType) == 4) {
      const interfaceParam = (props.interfaceParam || [])
        .filter(o => o.useSearch)
        .map(o => {
          let lydcKey = 'input';
          if (o.dataType === 'int' || o.dataType === 'decimal') lydcKey = 'inputNumber';
          if (o.dataType === 'datetime') lydcKey = 'datePicker';
          return {
            id: o.field,
            fullName: o.fieldName,
            __vModel__: o.field,
            __config__: { isFromParam: true, lydcKey },
          };
        });
    ...
    state.groupFieldOptions = list.filter(o => o.id.indexOf('-') < 0 && !noGroupList.includes(o.__config__.lydcKey)).map(o => ({ ...o, disabled: false }));
    state.treeFieldOptions = list.filter(o => o.id.indexOf('-') < 0 && o.__config__.lydcKey == 'treeSelect');
    state.columnOptions = columnOptions.map(o => ({
      label: o.fullName,
      prop: o.id,
      fixed: 'none',
      align: 'left',
      lydcKey: o.__config__.lydcKey,
      sortable: false,
      width: null,
      ...o,
    }));
    state.searchOptions = searchOptions.map(o => ({
      ...
      searchMultiple: getSearchMultiple(o.__config__.lydcKey),
      isKeyword: false,
      ...o,
    }));
