        ...
        <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" :model="dataForm" :rules="formRules" ref="formElRef" hideRequiredMark>
          ...
          <template v-if="dataForm.btnType == 1">
            <a-form-item label="选择表单" name="modelId">
              <lydc-tree-select v-model:value="dataForm.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
            </a-form-item>
            ...
            <a-form-item label="弹窗类型" name="popupType" v-if="showType === 'pc'">
              <lydc-select v-model:value="dataForm.popupType" placeholder="请选择" :options="popupTypeOptions" />
            </a-form-item>
            ...
            <a-table :data-source="dataForm.formOptions" :columns="formOptionsColumns" size="small" :pagination="false">
              <template #bodyCell="{ column, record, index }">
                ...
                <template v-if="column.key === 'field'">
                  <lydc-select v-model:value="record.field" placeholder="请选择" :options="fieldOptions" showSearch @dropdownVisibleChange="visibleChange" />
                </template>
...
<script lang="ts" setup>
  ...
  function init(data) {
    ...
    formFieldsOptions.value = cloneDeep(data.formFieldsOptions)
      .filter(o => !noAllowFormFieldsList.includes(o.__config__.lydcKey) && o.id.indexOf('-') < 0)
