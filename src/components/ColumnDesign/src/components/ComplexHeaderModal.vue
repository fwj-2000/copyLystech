<template>
  <BasicModal v-bind="$attrs" width="1000px" class="LYDC-complex-header-Modal" @register="registerModal" title="复杂表头配置" @ok="handleSubmit" destroyOnClose>
    <a-table size="small" rowKey="id" class="complex-header-table" :data-source="list" :columns="columns" :pagination="false">
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.key === 'align'">
          <LydcSelect v-model:value="record.align" placeholder="请选择" :options="alignList" />
        </template>
