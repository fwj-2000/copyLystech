<template>
  <BasicModal v-bind="$attrs" class="LYDC-complex-header-Modal" @register="registerModal" title="默认排序配置" @ok="handleSubmit" destroyOnClose>
    <a-table size="small" rowKey="id" class="sort-table" :data-source="list" :columns="columns" :pagination="false">
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.key === 'sort'">
          <LydcRadio v-model:value="record.sort" :options="sortTypeOptions" />
        </template>
