<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="组件属性配置" @ok="handleSubmit" destroyOnClose class="extra-config-modal">
    <div class="extra-config-modal-body" :style="{ 'min-height': activeData.lydcKey === 'select' ? '300px' : '150px' }">
      <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" class="right-board-form">
        <template v-if="activeData.lydcKey === 'select'">
          ...
          <div v-if="activeData.__config__.dataType === 'dictionary'">
            ...
            <a-form-item label="存储字段">
              <lydc-select v-model:value="activeData.props.value" placeholder="请选择" :options="valueOptions" />
            </a-form-item>
          </div>
        ...
        <a-form-item label="日期类型" v-if="activeData.lydcKey === 'datePicker'">
          <lydc-select v-model:value="activeData.format" placeholder="请选择" :options="dateFormatOptions" />
        </a-form-item>
        <a-form-item label="时间类型" v-if="activeData.lydcKey === 'timePicker'">
          <lydc-select v-model:value="activeData.format" placeholder="请选择" :options="timeFormatOptions" />
        </a-form-item>
        <a-form-item label="" v-if="['organizeSelect', 'depSelect', 'userSelect', 'factoryAreaSelect'].includes(activeData.lydcKey)">
          <a-checkbox v-model:checked="activeData.isIncludeSubordinate">
            查询当前{{
              activeData.lydcKey === 'organizeSelect'
                ? '组织及子组织'
                : activeData.lydcKey === 'depSelect'
                ? '部门及子部门'
                : activeData.lydcKey === 'factoryAreaSelect'
                ? '厂区'
                : '用户及下属'
            }}
          </a-checkbox>
