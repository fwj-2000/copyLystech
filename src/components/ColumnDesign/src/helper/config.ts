...
const getSearchType = item => {
  ...
  if (RangeList.includes(lydcKey)) return 3;
  if (fuzzyList.includes(lydcKey)) return 2;
  return 1;
};
const getSearchMultiple = lydcKey => {
  const searchMultipleList = [
    'select',
    'depSelect',
    'roleSelect',
    'userSelect',
  ...
  return searchMultipleList.includes(lydcKey);
};
