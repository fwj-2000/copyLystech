      ...
      <div class="px-10 py-20px">
        <a-form :colon="false" labelAlign="right" :labelCol="{ style: { width: '90px' } }" :model="dataForm" :rules="rules" ref="formElRef">
          <a-form-item label="上级菜单" name="parentId" v-if="type == 'menu'">
            <lydc-tree-select v-model:value="dataForm.parentId" :options="treeOptions" placeholder="请选择" :lastLevel="props.data.category == 'App'" />
          </a-form-item>
          ...
          <a-form-item label="上传文件" name="fileList">
            <a-upload v-feature :file-list="dataForm.fileList" :accept="accept" :before-upload="beforeUpload" @remove="handleRemove">
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LydcUploadBtn', inheritAttrs: false });
