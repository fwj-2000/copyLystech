<template>
  <a-modal
    ...
    wrap-class-name="fullscreen-modal priview-modal">
    <template #closeIcon>
    ...
    <template #title>
      <div class="lydc-full-modal-header">
        ...
        <a-space class="options" :size="10">
          <a-button type="primary" @click="handleDownload()" v-if="showDownload">{{ t('common.downloadText') }}</a-button>
...
<script lang="ts" setup>
  ...
  function init(file) {
    state.title = t('component.filePreview.filPreview') + ' - ' + file.name;
    state.url = '';
    state.file = file;
    state.visible = true;
    state.loading = true;
