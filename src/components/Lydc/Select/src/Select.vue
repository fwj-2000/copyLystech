...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LydcSelect', inheritAttrs: false });
  ...
  if (props.dispatchChange) {
    const { value, options } = props;
    const _opsItem = options.filter((el: any) => {
      if (el[props.fieldNames.value as any] == value) return el;
    });
    if (_opsItem.length) {
      emit('change', value, _opsItem[0]);
    }
  }
</script>
