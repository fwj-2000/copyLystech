...
<script lang="ts" setup>
  ...
  import { customUserSelectProps, FieldNames } from './props';
  import { useAttrs } from '/@/hooks/core/useAttrs';
  import { getUserSelectorList } from '/@/api/permission/user';
  import { useDebounceFn } from '@vueuse/core';
  import { isNullOrUnDef } from '/@/utils/is';

  defineOptions({ name: 'LydcCustomUserSelect', inheritAttrs: false });
  defineExpose({ getSelectRef });
  const props = defineProps(customUserSelectProps);
  ...
  function handleFetch(visible: boolean) {
    if (visible && !props.alwaysLoad) {
      if (!props.immediate && !unref(isFirstLoaded)) {
        fetchUserList(innerValue.value);
      }
    }
    if (visible && props.alwaysLoad) {
      ...
      if (!innerValue.value?.length && options.value?.length) {
        options.value = [];
      }
    }
  }

  const debounceSetValue = useDebounceFn(setValue, 300);
  ...
  if (!isNullOrUnDef(props.defaultValue)) {
    ...
    debounceSetValue(props.defaultValue);
  ...
  function handleSearch(value, type = 'search') {
    ...
    // 接口换成可以获取离职人员接口
    getUserSelectorList({ keyword: searchValue, ...pagination })
      .then(res => {
        // 检查是否为最新请求
