  ...
  <a-modal v-model:open="visible" :title="modalTitle" :width="800" class="transfer-modal" @ok="handleSubmit" centered :maskClosable="false" :keyboard="false">
    ...
    <div class="transfer__body">
      <div class="transfer-pane">
        ...
        <div class="transfer-pane__body transfer-pane__body-tab">
          <a-tabs v-model:activeKey="activeKey" :tabBarGutter="10" size="small" class="pane-tabs" v-if="hasSys">
            <a-tab-pane key="1" :tab="t('component.lydc.common.allData')"></a-tab-pane>
            <a-tab-pane key="system" :tab="t('component.lydc.common.dynamicParameter')"></a-tab-pane>
          </a-tabs>
      ...
      <div class="transfer-pane right-pane">
        <div class="transfer-pane__tool">
          <span>{{ t('component.lydc.common.selected') }}</span>
          <span class="remove-all-btn" @click="removeAll">{{ t('component.lydc.common.clearAll') }}</span>
        </div>
        <div class="transfer-pane__body">
          <ScrollContainer>
            ...
            <lydc-empty v-if="!selectedData.length" />
          </ScrollContainer>
        </div>
      </div>
    </div>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LydcOrganizeSelect', inheritAttrs: false });
  ...
  function setValue() {
    ...
    for (let i = 0; i < ids.length; i++) {
      inner: for (let j = 0; j < allList.value.length; j++) {
        ...
        if (ids[i].join() === organizeIds.join()) {
          selectedList.push({ ...allList.value[j], organize: getLastNValues(allList.value[j].organize, props.lastLevel) });
          break inner;
        }
      }
    }
    ...
    // const lastTreeData = cloneDeep(selectedList).map(item => ({
    //   ...item,
    //   organize: getLastNValues(item.organize, props.lastLevel)
    // }))
    // options.value = lastTreeData
  ...
  function handleSelect(keys) {
    ...
    if (props.multiple) {
      ...
      selectedData.value.push({ ...data, organize: getLastNValues(data.organize, props.lastLevel) });
    } else {
      selectedIds.value = [currId];
      selectedData.value = [{ ...data, organize: getLastNValues(data.organize, props.lastLevel) }];
    }
  }
  function removeAll() {
    selectedIds.value = [];
  ...
  async function initData() {
    ...
    const topItem = {
      fullName: t('component.topNodes'),
      hasChildren: true,
      id: '-1',
      icon: 'icon-ym icon-ym-tree-organization3',
      organize: t('component.topNodes'),
      organizeIds: ['-1'],
    };
  ...
  function getLastNValues(input, n) {
    // 按'/'分割字符串
