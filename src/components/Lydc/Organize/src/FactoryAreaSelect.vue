...
<script lang="ts" setup>
  import { getFactoryAreaTree } from '/@/api/permission/organize';
  ...
  import { factoryAreaProps } from './props';
  ...
  defineOptions({ name: 'LydcFactoryAreaSelect', inheritAttrs: false });
  const props = defineProps(factoryAreaProps);
  ...
  function handleSelect(keys) {
    ...
    if (data?.disabled || data?.type === 'company' || data?.type === 'coo' || data?.type === 'bg' || data?.type === 'bu') return;
  ...
  async function initData() {
    ...
    await getFactoryAreaTree().then(res => {
