...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LydcRelationForm', inheritAttrs: false });
  ...
  function setValue(value) {
    ...
    if (!props.multiple) {
      getDataChange(props.modelId, value).then(res => {
        ...
        emitter.emit('setRelationData', { lydcRelationField: props.field, ...data, id: res.data.id });
      });
    }
  }
  function setNullValue() {
    ...
    generatorStore.setRelationData(relationData);
    emitter.emit('setRelationData', { lydcRelationField: props.field });
  }
  function onChange() {
    options.value = [];
    emit('update:value', '');
