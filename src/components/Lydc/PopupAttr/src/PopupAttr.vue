<template>
  <template v-if="!detailed">
    <a-input v-model:value="innerValue" readonly :disabled="disabled" placeholder="用于展示关联弹窗的属性，且数据同时会保存入库" v-if="isStorage" />
    <a-input v-model:value="innerValue" readonly :disabled="disabled" placeholder="用于展示关联弹窗的属性，且数据不会保存" v-else />
  </template>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'LydcPopupAttr', inheritAttrs: false });
  ...
  onBeforeMount(() => {
    emitter.on('setRelationData', data => {
      if (!props.isStorage || !props.showField || !props.relationField || props.relationField != data.lydcRelationField) return;
