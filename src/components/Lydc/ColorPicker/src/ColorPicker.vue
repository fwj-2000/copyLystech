<template>
  <a-popover
    ...
    trigger="click"
    class="lydc-color-picker"
    :class="[colorSize ? 'lydc-color-picker--' + colorSize : '']"
    v-bind="$attrs"
    v-if="!colorDisabled">
    <template #content>
  ...
  <div class="lydc-color-picker ant-color-picker__trigger is-disabled" :class="[colorSize ? 'lydc-color-picker--' + colorSize : '']" v-if="colorDisabled">
    <span class="ant-color-picker__color" :class="{ 'is-alpha': showAlpha }">
      <span class="ant-color-picker__color-inner" :style="{ backgroundColor: displayedColor }">
        <DownOutlined v-show="value || showPanelColor" style="color: #909399" />
        <CloseOutlined v-if="!value && !showPanelColor" style="color: #909399" />
...
<style lang="less" scoped>
  ...
  // large 大小
  .lydc-color-picker--large {
    width: 40px;
    height: 40px;
  }

  //small 大小
  .lydc-color-picker--small {
    width: 32px;
    height: 32px;
  }
</style>
