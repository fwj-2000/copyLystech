        ...
        <template v-for="(item, cIndex) in rowConfig">
          <template v-if="column.key === item.__vModel__ && column.__config__.formId === item.__config__.formId">
            <div :key="item.__config__.formId">
              <LydcRelationForm
                v-if="item.__config__.tag === 'LydcRelationForm'"
                :rowIndex="index"
...
<script lang="ts" setup>
  ...
  import { getScriptFunc, getDateTimeUnit, thousandsFormat } from '/@/utils/lydc';
  ...
  defineOptions({ name: 'LydcInputTable', inheritAttrs: false });
  ...
  function buildOptions() {
    state.rowConfig.forEach((cur, index) => {
      ...
      if (dyOptionsList.indexOf(config.lydcKey) > -1) {
    ...
    for (let key in currRelations) {
      if (key === data.__vModel__) {
        for (let i = 0; i < currRelations[key].length; i++) {
          ...
          const lydcKey = config.lydcKey;
          let defaultValue: any = null;
          if (
            ['checkbox', 'cascader', 'organizeSelect'].includes(lydcKey) ||
            (['select', 'treeSelect', 'popupSelect', 'popupTableSelect', 'userSelect'].includes(lydcKey) && e.multiple)
          ) {
            defaultValue = [];
          }
  ...
  function buildRowAttr(rowIndex) {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      if (config.lydcKey === 'userSelect' && cur.relationField && cur.selectType !== 'all' && cur.selectType !== 'custom') {
        let value = getFieldVal(cur.relationField, rowIndex) || [];
        cur.ableRelationIds = Array.isArray(value) ? value : [value];
      }
      if (config.lydcKey === 'datePicker' || config.lydcKey === 'timePicker') {
  ...
  function onFormDataChange(rowIndex, colIndex, val, row) {
    ...
    if (['popupSelect', 'relationForm'].includes(data.lydcKey)) setTransferFormData(row, data.config.__config__);
  }
  ...
  function getConfById(formId, rowIndex) {
    ...
    let newObj = {};
    item = getRealProps(item, itemConfig.lydcKey);
    ...
    if (['relationForm', 'popupSelect'].includes(itemConfig.lydcKey)) {
      newObj['field'] = props.config.__vModel__ + item.__vModel__ + '_lydcRelation_' + rowIndex;
    }
    if (['relationFormAttr', 'popupAttr'].includes(itemConfig.lydcKey)) {
      let prop = newObj['relationField'].split('_lydcTable_')[0];
      newObj['relationField'] = props.config.__vModel__ + prop + '_lydcRelation_' + rowIndex;
    }
    return newObj;
  }
  /**
  ...
  function getEmptyRow(val) {
    return state.rowConfig.map((t: any) => ({
      ...
      options: dyOptionsList.indexOf(t.__config__.lydcKey) > -1 ? t.options : [],
      valid: true,
      regValid: true,
      regErrorText: '',
      on: t.on || {},
      lydcKey: t.__config__.lydcKey,
  ...
  function getDefaultEmptyValue(item) {
    if (!item.__config__ || !item.__config__.lydcKey) return undefined;
    const lydcKey = item.__config__.lydcKey;
    const list = ['checkbox', 'uploadFile', 'uploadImg', 'cascader', 'organizeSelect', 'areaSelect'];
    return list.includes(lydcKey) || item.multiple ? [] : undefined;
  }
  // 获取表格数据
  ...
  function addRow(val?, isUpdate = true) {
    ...
    const lydcId = buildUUID();
    rowData.lydcId = lydcId;
  ...
  function copyData(data) {
    ...
    for (let [key] of Object.entries(state.defaultRowData)) {
      rowData[key] = isUnDef(data[key]) ? null : data[key];
    }
    addRow(rowData);
  }
  ...
  function batchRemoveRow(item) {
    ...
    const handleBatchRemove = () => {
      state.tableFormData = state.tableFormData.filter(o => !state.selectedRowKeys.includes(o.lydcId));
      state.dataSource = state.dataSource.filter(o => !state.selectedRowKeys.includes(o.lydcId));
      nextTick(() => {
  ...
  function updateRowConfig() {
    ...
    state.rowConfig = props.config.__config__.children.map(o => {
      if (o.__config__.defaultCurrent) {
        if (o.__config__.lydcKey === 'datePicker') {
          o.__config__.defaultValue = dayjs(currDate).tz().startOf(getDateTimeUnit(o.format)).valueOf();
        }
        if (o.__config__.lydcKey === 'timePicker') {
          o.__config__.defaultValue = dayjs(currDate)
            .tz()
            .format(o.format || 'HH:mm:ss');
        }
      }
      defaultRowData[o.__vModel__] = o.__config__.defaultValue;
      if (o.__config__ && dyOptionsList.includes(o.__config__.lydcKey) && o.__config__.dataType !== 'static') o.options = [];
      return o;
    });
