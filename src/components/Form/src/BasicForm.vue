<template>
  <Form v-bind="getBindValue" :class="getFormClass" ref="formElRef" labelWrap :model="formModel" @keypress.enter="handleEnterPress" :name="getFormName">
    <Row v-bind="getRow">
...
<script lang="ts">
  ...
  import { deepMerge, transformI18nBasicForm } from '/@/utils';
  ...
  import { isFunction, isArray, isNullOrUnDef } from '/@/utils/is';
  ...
  export default defineComponent({
    ...
    setup(props, { emit, attrs }) {
      ...
      const getProps = computed((): FormProps => {
        ...
        const { i18nConfig } = { ...props, ...newProps };
        ...
        return { ...props, ...newProps, ...mergeCustomProps, autocomplete: 'on' } as FormProps;
      });
      ...
      const getSchema = computed((): FormSchema[] => {
        ...
        let schemas: FormSchema[] = unref(schemaRef) || (_props.schemas as any);
        // 国际化配置转化
        const { i18nConfig } = _props;
        schemas = transformI18nBasicForm(schemas, i18nConfig);
        schemas.forEach(item => {
          ...
          // 添加autocomplete属性
          if (item.component === 'Input' || item.component === 'InputNumber') {
            if (item.componentProps) {
              item.componentProps.autocomplete = item.field;
              item.componentProps.name = item.field;
              item.componentProps.allowClear = true;
            } else {
              item.componentProps = {
