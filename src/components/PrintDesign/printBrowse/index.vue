<template>
  <BasicModal v-bind="$attrs" @register="registerModal" defaultFullscreen :footer="null" :closable="false" :keyboard="false" class="lydc-full-modal full-modal">
    <template #title>
      <div class="lydc-full-modal-header">
        <div class="header-title">
          <img src="../../../assets/images/lydc.png" class="header-logo" />
...
<script lang="ts" setup>
  ...
  import { getAmountChinese, getDateFormat } from '/@/utils/lydc';
  ...
  function handleReplaceValue(dom, tag) {
    ...
    for (let i = 0; i < dataList.length; i++) {
      ...
      if (elementText.includes('{') && elementText.includes('data-tag')) {
        ...
        if (tag == 'td') {
          let value = getValueFun(elementText);
          let spanText = elementText.match(/<span class="lydc-print-tag-span.*?[^}]}.*?<\/span>/);
        ...
        if (elementText.includes('千位分隔符(')) {
          ...
          if (!data || Number.isNaN(Number(data))) continue;
  ...
  function replaceCommonValue() {
    const list: any[] = state.printTemplate.match(/<span class="lydc-print-tag-span.*?[^}]}.*?<\/span>/g) || [];
    for (let i = 0; i < list.length; i++) {
      if (list[i].includes('{') && list[i].includes('data-tag')) {
        let value = getValue(list[i]);
        let spanText = list[i].match(/<span class="lydc-print-tag-span.*?[^}]}.*?<\/span>/);
        if (spanText?.length) {
          spanText = spanText[0];
          let fieldStr = spanText.match(/{.*}/g);
  ...
  function getTableKey(dom) {
    ...
    for (let tdDom of tdDoms) {
      ...
      if (['thousands', 'isAmountChinese', 'lydcDictionary', 'lydcDate'].includes(dataTag)) {
  ...
  function getPlace(value) {
    ...
    let index = value.toString().indexOf('.') + 1;
    let count = value.toString().length - index;
    return count;
  }
  function pre() {
  ...
  function scrollToPage() {
    let index = Math.max(state.pageIndex - 1, 0);
    nextTick(() => {
  ...
  function handlePrint() {
    ...
    iframe.onload = function () {
      ...
      document.title = 'DC Manage System';
  ...
  function replaceSysValue() {
    ...
    const systemPrintTime = dayjs(Date.now()).tz().format('YYYY-MM-DD HH:mm:ss');
    ...
    if (recordList.length) {
      ...
      for (let i = 0; i < recordList.length; i++) {
        ...
        content += `<tr><td style="width: 20%;" data-mce-style="width: 20%;"><span class="lydc-print-tag-span lydc-tiny-color--common" contenteditable="false">${
          record.nodeName
        }</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="lydc-print-tag-span lydc-tiny-color--common" contenteditable="false">${
          record.userName
        }</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="lydc-print-tag-span lydc-tiny-color--common" contenteditable="false">${dayjs(
          record.handleTime,
        ).format(
          'YYYY-MM-DD HH:mm:ss',
        )}</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="lydc-print-tag-span lydc-tiny-color--common" contenteditable="false">${getHandleName(
          record.handleStatus,
        )}</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="lydc-print-tag-span lydc-tiny-color--common" contenteditable="false">${
          record.handleOpinion || ''
        }</span></td><td style="width: 100px" data-mce-style="width: 20%;"><span class="lydc-print-tag-span lydc-tiny-color--common" contenteditable="false">${
  ...
  function getDateValue(value, format) {
    ...
    return dayjs(value)
      .tz()
      .format(getDateFormat(format) || 'YYYY-MM-DD HH:mm:ss');
  }
</script>
