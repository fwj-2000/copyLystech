  ...
  <div class="print-template-detail">
    <div class="print-template-detail__body">
      <div class="system-view-nav">
        <ScrollContainer>
          <div class="lydc-common-title"><h2>表单字段</h2></div>
          <BasicLeftTree
            ref="leftTreeRef"
            :showSearch="false"
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    ...
    funData: [
      ...
      { fullName: '数据字典(字段,字典分类)', id: 'lydcDictionary' },
      { fullName: '日期选择(字段,日期格式)', id: 'lydcDate' },
    ],
  });
  ...
  function handleNodeClick(_item, node) {
    ...
    const tableParent = getCurrentParentByTag('table[data-lydc-table-tag="table"]');
    if (!tableParent) {
      if (item == 'lydcDictionary') return openDictionaryModal(true, { node });
  ...
  function reload(item, node) {
    unref(editor).insertContent(getSpanNode('lydcDictionary', node, item));
    state.content = unref(editor).getContent({ format: 'html' });
  }
  function getEditConfig(e) {
    ...
    state.initConfig = {
      ...
      content_style: `html {
        ...
        .lydc-tiny-color--customer {
          color: #005AF3;
        }
        .lydc-tiny-color--contacts {
          color: #1CB24C;
        }
        .lydc-tiny-color--business {
          color: #FF852F;
        }
        .lydc-tiny-color--contract {
          color: #FF4A43;
        }
        .lydc-tiny-color--receivables {
          color: #FFAE46;
        }
        .lydc-tiny-color--product {
          color: #00D0D4;
        }
        .lydc-tiny-color--common {
          color: #2362FB;
        }
      ...
      setup: editor => {
        ...
        editor.on('keydown', e => {
          ...
          if (e.keyCode === 8 || e.keyCode == 46) {
            ...
            if (currentNode.getAttribute('data-lydc-table-td-tag') === 'value') {
              e.preventDefault();
              return false;
  ...
  function getSpanNode(id, node, item?) {
    ...
    if (id == 'lydcDictionary') return `<p style="display:inline-block" data-tag='lydcDictionary'>数据字典(${parent},"${item}")<p>`;
    if (id == 'lydcDate') return `<p style="display:inline-block" data-tag='lydcDate'>日期选择(${parent},"yyyy-MM-dd")<p>`;
    return `<span data-tag="${parent}.${id}" class="lydc-print-tag-span ${getSpanColorClass()}" contenteditable="false">{${id}}</span>`;
  }
