...
export const getRealProps = (data, lydcKey) => {
...
export const getFormSchemaItem = data => {
  const item = getRealProps(data, data.__config__.lydcKey);
  ...
  const schema = {
    ...
    component: upperFirst(item.__config__.lydcKey),
    componentProps,
...
export const getSearchFormSchemas = (list: any[]) => {
  ...
  for (let i = 0; i < schemas.length; i++) {
    ...
    if (['OrganizeSelect', 'DepSelect', 'PosSelect', 'RoleSelect', 'GroupSelect', 'UsersSelect', 'FactoryAreaSelect'].includes(e.component)) {
      e.componentProps = {
