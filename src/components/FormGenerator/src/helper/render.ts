...
export default defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    function buildDataObject(confClone, dataObject, formData) {
      const lydcKey = confClone.__config__.lydcKey;
      ...
      if (['calculate', 'table', 'barcode', 'qrcode', 'popupSelect', 'popupTableSelect', 'autoComplete'].includes(lydcKey)) {
        dataObject['formData'] = formData;
      }
      if (['table'].includes(lydcKey)) {
        dataObject['relations'] = props.relations;
        dataObject['vModel'] = confClone.__vModel__;
        dataObject['ref'] = 'tableRef';
      }
      if (['relationForm', 'popupSelect'].includes(lydcKey)) {
        dataObject['field'] = confClone.__config__.tableName
          ? confClone.__vModel__ + '_lydcTable_' + confClone.__config__.tableName + (confClone.__config__.isSubTable ? '0' : '1')
    ...
    return () => {
      ...
      const lydcKey = upperFirst(props.conf.__config__.lydcKey) === 'Table' ? 'InputTable' : upperFirst(props.conf.__config__.lydcKey);
      const Comp = componentMap.get(lydcKey as ComponentType) as ReturnType<typeof defineComponent>;
      if (!Comp) return null;
      const realDataObject = getRealProps(dataObject, props.conf.__config__.lydcKey);
