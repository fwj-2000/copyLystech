<template>
  <div class="right-board common-board">
    ...
    <div class="field-box" v-if="activeData?.__config__ && activeData.__config__.lydcKey">
      <ScrollContainer>
        <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" v-show="activeKey === 'field'" class="right-board-form">
          <a-form-item label="控件类型" v-if="!['tableGridTr', 'tableGridTd'].includes(activeData.__config__.lydcKey)">
            <a-input v-model:value="getCompName" disabled />
          </a-form-item>
          <a-form-item label="控件用途" v-if="lydcKey == 'calculate'">
            <lydc-select v-model:value="activeData.isStorage" :options="storageTypeOptions" @change="onStorageChange" />
          </a-form-item>
          <div v-if="(activeData.__vModel__ !== undefined && !noVModelList.includes(lydcKey)) || activeData.isStorage">
            <div v-if="getHasTable">
              <div v-if="!config.isSubTable">
                <a-form-item label="数据库表" v-if="lydcKey !== 'table'">
                  <lydc-select
                ...
                <a-form-item label="控件字段">
                  <lydc-select
                    ...
                    showSearch
                    allowClear
                    :disabled="lydcKey === 'table'"
                    :showArrow="lydcKey !== 'table'"
          ...
          <a-form-item label="控件标题" v-if="!layoutList.includes(lydcKey) || lydcKey === 'table'">
            <a-input v-model:value="activeData.__config__.label" placeholder="请输入" />
          </a-form-item>
          <a-form-item label="显示标题" v-if="!layoutList.includes(lydcKey)">
            <a-switch v-model:checked="activeData.__config__.showLabel" />
          </a-form-item>
          <a-form-item label="显示标题" v-if="lydcKey === 'table'">
            <a-switch v-model:checked="activeData.__config__.showTitle" />
          </a-form-item>
          <a-form-item label="卡片标题" v-if="lydcKey === 'card'">
            <a-input v-model:value="activeData.header" placeholder="请输入" />
          </a-form-item>
          <a-form-item label="标题提示" v-if="hasTipLabel(lydcKey)">
            <a-input v-model:value="activeData.__config__.tipLabel" placeholder="请输入" />
          </a-form-item>
          <a-form-item label="占位提示" v-if="activeData.placeholder !== undefined && !systemList.includes(lydcKey)">
            <a-input v-model:value="activeData.placeholder" placeholder="请输入" />
          </a-form-item>
          <div v-if="lydcKey === 'table'">
          ...
          <div v-if="![...layoutList, ...noVModelList].includes(lydcKey) && lydcKey !== 'input'">
            <a-form-item label="是否禁用" v-if="!systemList.includes(lydcKey)">
              <a-switch v-model:checked="activeData.disabled" />
            </a-form-item>
            <a-form-item label="是否必填" v-if="!systemList.includes(lydcKey)">
          ...
          <a-form-item label="是否隐藏" v-if="['relationFormAttr', 'popupAttr', 'calculate'].includes(lydcKey) && activeData.isStorage">
            <a-switch v-model:checked="activeData.__config__.noShow" />
          </a-form-item>
...
<script lang="ts" setup>
  ...
  const getCompName = computed(() => {
    ...
    const comp = allComps.filter(o => o.__config__.lydcKey === unref(lydcKey));
    if (!comp.length) return '';
    return comp[0].__config__.label;
  });
  const getRightComp = computed(() => {
    if (!unref(lydcKey)) return null;
    if (orgList.includes(unref(lydcKey))) return RightComp.ROrgRight;
    if (unref(lydcKey) === 'popupTableSelect') return RightComp.RPopupSelect;
    return RightComp['R' + upperFirst(unref(lydcKey))];
  });
  ...
  const drawingOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && data.__config__.lydcKey !== 'table') {
  ...
  watch(
    ...
    val => {
      ...
      if (!val.__config__.tableName && val.__config__.lydcKey !== 'table') {
  ...
  function onChildTableFieldChange(val, formId) {
    ...
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        ...
        if (config.lydcKey === 'table' && config.tableName === props.activeData.__config__.relationTable) {
        ...
        if (config && config.lydcKey != 'table' && config.children && Array.isArray(config.children)) {
  ...
  function onInputFieldChange(val, formId, parentVModel) {
    ...
    if (parentVModel) {
      const loop = (data, parent?) => {
        ...
        if (Array.isArray(data)) data.forEach(d => loop(d, parent));
        if (parent?.__config__?.lydcKey == 'table') {
          if (
            data.__vModel__ &&
      ...
      const loop = (data, parent?) => {
        if (!data) return;
        if (data.__config__?.lydcKey !== 'table' && data.__config__?.children && Array.isArray(data.__config__.children)) {
          loop(data.__config__.children, data);
        }
        if (Array.isArray(data)) data.forEach(d => loop(d, parent));
  ...
  function onStorageChange(val) {
    if (val == 0) {
      ...
      if (!unref(getHasTable)) {
        props.activeData.__vModel__ = toggleVmodelCase(`${props.activeData.__config__.lydcKey}Field${props.activeData.__config__.formId}`);
      } else {
  ...
  function onChildTableNameChange(tableName, formId) {
    ...
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        const e = list[i];
        const config = e.__config__;
        if (config.lydcKey === 'table' && config.tableName === tableName && config.formId !== formId) {
          boo = true;
          break;
        }
        if (config && config.lydcKey != 'table' && config.children && Array.isArray(config.children)) {
  ...
  function setDefaultOptions() {
    ...
    if ((props.activeData.__vModel__ === undefined && props.activeData.isStorage != 1) || props.activeData.__config__.lydcKey === 'table') return;
    if (!props.activeData.__config__.tableName || props.activeData.__config__.tableName === unref(getMainTable)) {
      ...
      if (!list.length) {
        ...
        let options = item.fields.map(o => ({
          ...o,
          realField: 'lydc_' + props.activeData.__config__.tableName + '_lydc_' + o.field,
        }));
  ...
  function hasTipLabel(lydcKey) {
    return ![...layoutList, ...systemList].includes(lydcKey) || ['table', 'card', 'billRule'].includes(lydcKey);
  }
  ...
  onMounted(() => {
    ...
    if (!props.activeData.__config__.tableName && props.activeData.__config__.lydcKey !== 'table') {
      props.activeData.__config__.tableName = unref(getMainTable);
    }
    setDefaultOptions();
  });
