<template>
  ...
  <a-form-item label="文件类型">
    <lydc-select v-model:value="getAccept" placeholder="不限制" allowClear :options="acceptOptions" multiple />
  </a-form-item>
  ...
  <a-form-item label="上传路径">
    <lydc-radio v-model:value="activeData.pathType" :options="pathTypeOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  <template v-if="activeData.pathType === 'selfPath'">
    <a-form-item label="分用户存储">
      <lydc-switch v-model:value="activeData.isAccount" />
    </a-form-item>
    <a-form-item>
      <template #label
        >文件夹名
