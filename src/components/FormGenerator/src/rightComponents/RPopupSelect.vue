<template>
  <a-form-item label="弹窗标题" v-if="lydcKey === 'popupSelect'">
    <a-input v-model:value="activeData.popupTitle" placeholder="请输入" />
  </a-form-item>
  <a-form-item label="弹窗类型" v-if="lydcKey === 'popupSelect' && showType === 'pc'">
    <lydc-select v-model:value="activeData.popupType" placeholder="请选择" :options="popupTypeOptions" />
  </a-form-item>
  <a-form-item label="弹窗宽度" v-if="lydcKey === 'popupSelect' && showType === 'pc'">
  ...
  <a-form-item v-if="lydcKey === 'popupSelect' && (modelType == 1 || modelType == 6)">
  ...
  <a-form-item label="分页条数" v-if="activeData.hasPage">
    <lydc-radio v-model:value="activeData.pageSize" :options="pageSizeOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  <a-divider></a-divider>
  <a-form-item label="能否清空">
    <a-switch v-model:checked="activeData.clearable" />
  </a-form-item>
  <a-form-item label="能否多选" v-if="lydcKey === 'popupTableSelect'">
