    ...
    <a-form
      ...
      <template v-if="!['copy', 'remove'].includes(dataForm.value)">
        ...
        <a-form-item label="按钮图标">
          <lydc-icon-picker v-model:value="dataForm.btnIcon" placeholder="请选择" />
        </a-form-item>
      </template>
      ...
      <template v-if="!['copy', 'remove', 'add', 'batchRemove'].includes(dataForm.value)">
        <lydc-group-title content="动作设置" :bordered="false" />
        <a-form-item label="弹窗标题">
          <a-input v-model:value="actionConfig.popupTitle" placeholder="请输入" />
        </a-form-item>
        <a-form-item label="弹窗类型">
          <lydc-radio v-model:value="actionConfig.popupType" :options="popupTypeOptions" optionType="button" button-style="solid" class="right-radio" />
        </a-form-item>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'relationField'">
                ...
                <template v-else-if="record.sourceType == 2">
                  <lydc-input-number v-if="['int', 'decimal'].includes(record.dataType)" v-model:value="record.relationField" placeholder="请输入" clearable />
                  <lydc-date-picker
        ...
        <a-form-item label="表单数据" v-else>
          <LydcTreeSelect v-model:value="actionConfig.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
        </a-form-item>
        ...
        <a-table :data-source="actionConfig.columnOptions" :columns="getColumnOptionsColumns" size="small" :pagination="false">
          ...
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'value'">
              ...
              <lydc-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch @change="onChange($event, record)" />
            </template>
        ...
        <a-table :data-source="actionConfig.relationOptions" :columns="relationOptionsColumns" size="small" :pagination="false">
          <template #bodyCell="{ column, record, index }">
            <template v-if="column.key === 'field'">
              <lydc-select v-model:value="record.field" placeholder="请选择" :options="childList" showSearch class="!w-134px" />
            </template>
            <template v-if="column.key === 'type'">
              <lydc-select v-model:value="record.type" placeholder="请选择" :options="typeOptions" @change="onTypeChange(record)" />
            </template>
            <template v-if="column.key === 'value'">
              <template v-if="record.type == 1">
                <a-auto-complete
                  ...
                  @focus="onFocus(record.value)"
                  @search="debounceOnSearch(record.value)" />
                <lydc-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch />
              </template>
...
<script lang="ts" setup>
  ...
  function init(data) {
    ...
    childList.value = cloneDeep(data.children)
      .filter(o => o.__vModel__ && !noAllowSelectList.includes(o.__config__.lydcKey))
      .map(o => ({
        id: o.__vModel__,
