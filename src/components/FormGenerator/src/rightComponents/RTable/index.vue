<template>
  <a-form-item label="合计设置">
    <lydc-switch v-model:value="activeData.showSummary" />
  </a-form-item>
  <a-form-item label="合计字段" v-if="activeData.showSummary">
    <lydc-select v-model:value="activeData.summaryField" placeholder="请选择" :options="childrenList" allowClear showSearch multiple />
  </a-form-item>
...
<script lang="ts" setup>
  ...
  const childrenList = computed(() => {
    const list = props.activeData.__config__.children.filter(
      o => ['input', 'inputNumber', 'calculate'].includes(o.__config__.lydcKey) && o.__vModel__ && !o.useMask,
    );
    return list.map(o => ({ id: o.__vModel__, fullName: o.__config__.label }));
  });
  const getInputComponents = computed(() => inputComponents.filter(o => !noTableAllowList.includes(o.__config__.lydcKey)));
  const getSelectComponents = computed(() => {
    const list = selectComponents.filter(o => !noTableAllowList.includes(o.__config__.lydcKey));
    return props.formInfo.type == 1 || props.formInfo.type == 6 ? [...list, calculateItem] : list;
  });
  const getSystemComponents = computed(() => systemComponents.filter(o => !noTableAllowList.includes(o.__config__.lydcKey)));
