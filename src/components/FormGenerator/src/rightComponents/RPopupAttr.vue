<template>
  <a-form-item label="关联弹窗">
    <lydc-select v-model:value="activeData.relationField" placeholder="请选择" :options="formFieldsOptions" allowClear showSearch />
  </a-form-item>
  ...
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__config__ && data.__config__.lydcKey) {
        if (data.__config__.lydcKey === 'popupSelect' && data.__vModel__ && isSameSource(data)) {
          list.push({
            fullName: data.__config__.label,
            id:
              data.__config__ && data.__config__.tableName
                ? data.__vModel__ + '_lydcTable_' + data.__config__.tableName + (data.__config__.isSubTable ? '0' : '1')
                : data.__vModel__,
            ...data,
            disabled: false,
          });
