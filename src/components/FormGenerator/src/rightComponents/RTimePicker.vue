<template>
  <a-form-item label="格式">
    <lydc-select v-model:value="activeData.format" placeholder="请选择" :options="formatOptions" @change="onFormatChange" />
  </a-form-item>
  ...
  <template v-if="activeData.__config__.startTimeRule">
    <a-form-item label="类型">
      <lydc-select v-model:value="activeData.__config__.startTimeType" placeholder="请选择" :options="typeOptions" @change="onStartTimeTypeChange" />
      <div class="mt-10px">
        ...
        <template v-else-if="activeData.__config__.startTimeType == 2">
          <lydc-select v-model:value="activeData.__config__.startRelationField" placeholder="请选择关联时间选择" :options="formFieldsOptions" />
        </template>
        <template v-else-if="activeData.__config__.startTimeType == 4 || activeData.__config__.startTimeType == 5">
          <a-input-number v-model:value="activeData.__config__.startTimeValue" :min="1" :precision="0" @change="onStartValueChange">
            <template #addonAfter>
              <lydc-select v-model:value="activeData.__config__.startTimeTarget" :options="getUnitOptions" class="!w-60px" />
            </template>
          </a-input-number>
        </template>
      </div>
  ...
  <template v-if="activeData.__config__.endTimeRule">
    <a-form-item label="类型">
      <lydc-select v-model:value="activeData.__config__.endTimeType" placeholder="请选择" :options="typeOptions" @change="onEndTimeTypeChange" />
      <div class="mt-10px">
        ...
        <template v-else-if="activeData.__config__.endTimeType == 2">
          <lydc-select v-model:value="activeData.__config__.endRelationField" placeholder="请选择关联时间选择" :options="formFieldsOptions" />
        </template>
        <template v-else-if="activeData.__config__.endTimeType == 4 || activeData.__config__.endTimeType == 5">
          <a-input-number v-model:value="activeData.__config__.endTimeValue" :min="1" :precision="0" @change="onEndValueChange">
            <template #addonAfter>
              <lydc-select v-model:value="activeData.__config__.endTimeTarget" :options="getUnitOptions" class="!w-60px" />
            </template>
          </a-input-number>
        </template>
      </div>
...
<script lang="ts" setup>
  ...
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && data.__config__.lydcKey === 'timePicker' && data.__vModel__ !== props.activeData.__vModel__) {
