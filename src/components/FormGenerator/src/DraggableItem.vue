    ...
    setup(props, { attrs }) {
      ...
      const layouts = {
        colFormItem(element, index, parent) {
          ...
          if (config.lydcKey == 'location') element.useAutoLocation = false;
          const Item =
            props.showType === 'app' && !useCompList.includes(config.lydcKey) ? (
        ...
        rowFormItem(element, index, parent) {
          ...
          if (element.__config__.lydcKey === 'tableGrid') {
          ...
          if (config.lydcKey === 'row' || (props.showType === 'app' && config.lydcKey === 'card')) {
            return (
              <a-col
                span={props.showType === 'app' ? 24 : config.span}
                data-draggable={true}
                ...
                <a-row class={className + ' drawing-row-item-row'}>
                  <span class="component-name">{config.lydcKey === 'card' ? config.label : config.componentName}</span>
                  <a-col>
