<script lang="tsx">
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, getParamList } from '/@/utils/lydc';
    ...
    setup(props, { expose, emit }) {
        ...
        colFormItem(element) {
          ...
          const Item = (
            <render
              ...
              {...listeners}
              ref={config.lydcKey === 'table' ? element.__vModel__ : undefined}
              relations={config.lydcKey === 'table' ? state.relations : undefined}
            />
          );
          ...
          Object.keys(scheme.on).forEach(key => {
            ...
            listeners['on' + upperFirst(key)] = (...arg) => {
              if (key === 'change') {
                ...
                if (['popupSelect', 'relationForm'].includes(config.lydcKey)) setTransferFormData(data, config);
      ...
      function comSet(field, prop, value) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (item.__vModel__ && item.__vModel__ === prop) {
              switch (field) {
                ...
                case 'endTime':
                  ...
                  break;
                case 'options':
                  if (dyOptionsList.indexOf(item.__config__.lydcKey) > -1) item.options = value;
                  break;
                default:
            ...
            if (item.__config__ && item.__config__.lydcKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
      ...
      function initFormData(componentList) {
        ...
        componentList.forEach(cur => {
          ...
          if (cur.__config__.lydcKey == 'table') return;
      ...
      function initRelationForm(componentList) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (config.lydcKey == 'relationFormAttr' || config.lydcKey == 'popupAttr') {
            const relationKey = cur.relationField.split('_lydcTable_')[0];
      ...
      function buildRules(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.lydcKey !== 'table') buildRules(config.children);
        });
      }
      ...
      function buildOptions(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.lydcKey !== 'table') buildOptions(config.children);
        });
      }
      function buildRelations(componentList, relations) {
        componentList.forEach(cur => {
          ...
          if (config.lydcKey === 'userSelect' && ['dep', 'pos', 'role', 'group'].includes(cur.selectType)) {
