<template>
  <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" class="right-board-form">
    ...
    <a-form-item label="标题宽度" v-if="!config.isSubTable && !layoutList.includes(lydcKey)">
      <a-input-number v-model:value="activeData.__config__.labelWidth" placeholder="请输入" :min="0" />
    </a-form-item>
    <a-form-item v-if="!config.isSubTable && !noWithList.includes(lydcKey) && activeData.style && showType === 'pc'">
      <template #label>
        控件宽度
    ...
    <template v-if="config.isSubTable && showType === 'pc'">
      <a-form-item label="冻结方式">
        <lydc-radio v-model:value="activeData.__config__.tableFixed" :options="fixedOptions" optionType="button" button-style="solid" class="right-radio" />
      </a-form-item>
      <a-form-item label="对齐方式">
        <lydc-radio v-model:value="activeData.__config__.tableAlign" :options="alignOptions" optionType="button" button-style="solid" class="right-radio" />
      </a-form-item>
    </template>
    ...
    <a-form-item label="控件Class" v-if="modelType == 1 || modelType == 6">
      <lydc-select v-model:value="activeData.__config__.className" multiple allowClear :options="getClassNameOptions" showSearch />
    </a-form-item>
    <a-form-item label="多端显示">
      <lydc-checkbox v-model:value="activeData.__config__.visibility" :options="getVisibilityOptions" />
    </a-form-item>
  </a-form>
</template>
<script lang="ts" setup>
  ...
  const getVisibilityOptions = computed(() => (unref(lydcKey) == 'iframe' ? iframeVisibilityOptions : visibilityOptions));
</script>
