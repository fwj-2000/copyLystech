<template>
  <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '100px' } }" class="right-board-form">
    <a-form-item label="表单尺寸">
      <lydc-radio v-model:value="formConf.size" :options="sizeOptions" optionType="button" button-style="solid" />
    </a-form-item>
    <a-form-item label="标签对齐">
      <lydc-radio v-model:value="formConf.labelPosition" :options="labelPositionOptions" optionType="button" button-style="solid" class="right-radio" />
    </a-form-item>
    ...
    <a-form-item label="弹窗类型" v-if="modelType != 3 && modelType != 6 && webType == 2 && !enableFlow">
      <lydc-select v-model:value="formConf.popupType" :options="popupTypeOptions" />
    </a-form-item>
    ...
    <a-form-item label="表单Class" v-if="modelType == 1 || modelType == 6">
      <lydc-select v-model:value="formConf.className" multiple allowClear :options="getClassNameOptions" showSearch />
    </a-form-item>
    <a-form-item label="主键策略">
      <lydc-select v-model:value="formConf.primaryKeyPolicy" :options="policyOptions" :disabled="!!formInfo.id" />
    </a-form-item>
    <a-form-item>
      <template #label>
        并发锁定
