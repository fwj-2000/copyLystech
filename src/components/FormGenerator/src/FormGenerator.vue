...
<script lang="ts" setup>
    ...
    return new Promise((resolve, reject) => {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.layout === 'colFormItem' && !noVModelList.includes(config.lydcKey) && !e.__vModel__) {
          ...
          if (config.lydcKey === 'billRule') {
            if (!config.rule) {
              reject({ msg: '单据组件“单据模板”属性为必填项', target: 1 });
              break;
            }
          }
          if (config.lydcKey === 'relationForm') {
            if (!e.modelId) {
          ...
          if (config.lydcKey === 'popupTableSelect') {
            if (!e.interfaceId) {
          ...
          if (config.lydcKey === 'autoComplete') {
            if (!e.interfaceId) {
          ...
          if (config.lydcKey === 'uploadFile' || config.lydcKey === 'uploadImg') {
          ...
          if (config.lydcKey === 'link' || config.lydcKey === 'iframe') {
  ...
  function createIdAndKey(item, parent: Nullable<GenItem> = null) {
    ...
    if (config.layout === 'colFormItem') {
      if (noVModelList.indexOf(config.lydcKey) < 0 || item.isStorage) {
        item.__vModel__ = !generatorStore.getHasTable ? toggleVmodelCase(`${config.lydcKey}Field${state.idGlobal}`) : '';
      }
      if (parent?.__vModel__ && parent?.__config__.lydcKey === 'table') {
        item.__config__.parentVModel = parent.__vModel__;
      }
    } else if (config.layout === 'rowFormItem') {
      if (config.lydcKey === 'table') {
        item.__vModel__ = toggleVmodelCase(`${config.lydcKey}Field${state.idGlobal}`);
      }
      config.componentName = `${config.lydcKey}${state.idGlobal}`;
  ...
  function shouldClone(_to, _from, target, _event, conf) {
    ...
    if (conf.__config__.lydcKey === 'table') {
      if (noTableAllowList.includes(targetConf.__config__.lydcKey)) {
        return false;
      }
    ...
    if (conf.__config__.lydcKey === 'tableGridTd' && targetConf.__config__.lydcKey === 'tableGrid') return false;
    return true;
  }
  // 预览表单
  ...
  function handleTableAddRow(element, insertPos, cloneRowIdx) {
    ...
    newRow.__config__.lydcKey = 'tableGridTr';
