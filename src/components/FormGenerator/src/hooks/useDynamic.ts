...
export function useDynamic(activeData, initFieldData?) {
  ...
  const defaultValue = computed(() => {
    if (activeData.__config__.lydcKey === 'checkbox' || activeData.__config__.lydcKey === 'cascader' || activeData.multiple) return [];
    return '';
  });
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && !noAllowRelationList.includes(data.__config__.lydcKey) && data.__vModel__ !== activeData.__vModel__) {
  ...
  const formFieldsSelectOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (Array.isArray(data)) data.forEach(d => loop(d, parent));
      if (data.__vModel__ && !noAllowSelectList.includes(data.__config__.lydcKey) && data.__vModel__ !== activeData.__vModel__) {
        const isTableChild = parent && parent.__config__ && parent.__config__.lydcKey === 'table';
        list.push({
          id: isTableChild ? parent.__vModel__ + '-' + data.__vModel__ : data.__vModel__,
          fullName: isTableChild ? parent.__config__.label + '-' + data.__config__.label : data.__config__.label,
