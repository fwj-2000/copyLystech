...
export function useField(activeData) {
  ...
  function initFieldData() {
    const lydcKey = activeData.__config__.lydcKey;
    const list = ['popupTableSelect', 'popupSelect', 'autoComplete'];
    if (list.includes(lydcKey)) {
    ...
    const url = list.includes(lydcKey) ? activeData.interfaceId : activeData.__config__.propsUrl;
