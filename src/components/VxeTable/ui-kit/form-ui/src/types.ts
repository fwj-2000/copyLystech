...
import type { VbenButtonProps } from '/@/components/VxeTable/ui-kit/shadcn-ui/index';
...
export type MaybeComponentPropKey = 'options' | 'placeholder' | 'title' | 'class' | keyof HtmlHTMLAttributes | (Record<never, never> & string);
...
export interface VbenFormProps<T extends BaseFormComponentType = BaseFormComponentType>
  ...
  i18nConfig?: {
    moduleCode: string;
    transferRange?: Array<'label' | 'placeholder'>;
  } & Partial<
    Record<
      LocaleType,
      {
         ...
         */
        transferRange?: Array<'label' | 'placeholder'>;
      } & Omit<Partial<VbenFormProps>, 'i18nConfig'>
    >
  >;
  /**
   * 路由参数配置
