<script setup lang="ts">
  ...
  import { FormControl, FormDescription, FormField, FormItem, FormMessage, VbenRenderContent } from '../../../shadcn-ui/index';
  import { isFunction, isObject, isString } from '/@/utils/is';
  import { cn } from '/@/utils/shared/src/index';
  ...
  const shouldDisabled = computed(() => {
    return isDisabled.value || disabled || computedProps.value?.disabled || computedProps?.value?.dynamicDisabled?.(values.value);
  });
  ...
  function createComponentProps(slotProps: Record<string, any>) {
    ...
    if (component === 'Input' && !binds.autocomplete) {
      binds['autocomplete'] = 'off';
    }
      ...
      <FormLabel
        ...
        :class="
          cn(
            'flex leading-6',
            {
              'flex-shrink-0 justify-end': !isVertical,
              'flex-row': isVertical,
            },
            labelClass,
          )
