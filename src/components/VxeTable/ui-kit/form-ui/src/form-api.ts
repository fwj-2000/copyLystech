...
import { Store } from '/@/utils/shared/src/store';
import { bindMethods, mergeWithArrayOverride, StateHandler } from '/@/utils/shared/src/index';
import { isDate, isDayjsObject, isFunction, isObject, isUnDef } from '/@/utils/is';
...
export class FormApi {
  ...
  async setValues(fields: Record<string, any>, filterFields: boolean = true, shouldValidate: boolean = false) {
     ...
     * 以上两种类型需要排除深度合并
     */
    const filterAndMerge = (incoming: Record<string, any>, current: Record<string, any>): Record<string, any> => {
      ...
      Object.keys(incoming).forEach(key => {
        ...
        if (
          ...
          isObject(newValue) &&
          !isDayjsObject(newValue) &&
          !isDate(newValue) &&
          isObject(currentValue) &&
          !Array.isArray(currentValue) &&
          !isDayjsObject(currentValue) &&
          !isDate(currentValue)
        ) {
          const nestedResult = filterAndMerge(newValue, currentValue);
    ...
    const filteredFields = filterAndMerge(fields, form.values);
  ...
  updateSchema(schema: Partial<FormSchema>[]) {
    console.log('🚀 ~ updateSchema ~ schema: ', schema);
