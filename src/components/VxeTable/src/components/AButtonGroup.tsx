import { FormItemContentRenderParams, VxeFormItemPropTypes, VxeGlobalRendererHandles, VxeGlobalRendererOptions } from 'vxe-table';
...
function createEditButtonRender() {
  return function (renderOpts: VxeGlobalRendererHandles.RenderTableEditOptions, params: VxeGlobalRendererHandles.RenderTableEditParams) {
    ...
    if (children) {
      return children.map((childRenderOpts: VxeGlobalRendererHandles.RenderTableEditOptions) => buttonEditRender(childRenderOpts, params)[0]);
    }
    return [];
  };
}
...
function createButtonItemRender() {
  return function (renderOpts: VxeFormItemPropTypes.ItemRender, params: FormItemContentRenderParams) {
    ...
    const { children } = renderOpts;
    if (children) {
      return children.map((childRenderOpts: VxeFormItemPropTypes.ItemRender) => buttonItemRender(childRenderOpts, params)[0]);
    }
    return [];
  };
}
