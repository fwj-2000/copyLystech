...
import { FormItemContentRenderParams, VxeFormItemPropTypes, VxeGlobalRendererHandles } from 'vxe-table';
...
export function isEmptyValue(cellValue: any) {
  return cellValue === null || cellValue === undefined || cellValue === '' || (XEUtils.isObject(cellValue) && XEUtils.isEmpty(cellValue));
}
...
export function getLabelValue(cellValue: any, labelField: string) {
  if (isObject(cellValue)) {
    return cellValue[labelField || 'label'];
  }
  return cellValue;
}
...
export function createDefaultRender(
  ...
  callBack?: (renderOpts: VxeGlobalRendererHandles.RenderDefaultOptions, params: VxeGlobalRendererHandles.RenderTableDefaultParams) => Record<string, any>,
) {
  return function (renderOpts: VxeGlobalRendererHandles.RenderDefaultOptions, params: VxeGlobalRendererHandles.RenderTableDefaultParams) {
...
export function createEditRender(
  defaultProps?: { [key: string]: any },
  callBack?: (renderOpts: VxeGlobalRendererHandles.RenderTableEditOptions, params: VxeGlobalRendererHandles.RenderTableEditParams) => Record<string, any>,
) {
  return function (renderOpts: VxeGlobalRendererHandles.RenderTableEditOptions, params: VxeGlobalRendererHandles.RenderTableEditParams) {
    const { row, column, $table } = params;
    const { name, attrs } = renderOpts;
    const cellValue = XEUtils.get(row, column.field as string);
    const args = (callBack && callBack(renderOpts, params)) ?? {};
    ...
    //     () => $table.updateStatus(params),
    //   ),
    // }, '129837198273821-----------------a9w8e791872318723');
    return [
      h(Component, {
        ...attrs,
        ...createProps(renderOpts, cellValue, defaultProps),
...
export function createFilterRender(
  defaultProps?: { [key: string]: any },
  callBack?: (renderOpts: VxeGlobalRendererHandles.RenderFilterOptions, params: VxeGlobalRendererHandles.RenderTableFilterParams) => Record<string, any>,
) {
  return function (renderOpts: VxeGlobalRendererHandles.RenderFilterOptions, params: VxeGlobalRendererHandles.RenderTableFilterParams) {
    const { column } = params;
    const { name, attrs } = renderOpts;
    const args = (callBack && callBack(renderOpts, params)) ?? {};
...
export function createFormItemRender(
  defaultProps?: { [key: string]: any },
  callBack?: (renderOpts: VxeFormItemPropTypes.ItemRender, params: FormItemContentRenderParams) => Record<string, any>,
) {
...
export function createCellRender(
  ...
  callBack?: (renderOpts: VxeGlobalRendererHandles.RenderTableCellOptions, params: VxeGlobalRendererHandles.RenderTableCellParams) => Array<any>,
) {
  return function (renderOpts: VxeGlobalRendererHandles.RenderTableCellOptions, params: VxeGlobalRendererHandles.RenderTableCellParams) {
    const args = (callBack && callBack(renderOpts, params)) ?? [];
    const cellLabel = getSelectCellValue && getSelectCellValue(renderOpts, params, ...args);
    const { placeholder } = renderOpts as any;
 ...
 * @author: *
 */
export function createExportMethod(getExportCellValue: Function, callBack?: (params: VxeGlobalRendererHandles.ExportMethodParams) => Array<any>) {
  return function (params: VxeGlobalRendererHandles.ExportMethodParams) {
    const { row, column, options } = params;
    const args = (callBack && callBack(params)) ?? [];
    return options && options.original ? XEUtils.get(row, column.field as string) : getExportCellValue(column.editRender || column.cellRender, params, ...args);
  };
}
...
export function createToolbarToolRender(
  defaultProps?: { [key: string]: any },
  callBack?: (renderOpts: VxeGlobalRendererHandles.RenderToolOptions, params: VxeGlobalRendererHandles.RenderToolParams) => Record<string, any>,
) {
