...
<script lang="ts" setup>
    ...
    if (state.activeStep == 0) {
      ...
      getImportPreview(state.apiUrl, { fileName: state.fileName })
        .then(res => {
          ...
          state.resultColumnsChildList = state.resultColumns.filter(o => o.lydcKey === 'table' && o.children && o.children.length);
          const preHeaderList = cloneDeep(headerList);
          for (let i = 0; i < preHeaderList.length; i++) {
            if (preHeaderList[i].children?.length && preHeaderList[i]?.lydcKey == 'table') {
          ...
          state.columnsChildList = state.columns.filter(o => o.lydcKey === 'table' && o.children && o.children.length);
          state.btnLoading = false;
          state.activeStep += 1;
        })
        .catch(() => {
  ...
  function getMergeList(list) {
    list.forEach(item => {
      if (item.children && item.children.length && item?.lydcKey == 'table') {
