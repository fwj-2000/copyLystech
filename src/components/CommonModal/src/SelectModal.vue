  ...
  <div class="common-container">
    <template v-if="config.popupType === 'dialog'">
      <a-modal
        ...
        <div class="lydc-common-search-box lydc-common-search-box-modal">
          ...
          <div class="lydc-common-search-box-right">
            <a-tooltip placement="top">
              <template #title>
                <span>{{ t('common.redo') }}</span>
              </template>
              <RedoOutlined class="lydc-common-search-box-right-icon" @click="initData" />
            </a-tooltip>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  function handleSubmit() {
    ...
    for (let i = 0; i < unref(selectRow).length; i++) {
      ...
      for (let j = 0; j < props.config.relationOptions.length; j++) {
        ...
        item[row.field] = row.type === 1 ? e[!unref(getIsDynamic) ? row.value + '_lydcId' : row.value] : row.value;
      }
      checked.push(item);
    }
    emit('select', checked);
  ...
  function getParamList() {
    ...
    for (let i = 0; i < templateJson.length; i++) {
      ...
      if (e.lydcKey === 'createUser') e.defaultValue = unref(getUserInfo).userId;
      if (e.lydcKey === 'createTime') e.defaultValue = new Date().getTime();
      if (e.lydcKey === 'currOrganize') e.defaultValue = unref(getUserInfo).organizeId;
      if (e.lydcKey === 'currPosition' && unref(getUserInfo).positionIds?.length) e.defaultValue = unref(getUserInfo).positionIds[0].id;
    }
    return templateJson;
  }
